...
export function envHelpers(args, deps = {}) {
	...
	ctx.fs.writeFileSync(path.join(ctx.cwd, 'node.config.json'), JSON.stringify(getNodeConfig(ctx), null, '\t') + '\n')
...
export function getNodeConfig(ctx) {
	...
	allow(write, serverconfig.writableConfigFile)
	...
	return {
		// node ignores this top-level key, but would reject it inside nodeOptions
		'//': 'auto-generated by proteinpaint/container/envHelpers.mjs on each server start, do not edit; to allow more paths, set PP_ALLOW_FS_READ or PP_ALLOW_FS_WRITE',
		nodeOptions: {
			permission: true,
...
function getServerconfigPaths(ctx) {
	...
	return {
		...
		// app-*.mjs rewrites the mounted serverconfig.json
		writableConfigFile: file
	}
}
...
export function getCreds(ctx) {
	...
	for (const name of new Set([...Object.keys(ctx.env), ...Object.keys(ctx.dotenv)])) {
		...
		if (!file || ctx.env[credsName]) continue
...
// the value of an env variable, or else of its entry in ./.env
function getEnvValue(name, ctx) {
	return ctx.env[name] || ctx.dotenv[name] || ''
}
