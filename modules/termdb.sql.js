...
*/
  ...
  const t1excluded = t1.values ? Object.keys(t1.values).map(Number) : [];
  const t1unannovals = t1excluded.length
    ? `AND value NOT IN (${t1excluded.join(",")})`
    : "";

  const t2excluded = t2.values ? Object.keys(t2.values).map(Number) : [];
...
*/
  ...
  }
  {
    ...
    const s = cn.prepare(
      "SELECT sample,value FROM annotations WHERE term_id=?"
    );
