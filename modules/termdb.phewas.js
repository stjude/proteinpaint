...
export async function update_image ( q, res ) {
	...
	for(const line of str.trim().split('\n')) {
		const [group_name, tmp] = line.split('\t')
		groups.push({
			group_name,
			categories: tmp.split(' ').map(i=>{ return {logp: Number(i)} })
		})
	}
	const result = {
...
function plot_canvas ( groups, result ) {
/*
...
*/
	...
	ctx.font = groupnamefontsize+'px arial'
	...
	const canvasheight = toppad
		+ axisheight
		+ groups.reduce((i,g)=>Math.max(i, ctx.measureText(g.group_name).width ), 0)
		+ dotlabelyspace
		+ bottompad
...
function write_resultfile ( groups ) {
	const lines = []
	for(const g of groups) {
		lines.push( g.group_name +'\t'+ g.categories.map(i=>i.logp).join(' ') )
	}
