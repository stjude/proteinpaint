	...
	queries:[
		{
			...
			genemcount:{
				query:n=>{
					return 'select gene_symbol,sample_name,variant_class from snvindel where gene_symbol=\''+n+'\' collate nocase'
					},
				symmary:(mlst,re)=>{
					let gene
					for(const m of mlst) {
			...
			makequery:q=>{
				const k=q.isoform
				if(!k) return null
				return 'select snvindel.*, sample_master.* from snvindel left join sample_master on snvindel.sample_name=sample_master.sample_name where snvindel.isoform_accession=\''+k+'\' collate nocase'
			},
			tidy:l=>{
				const m={
		...
		},
		{
			...
			makequery:q=>{
				const k=q.genename
				if(!k) return null
				return 'select fpkm.*, sample_master.* from fpkm left join sample_master on fpkm.sample=sample_master.sample_name where fpkm.gene=\''+k+'\' collate nocase'
			},
			tidy:item=>{
