...
module.exports={
	...
	snvindel_attributes:[
		{get:function(m){return m.mname || ''},label:'Mutation'},
		{get:function(m){return m.sample},label:'Sample',sort:true,descend:true},
		{get:function(m){return m.specimen || ''},label:'Specimen',hide:true},
		{label:'Genome pos.',hide:true,get:function(m){
			if(m.chr && m.pos) return m.chr+':'+(m.pos+1)
			return null
		}},
		{label:'Allele',
			lst:[
				{get:function(m){return m.ref || ''},label:'Ref',valuecenter:true},
				{get:function(m){return m.alt || ''},label:'Alt',valuecenter:true},
			]
		},
		{label:'Mut. Origin',get:function(m){return m.origin_type || ''}},
		{label:'Data set',get:function(m){return m.dataset_label || ''},hide:true},
		{label:'Committee classification',get:function(m){return m.committee_classification || ''},hide:true},
		{label:'PubMed',get:function(m){
			if(!m.pmid) return ''
			var out=[]
		...
		{label:'Somatic LOH',get:function(m){return m.loh || ''}},
		{label:'DNA MAF',
			lst:[
				{get:function(m){return m.maf_tumor},
				...
				{get:function(m){return m.maf_normal},
				readcountcredible:30,
				ismaf:true,
				width:40, height:12,
				label:'Normal',
	...
	queries:[
...

		{
			...
			genemcount:{
				query:n=>{
					return 'select gene_symbol,sample_name,variant_class from snvindel_hg19 where gene_symbol=\''+n+'\' collate nocase'
					},
				summary:(mlst,re)=>{
					let gene
					for(const m of mlst) {
			...
			makequery:q=>{
				const k=q.isoform
				if(!k) return null
				return 'select snvindel_hg19.*, sample_master.* from snvindel_hg19 left join sample_master on snvindel_hg19.sample_name=sample_master.sample_name where snvindel_hg19.isoform_accession=\''+k+'\' collate nocase'
			},
			tidy:l=> {
				const m={
...

		{
			...
			genemcount:{
				query:n=>{
					return 'select genes,sample_name from rna_fusion where genes like \'%'+n+'\' or genes like \'%'+n+',%\' collate nocase'
					},
				summary:(mlst,re)=>{
					for(const m of mlst) {
						const genes=m.genes,
			...
			makequery:q=>{
				const s=q.isoform
				if(!s) return null
				return 'select e.*,s.* from rna_fusion as e,sample_master as s '
					+'where (e.isoforms like \'%'+s.toUpperCase()+'\' or e.isoforms like \'%'+s.toUpperCase()+',%\''
					+' or e.genes like \'%'+s.toUpperCase()+'\' or e.genes like \'%'+s.toUpperCase()+',%\')'
...

		{
			...
			genemcount:{
				query:n=>{
					return 'select genes,sample_name from sv where genes like \'%'+n+'\' or genes like \'%'+n+',%\' collate nocase'
					},
				summary:(mlst,re)=>{
					for(const m of mlst) {
						const genes=m.genes,
			...
			makequery:q=>{
				const s=q.isoform
				if(!s) return null
				return 'select e.*,s.* from sv as e,sample_master as s '
					+'where (e.isoforms like \'%'+s.toUpperCase()+'\' or e.isoforms like \'%'+s.toUpperCase()+',%\''
					+' or e.genes like \'%'+s.toUpperCase()+'\' or e.genes like \'%'+s.toUpperCase()+',%\')'
					+'and e.sample_name=s.sample_name collate nocase'
				},
...

		{
			...
			makequery:q=>{
				const k=q.genename
				if(!k) return null
				return 'select mbarrayexpression.*, sample_master.* from mbarrayexpression left join sample_master on mbarrayexpression.sample=sample_master.sample_name where mbarrayexpression.gene=\''+k+'\' collate nocase'
			},
			config:{
...

		{
			...
			makequery:q=>{
				const k=q.genename
				if(!k) return null
				return 'select pcgp_fpkm.*, sample_master.* from pcgp_fpkm left join sample_master on pcgp_fpkm.sample=sample_master.sample_name where pcgp_fpkm.gene=\''+k+'\' collate nocase'
			},
			config:{
				// client-side, tk-specific attributes
				gtexlink:true,
					...
					get:function(m){
						...
						case common.dtnloss:
							if(m.ratio!=undefined) {
								return m.ratio+(m.chimericreads ? ' ('+m.chimericreads+'/'+Math.ceil(m.chimericreads/m.ratio)+')' : '')
							}
							return ''
						case common.dtitd:
