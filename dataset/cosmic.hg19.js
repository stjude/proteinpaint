	...
	queries:[
		{
			...
			genemcount:{
				query:n=>{
					return [
							'select annovar_sj_gene,sample_name,annovar_sj_class from cosmic_hg19 where annovar_sj_gene=?',
							n
						]
					},
				summary:(mlst,re)=>{
		...
		},
		{
			...
			makequery:q=>{
				...
				return [
					'select * from cosmic_fusion where isoforms like ? or isoforms like ?',
					['%'+n, '%'+n+'%']
				},
			tidy:r=>{
				let fusion
