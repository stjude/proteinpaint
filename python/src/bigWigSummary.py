...
import requests
from topVEgene import _read_stdin_payload
...
def get_bigwig_stats(bw_file:str, chrom:str, start:int, end:int, n_bins:int) -> list[float | None]:
    try:
        ...
        stats = bw.stats(chrom, start, end, type="mean", nBins=n_bins)
        bw.close()
        return stats
...
def main() -> int:
    try:
        ...
        if not isinstance(json_args, dict):
            raise ValueError("Input JSON must be an dict object")
