...
tape("filter term-value button", function(test) {
	...
	function testFilterRemove(obj) {
		test.equal(
			obj.obj.dom.termfilterdiv.selectAll(".term_name_btn").size(),
			0,
	...
	function testAddTerm(obj) {
		test.equal(
			obj.obj.dom.termfilterdiv.selectAll(".term_name_btn").size(),
			1,
			"should add term value filter for clicked term"
		)
		obj.tip.hide()
		test.end()
	}
...
tape("filter term-value button: categorical term", function(test) {
	...
	function testFilterDisplay(obj) {
		...
		setTimeout(() => {
			test.equal(
				obj.obj.dom.termfilterdiv.selectAll(".condition_btn").size(),
				1,
				"should have 'IS' button for categorical filter"
			)
			test.equal(
				obj.obj.dom.termfilterdiv.selectAll(".add_value_btn").size(),
				1,
