...
tape('allgm2sum() - complex scenario with multiple chromosomes and strands', test => {
	...
	test.ok(chr2Regions.every(r => r.reverse), 'chr2 regions should be marked as reverse')
	test.ok(chr1Regions.every(r => !r.reverse), 'chr1 regions should not be marked as reverse')
	test.end()
})
