...
export class ListSamples {
	...
	constructor(app: AppApi, termfilter: any, config: any, plot: Plot, getRange = true, start?: number, end?: number) {
		...
		if (this.hasGeneVariantTerm()) {
			...
			this.getTvsLst(rangeStart, rangeStop, getRange, this.t1, this.t2)
		}
	}
	...
	handleGeneVariantTerm(rangeStart: number, rangeStop: number, getRange: boolean): void {
		...
		this.getTvsLst(rangeStart, rangeStop, getRange, tw, null)
	}

	getTvsLst(rangeStart: number, rangeStop: number, getRange: boolean, tw1: TermWrapper, tw2?: TermWrapper | null) {
		if (tw2) {
			this.terms.push(tw2)
			this.handleTw2(tw1, tw2, getRange, rangeStart, rangeStop)
		} else {
			if (getRange) this.createTvsLstRanges(tw1, rangeStart, rangeStop, 0)
		}
	}

	handleTw2(tw1: TermWrapper, tw2: TermWrapper, getRange: boolean, rangeStart: number, rangeStop: number) {
		...
		if (isTw1CatOrCond) {
			...
			this.tvslst.lst[0].tvs.ranges = this.createOverlayTwLst(tw2)
	...
	createTvsLstRanges(tw: TermWrapper, rangeStart: number, rangeStop: number, lstIdx: number) {
		this.createTvsTerm(tw)
	...
	createOverlayTwLst(tw2: TermWrapper) {
