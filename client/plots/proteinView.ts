...
class ProteinView implements RxComponent {
...
function renderFCSummary(holder: any, data: any, self: ProteinView) {
	...
	for (const assay of nonPTMAssays) {
		for (const c of assay.nonPTMCohorts || []) {
			...
			if (Number.isFinite(log2ratio)) maxAbs = Math.max(maxAbs, Math.abs(log2ratio as number))
		}
	}
	...
	for (const assay of nonPTMAssays) {
		...
		for (const c of assay.nonPTMCohorts || []) {
			...
			row
				...
				.attr('title', 'Click to show violin plot')
				.on('click', () => launchViolinPlot(self, assayName, cohortName))
				.text(Number.isFinite(log2ratio) ? (log2ratio as number).toFixed(3) : 'NA')
		}
	}
}
...
async function renderPTMLollipop(holder: any, data: any, self: ProteinView) {
	...
	const gmCache = new Map<string, any>()
...
function parsePTMModSites(modSites: string | null | undefined): number | null {
...
async function getGmForPTM(geneName: string, genomeName: string, gmCache: Map<string, any>): Promise<any> {
...
function getLog2Ratio(foldChange: number): number | null {
