...
export function displayLabelClickMenu(t1, t2, plot, event, self) {
	displayLabelClickMenu.called = true
...
function displayMenu(t1, t2, self, plot, event, start, end) {
	self.app.tip.d.selectAll('*').remove()

	const options = []

	if (displayLabelClickMenu.called === true) {
		if (t2) {
		...
		displayLabelClickMenu.called = false
...
export function labelHideLegendClicking(t2, plot, self) {
