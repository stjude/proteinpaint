...
- **Gene Expression**: Uses edgeR, limma, or Wilcoxon for differential gene expression. Launches volcano plot and gene set enrichment analysis. Clicking a data point in the volcano plot opens a box plot for that gene.
- **DNA Methylation**: Uses limma on promoter-level M-values for differential methylation. Launches volcano plot. Results are at the promoter level (ENCODE cCRE regions), not individual CpG probes. The R script is `R/src/diffMeth.R`. Box plot on click is not supported.
