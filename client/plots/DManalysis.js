...
class DManalysis {
	...
	async setControls(output) {
		...
		this.components.controls.on('downloadClick.DManalysis', () => {
	...
	async main() {
		...
		this.settings = this.config.settings.DManalysis
		...
		console.log('This:')
		console.log(this)
		const output = await runDManalysis(this)
		console.log(output)
		wait.remove()
		...
		this.dom.header
			...
			.text('DIFFERENTIAL METABOLITES INTENSITY')
		console.log('output:')
		console.log(output)
...
- vo_circle
	*/
	...
	//self.config.settings.DManalysis.min_count = self.settings.min_count
	//self.config.settings.DManalysis.min_total_count = self.settings.min_total_count
...
export async function getPlotConfig(opts, app) {
	try {
		...
		throw `${e} [DManalysis getPlotConfig()]`
	}
}

export const DManalysisInit = getCompInit(DManalysis)
// this alias will allow abstracted dynamic imports
export const componentInit = DManalysisInit
...
async function runDManalysis(self) {
	const output = await dofetch3('DManalysis', {
		body: {
