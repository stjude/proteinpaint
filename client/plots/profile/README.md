...
`termdbConfig.plotConfigByCohort[cohort].profileForms2.domain2plotType` (`Record<domainId, friendlyLabel[]>`) drives the tab list and per-tab tree filter. Declared as a static map in the dataset config in [`sjglobal.profile.ts`](../../../../dataset/sjglobal.profile.ts) and shipped to the client as part of the existing `plotConfigByCohort` payload — no separate field on the wire, no SQL, no DB read. Adding or removing a template-bearing domain is a code edit reviewable in PRs. An empty inner submap (e.g., Abbreviated today) triggers the picker's empty-state message.

[`isUsableTerm`](../../../../shared/utils/src/termdb.usecase.js) `case 'profileForms2'` honors `usecase.cohort` and `usecase.subtype` (set by the picker): depth-3 gets `'plot'` only if `plotConfigByCohort[cohort].profileForms2.domain2plotType[term.id]` includes the active subtype; depth-1/2 gets `'branch'` only if any descendant matches. Terms with empty `uses` are entirely excluded by `tree.js`'s filter.
