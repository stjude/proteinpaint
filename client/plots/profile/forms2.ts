...
class ProfileForms2 extends profilePlot {
	static type = 'profileForms2' as const
	id: any
	svg: any
	components: any
	dom: any
	...
	async init(appState) {
		...
		const settings = config.settings.profileForms2
		this.tabs = []
	...
	/**
	 * Override setControls() to fetch data from the dedicated forms2 route.
	 * The base class setControls() builds filter UI and sets this.filter but
	 * skips the data fetch for profileForms2 (see profilePlot.ts).
	 */
	async setControls(additionalInputs: any[] = []) {
		await super.setControls(additionalInputs)
		this.data = await this.fetchFormsAggregatedScores()
		if (this.data && 'error' in this.data) throw this.data.error
	}

	private async fetchFormsAggregatedScores() {
		// No facilityTW — server derives it from activeCohort on __protected__.
		// scoreTerms / scScoreTerms stripped to { term: { id }, q } only —
		// server fills the rest via termjsonByOneid.
		return dofetch3('termdb/profileForms2Scores', {
			body: {
	...
	renderPlot() {
		try {
			...
			console.error('Error rendering profile forms2 plot:', e)
			throw e
		}
	}
	...
	renderLikert() {
		...
		for (const tw of this.scoreTerms) {
			if (tw.term.type != 'multivalue') continue
			const dict = this.getPercentageDict(tw)
...
export async function getPlotConfig(opts, app, _activeCohort) {
	...
	config.settings = getDefaultProfileForms2Settings()
...
export function getDefaultProfileForms2Settings() {
	const settings = {
		controls: {
			isOpen: false
		},
		profileForms2: { svgw: 420, svgh: 480 }
	}
	const profilePlotSettings = getDefaultProfilePlotSettings()
	settings.profileForms2 = copyMerge(settings.profileForms2, profilePlotSettings)

	return settings
}

export const profileForms2Init = getCompInit(ProfileForms2)
// this alias will allow abstracted dynamic imports
export const componentInit = profileForms2Init
