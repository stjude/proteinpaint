...
export function combineComPerTerms(self) {
	const CPterms = self.termOrder.filter(t => t.grp.type == 'compositePercentage')
	const { grpIndex, index, lstIndex, prevGrpTotalIndex, totalIndex, visibleGrpIndex } = CPterms[0]

	const termName = CPterms[0].grp.name == 'Mutation Signature' ? '% SNVs' : 'percentage'
	const CPcombinedTerm = {
		grp: CPterms[0].grp,
		grpIndex,
		index,
		lstIndex,
		...
		tw: {
			$id: CPterms[0].tw.$id,
			q: { mode: 'continuous' },
			term: { name: termName, id: termName, type: 'compositePercentage', isleaf: true },
			type: 'compositePercentage'
		}
	}
	self.termOrder.splice(totalIndex, CPterms.length, CPcombinedTerm)

	for (const ufs of self.unfilteredSampleOrder) {
		const combined = {
			values: [],
			key: CPcombinedTerm.grp.name,
			label: CPcombinedTerm.grp.label
		}

		let total_value = 0
		for (const cpTerm of CPterms) {
			const row = ufs.row[cpTerm.tw.$id]
			if (!row) continue
		...
		for (const cpTerm of CPterms) {
			const row = ufs.row[cpTerm.tw.$id]
			if (!row) continue
			combined.values.push({
				label: cpTerm.tw.term.name,
				value: (row.value / total_value) * 100,
				pre_val_sum,
				color: cpTerm.tw.term.color
			})
			pre_val_sum += (row.value / total_value) * 100
			delete ufs.row[cpTerm.tw.$id]
		}
		ufs.row[CPterms[0].tw.$id] = combined
	}
}
