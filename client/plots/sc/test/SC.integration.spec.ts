...
tape('Test initial SC app rendering', test => {
	...
	async function runTests(sc) {
		...
		// Test: Header row includes the configured sample column label
		const headerRow = table.querySelector('thead tr')
		test.ok(headerRow, 'Table has a header row')
		const headerCells = Array.from(headerRow.querySelectorAll('th')) as HTMLElement[]
		test.ok(headerCells.length >= 1, 'Header row has at least one th cell')
		const sampleHeader = headerCells.find((cell: HTMLElement) =>
			cell.textContent?.toLowerCase().includes(sampleColumnLabel.toLowerCase())
		)
		test.ok(sampleHeader, 'Sample column header is rendered with the configured label')

		// Test: Table body has one row with scTestSample under the sample column
