...
export async function makeChartBtnMenu(holder, chartsInstance) {
	...
	make_one_checkbox({
		...
		checked: true,
		testid: 'sjpp-summarizeMutationCnv-useSameGeneCheckbox',
	...
	// mutation variable
	{
		...
		const result = addGeneSearchbox({
			...
			tip,
			searchOnly: 'gene',
			testid: 'sjpp-summarizeMutationCnv-genesearch-mutation',
	...
	// cnv variable
	{
		...
		const result = addGeneSearchbox({
			...
			tip,
			searchOnly: 'gene',
			testid: 'sjpp-summarizeMutationCnv-genesearch-cnv',
			genome: chartsInstance.app.opts.genome!,
			callback: async () => {
	...
	const submitBtn = holder
		.append('button')
		.attr('data-testid', 'sjpp-summarizeMutationCnv-submitBtn')
		.text('Launch Plot')
