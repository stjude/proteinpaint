	...
	getValidationError(config: any) {
		for (const sections of [config.rowSections || [], config.colSections || []]) {
			...
			for (const section of sections as Section[]) {
				...
				if (types.size != 1 || !termType || !types.has(termType))
					return 'Every section must contain exactly one term type.'
		...
		if (this.hasTerms(config.rowSections) && this.hasTerms(config.colSections)) {
			...
			if (this.state.availableMethods.length < 2)
				return 'Selected column terms do not share at least two aggregate methods.'
			const availableIds = new Set(this.state.availableMethods.map(method => method.id))
			if (config.sizeMethod && !availableIds.has(config.sizeMethod))
				return 'The selected size method is not compatible.'
			if (config.gradientMethod && !availableIds.has(config.gradientMethod))
				return 'The selected gradient method is not compatible.'
		}
		try {
