...
tape('getOpacity, getStrokeWidth, getShape, and getCoordinates behave as expected', function (test) {
	...
	test.equal(
		...
		0,
		'Should hide category-hidden samples.'
	)
	test.equal(
		model.getStrokeWidth({ sampleId: 's2', sample: 'Beta', hidden: { category: true } }),
		0,
		'Should emit no stroke width for hidden samples.'
	)
	test.equal(
		model.getOpacity({ x: 0, y: 0, isRef: true }),
		0.6,
		'Should use reference opacity settings for ref markers.'
	)
...
tape('getScale and transform support fixed and data-driven sizing', function (test) {
	...
	test.equal(
		...
		settings.maxShapeSize / sample.scale,
		'Should use settings.size with data-driven sizing disabled.'
	)
	test.equal(
		model.getScale(chart, ref),
		settings.maxShapeSize / ref.scale,
		'Should use settings.refSize for reference points.'
	)
	...
	test.equal(
		model.getScale(chart, sample),
		0.75,
		'Should map descending scale values across the configured size range.'
	)
	...
	test.equal(
		...
		1.2000000000000002,
		'Should shrink non-matching sample scale under the filter.'
	)
...
tape('getColor supports categorical and continuous clamping', function (test) {
	...
	test.equal(
		...
		'rgb(10,0,0)',
		'Should clamp low continuous values to the minimum range.'
	)
	test.equal(
		model.getColor({ sampleId: 's1', category: 25 }, chart),
		'rgb(20,0,0)',
		'Should clamp high continuous values to the maximum range.'
	)
	test.equal(
		model.getColor({ sampleId: 's1', category: 12 }, chart),
		'rgb(12,0,0)',
		'Should use the value directly within the range.'
	)
	test.equal(
		model.getColor({ category: 'Default', isRef: true }, chart),
		'#ce768e',
		'Should use the scatter default color for the Default reference category.'
	)
	test.equal(
		model.getColor({ category: 'A', isRef: true }, chart),
		'#f00',
		'Should use the color legend mapping for categorical reference values.'
	)
	test.end()
})
...
tape('initColorDefaults seeds gradient colors for continuous color maps', function (test) {
	...
	test.equal(
		model.scatter.config.startColor.Default,
		'#f5f5f5',
		'Should use the no-exp color for gene-expression start gradients.'
	)
	test.equal(
		model.scatter.config.stopColor.Default,
		'#ff000d',
		'Should use the exp color for gene-expression stop gradients.'
	)
	...
	test.ok(
		typeof baseModel.scatter.config.startColor.A === 'string',
		'Should create a default start color for non-gene-expression gradient maps.'
	)
	test.ok(
		typeof baseModel.scatter.config.stopColor.A === 'string',
		'Should create a default stop color for non-gene-expression gradient maps.'
	)
	test.end()
})
...
tape('initAxes applies color scale modes and date axis orientation', function (test) {
	...
	test.ok(
		dateChart.yAxisScaleTime(lateDate) < dateChart.yAxisScaleTime(earlyDate),
		'Should render later dates above earlier dates.'
	)
	test.end()
})
