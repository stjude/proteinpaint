...
tape('getDataRequestOpts builds the single-cell request payload', function (test) {
	...
	test.deepEqual(
		opts.coordTWs,
		[scatter.config.term, scatter.config.term2],
		'Should pass both coordinate term wrappers.'
	)
	...
	test.equal(
		opts.canvasSettings.startColor,
		scatter.config.startColor.Default,
		'Should include the default gradient start color.'
	)
	test.equal(
		opts.canvasSettings.stopColor,
		scatter.config.stopColor.Default,
		'Should include the default gradient stop color.'
	)
	test.equal(
		opts.canvasSettings.devicePixelRatio,
		window.devicePixelRatio || 1,
		'Should honor the browser device pixel ratio.'
	)
	test.end()
})
