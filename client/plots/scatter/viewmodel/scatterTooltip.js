...
export class ScatterTooltipViewModel {
	constructor(scatter) {
	...
	showTooltip(event, chart) {
		...
		threshold = threshold / this.scatter.model.zoom //Threshold should consider the zoom
		...
		if (this.scatter.config.colorTW) this.addNodes('category', this.scatter.config.colorTW, level)
	...
	getTW(category) {
		const tw =
			category == 'category'
				...
				: category == 'scale'
				? this.scatter.config.scaleDotTW
				: category == 'X' && this.scatter.config.term
				? this.scatter.config.term
				: category == 'Y' && this.scatter.config.term2
				? this.scatter.config.term2
				: null
		return tw
	}
			...
			if (showIcon) {
				const color =
					tw == this.scatter.config.colorTW
						? this.scatter.getColor(sample, chart)
						: this.scatter.config.colorTW
						? 'gray'
				...
				if (tw?.term.type == 'geneVariant' && tw.q.type == 'values') {
					...
					if (onClick) {
						row
							...
							.text('Lollipop')
							.on('click', async e => {
								await this.scatter.viewModel.openLollipop(label)
			...
			for (const sample of node.samples) {
				...
				if ('sampleId' in sample && this.onClick) {
					row
						...
						.append('button')
						.text('Sample view')
						.on('click', e => this.scatter.viewModel.openSampleView(sample))
					if (hasDiscoPlot)
						row
							.append('td')
							.append('button')
							.text('Disco')
							.on('click', async e => this.scatter.viewModel.openDiscoPlot(sample))
					...
					if (hasMetArrayPlot)
						row
							...
							.append('button')
							.text('Met Array')
							.on('click', async e => this.scatter.viewModel.openMetArray(sample))
				}
			}
		}
	}
	...
	addNodes(category, tw, parentTW, level) {
		for (const sample of this.samples) {
			...
			let parent = this.tree.find(item => item.id == parentId)
			if (!node) {
