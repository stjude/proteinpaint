import { ColorScale, getMaxLabelWidth } from '#dom'
import { rgb } from 'd3-color'
import { morigin, dt2label } from '#shared/common.js'
import { shapes } from '../model/scatterModel.js'
import { roundValueAuto } from '#shared/roundValue.js'
import { ScatterLegendInteractivity } from '../interactions/scatterLegendInteractivity.js'

export class ScatterLegend {
	constructor(scatter) {
		this.scatter = scatter
		this.scattervm = scatter.scattervm
		this.model = scatter.model
		this.interactivy = scatter.interactivy
	...
	renderLegend(chart, step) {
		...
		if (!this.scatter.config.colorTW && !this.scatter.config.shapeTW && !this.scatter.config.colorColumn) {
			if (this.scatter.config.scaleDotTW) {
				chart.scaleG = legendG
					.append('g')
					.attr('transform', `translate(${offsetX + 45},${this.model.legendHeight - 150})`)
		...
		if (this.filterSampleStr) title0 += `, search = ${this.filterSampleStr}`
		...
		if (this.scatter.config.colorTW || this.scatter.config.colorColumn) {
			...
			if (colorRefCategory?.sampleCount > 0) {
				...
				refColorG.on('click', e => this.legendInteractivy.onLegendClick(chart, 'colorTW', 'Ref', e, colorRefCategory))
				...
				refText.on('click', e => this.legendInteractivy.onLegendClick(chart, 'colorTW', 'Ref', e, colorRefCategory))
			}
			...
			if (this.scatter.config.shapeTW.term.type == 'geneVariant' && this.scatter.config.shapeTW.q.type == 'values')
				...
				for (const [key, shape] of chart.shapeLegend) {
					...
					itemG
						...
						.attr('alignment-baseline', 'middle')
					offsetY += step
					itemG.on('click', event => this.legendInteractivy.onLegendClick(chart, 'shapeTW', key, event, shape))
				}
			}
	...
	drawScaleDotLegend(chart) {
		...
		scaleG
			.append('rect')
			.attr('width', 110 * this.scatter.model.zoom)
			.attr('height', 50)
			...
			.on('click', e => {
				...
				function addRadio(text) {
					...
					input.on('change', e => {
						this.scatter.config.settings.sampleScatter.scaleDotOrder = e.target.value
						self.app.dispatch({
							type: 'plot_edit',
							id: self.id,
							config: this.scatter.config
