...
export class ScatterLasso {
	...
	showTable(group, x, y, addGroup) {
		...
		if (this.scatter.config.term) columns.push(formatCell(this.scatter.config.term?.term?.name || 'X', 'label'))
		if (this.scatter.config.term2) columns.push(formatCell(this.scatter.config.term2?.term?.name || 'Y', 'label'))
