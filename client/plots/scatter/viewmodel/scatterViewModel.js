...
import { ScatterTooltipViewModel } from './scatterTooltip.js'
...
export class ScatterViewModel {
	constructor(scatter) {
		...
		this.tooltipvm = new ScatterTooltipViewModel(scatter)
		this.lassovm = new ScatterLasso(scatter)
	...
	renderChart(chart, div) {
		...
		chart.chartDiv.on('mouseover', event => {
			if (!this.tooltipvm.onClick) this.tooltipvm.showTooltip(event, chart)
		})
		chart.chartDiv.on('click', event => this.tooltipvm.showTooltip(event, chart))
	...
	async renderSVG(chart, s) {
		...
		let fontSize = this.legendvm.getFontSize(chart)
	...
	toggleLasso() {
		this.lassovm.lassoOn = !this.lassovm.lassoOn
		for (const chart of this.model.charts) {
			if (this.lassovm.lassoOn) {
	...
	setTools() {
		...
		icon_functions['restart'](homeDiv, { handler: () => this.resetToIdentity(zoomD3), title: 'Reset plot to defaults' })
		...
		if (!(this.model.is2DLarge || this.model.is3D)) {
			icon_functions['search'](searchDiv, { handler: e => this.searchSample(e), title: 'Search samples' })
			icon_functions['lasso'](this.lassoDiv, {
				handler: () => this.toggleLasso(),
				enabled: this.lassovm.lassoOn,
				title: 'Select a group of samples'
		...
		const mainG = this.model.charts[0].mainG

		if (this.scatter.config.scaleDotTW && this.zoom > 4) this.resetToIdentity()
		mainG.call(zoomD3)
		for (const chart of this.model.charts) {
			chart.lasso = d3lasso()
			this.lassovm.lassoReset(chart)
		}
	}
	...
	handleZoom(event) {
		for (const chart of this.model.charts) {
			...
			this.model.zoom = event.transform.scale(1).k
			//on zoom in the particle size is kept
			const symbols = chart.serie.selectAll('path[name="serie"')
			symbols.attr('transform', c => this.model.transform(chart, c, 1))
			if (this.lassovm.lassoOn) chart.lasso.selectedItems().attr('transform', c => this.model.transform(chart, c, 1.2))
			if (this.scatter.config.scaleDotTW) this.legendvm.drawScaleDotLegend(chart)
		}
	}

	zoomIn() {
		for (const chart of this.model.charts)
			if (this.model.is2DLarge) this.model.zoom = this.model.zoom + 0.15
	...
	resetToIdentity(zoom) {
		for (const chart of this.model.charts)
			if (this.model.is2DLarge) this.model.zoom = 1
