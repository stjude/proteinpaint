...
type DMVolcanoPlotConfig = PlotConfig & {
	confounderTws: TermWrapper[]
	highlightedData: string[]
	samplelst: {
		groups: {
...
type DapVolcanoPlotConfig = PlotConfig & {
	highlightedData: string[]
	proteomeDetails: { organism: string; assay: string; cohort: string }
	termType: 'proteomeDAP'
}

/** Final config created in getPlotConfig() */
export type VolcanoPlotConfig = DEVolcanoPlotConfig | DMVolcanoPlotConfig | SCCTPlotConfig | DapVolcanoPlotConfig
...
export type VolcanoViewData = {
	...
	xOffset: number
	/** Whether the server centred the x axis. Separate from xOffset, which is also 0 for a centred
	 * run whose median effect happens to be exactly 0. */
	centered: boolean
