...
import { domainBins, domainBinBp } from '#shared/dmrDomainBins.js'
import { DOMAIN_MAP_MIN_DMRS, DOMAIN_MAP_CLAMP } from '../interactions/domainMap'
...
map thresholds are the ones the caption states
*/
...
const dmr = (chr: string, start: number, direction: string) => ({ chr, start, direction })

tape('\n', t => {
	t.comment('-***- volcano DMR scan domain map specs -***-')
	t.end()
})
...
tape('domainBins counts by chromosome, bin and direction', t => {
	...
	const b = domainBins(rows, ['chr1', 'chr2'], lens, 1e6)
	t.equal(b.chr1.length, 20, 'chr1 gets 20 bins of 1 Mb')
	t.equal(b.chr2.length, 10, 'chr2 gets 10')
	t.deepEqual(b.chr1[0], [2, 0], 'two hyper DMRs in the first Mb')
	t.deepEqual(b.chr1[1], [0, 1], 'the DMR at exactly 1 Mb starts the second bin')
	t.deepEqual(b.chr1[19], [1, 0], 'the last base lands in the last bin')
	t.deepEqual(b.chr2[0], [0, 1], 'chr2 counted separately')
	t.end()
})

tape('domainBins clamps past-the-end DMRs and drops unrequested chromosomes', t => {
	const lens = { chr1: 10e6 }
	const b = domainBins([dmr('chr1', 10_500_000, 'hyper'), dmr('chrY', 5, 'hyper')], ['chr1'], lens, 1e6)
	t.deepEqual(b.chr1[9], [1, 0], 'a DMR past the chromosome end clamps into the last bin')
	t.equal('chrY' in b, false, 'a chromosome that was not scanned gets no track')
	t.equal(
		b.chr1.reduce((s, x) => s + x[0] + x[1], 0),
		1,
