...
let inputIndex = 0
const svgIcons = {
	externalLink: `<svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 512 512" transform="scale(0.8)" style="top:2px;position:relative" ><!--!Font Awesome Free 6.5.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2023 Fonticons, Inc.--><path d="M320 0c-17.7 0-32 14.3-32 32s14.3 32 32 32h82.7L201.4 265.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L448 109.3V192c0 17.7 14.3 32 32 32s32-14.3 32-32V32c0-17.7-14.3-32-32-32H320zM80 32C35.8 32 0 67.8 0 112V432c0 44.2 35.8 80 80 80H400c44.2 0 80-35.8 80-80V320c0-17.7-14.3-32-32-32s-32 14.3-32 32V432c0 8.8-7.2 16-16 16H80c-8.8 0-16-7.2-16-16V112c0-8.8 7.2-16 16-16H192c17.7 0 32-14.3 32-32s-14.3-32-32-32H80z"/></svg>`
}
		...
		if (self.state.termdbConfig.urlTemplates) {
			...
			if (self.chartType != 'hierCluster' && templates.sample) {
				...
				if (!templates.sample.regex /*|| name has a matching pattern */) {
					const menuDiv = self.dom.clickMenu.d
						...
						.html(
							`${s.controlLabels.Sample} summary: <span class='linkText'>${sampleData._SAMPLENAME_} ${svgIcons.externalLink}</span>`
						)
			...
			if (sampleData.tw?.term?.type == 'geneVariant' && templates.gene) {
				const menuDiv = self.dom.clickMenu.d
					.append('div')
					.attr('class', 'sja_menuoption sja_sharp_border')
					.html(`Gene summary: <span class='linkText'>${sampleData.tw.term.name} ${svgIcons.externalLink}</span>`)
					.on('click', async event => {
	...
	self.showTermMenu = async function (event) {
		...
		if (self.state.termdbConfig.urlTemplates?.gene && vartype == 'gene') {
			...
			self.dom.geneSummaryLink = labelEditDiv
				...
				.html(`Gene summary ${svgIcons.externalLink}`)
				.on('click', async () => {
