...
class GRIN2 extends RxComponentInner {
	...
	//Style parameters
	//Set here to easily adjust styles
	...
	private createConfigTable() {
		...
		/** These two nested funcs allow for the styling to be updated easily
		...
		if (queries.svfusion) {
			...
			addDataTypeTd(fusionRow, 'Fusion')
		...
		const generalOptionsCell = addOptsTd(generalRow)
		generalOptionsCell
			.style('color', this.optionsTextColor)
			.style('font-style', 'italic')
