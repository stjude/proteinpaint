...
import { getDefaultViolinSettings } from './violin/settings/defaults'
import { getDefaultBarSettings } from './barchart/barchart.ts'
...
export async function getPlotConfig(opts, app) {
	try {
		...
		// want to override with q.mode from the default q
		if (opts.term2)
			opts.term2 = await fillTermWrapper(
				opts.term2,
				app.vocabApi,
				opts.term2.bins || opts.term2.q ? undefined : getT0T2defaultQ()
			)
		if (opts.term0)
			opts.term0 = await fillTermWrapper(
				opts.term0,
				app.vocabApi,
				opts.term0.bins || opts.term0.q ? undefined : getT0T2defaultQ()
			)
