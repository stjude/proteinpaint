...
function mayaddGetter_singleSampleGenomeQuantification(tk, block) {
	if (!tk.mds.queries?.singleSampleGenomeQuantification) return
	for (const k in tk.mds.queries.singleSampleGenomeQuantification) {
		tk.mds.queries.singleSampleGenomeQuantification[k].get = async sample => {
			const body = {
				genome: block.genome.name,
				dslabel: tk.mds.label,
