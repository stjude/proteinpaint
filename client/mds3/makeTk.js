...
export async function makeTk(tk, block) {
	...
	/* keys are mclass or dt. generated by flags such as hardcodeCnvOnly and snvIndelOnly,
	to silently hide some data types from display and legend
	this setting is invisible to user, not user-controllable, and should be like "frozen" at runtime
	this is not mixed into legend.mclass.hiddenvalues as that is updated by user choices through legend
	this is combined with legend.mclass.hiddenvalues in request payload for data filtering on backend
	*/
	if (!tk.hiddenMclassDt) tk.hiddenMclassDt = new Set()
	if (!(tk.hiddenMclassDt instanceof Set)) throw new Error('hiddenMclassDt not set')
	if (tk.snvIndelOnly) {
		tk.hiddenMclassDt.add(mclassfusionrna)
		tk.hiddenMclassDt.add(mclasssv)
		tk.hiddenMclassDt.add(dtcnv)
		tk.hiddenMclassDt.add(dtitd)
	}
	if (tk.hardcodeCnvOnly) {
		tk.hiddenMclassDt.add(mclassfusionrna)
		tk.hiddenMclassDt.add(mclasssv)
		tk.hiddenMclassDt.add(dtsnvindel)
		tk.hiddenMclassDt.add(dtitd)
	}
	...
	*/
	...
	tk.duplicateTk = filter => {
		...
		const tkarg = {
			...
			hiddenMclassDt: tk.hiddenMclassDt,
			hardcodeCnvOnly: tk.hardcodeCnvOnly, // still needed by legend.js
...
async function mayInitTermdb(tk, block) {
	...
	if (tk.mds.termdbConfig.queries) {
		tk.mds.has_skewer = false // convenient flag for downstream to decide if to set tk.skewer{}
		if (tk.mds.termdbConfig.queries.snvindel && !tk.hiddenMclassDt.has(dtsnvindel)) {
			tk.mds.has_skewer = true
		} else if (
			tk.mds.termdbConfig.queries.svfusion &&
			!tk.hiddenMclassDt.has(dtsv) &&
			!tk.hiddenMclassDt.has(dtfusionrna)
		) {
			tk.mds.has_skewer = true
		}
	} else {
		// queries missing for custom tk. in such case no check
	}
	...
	*/
	tk.mds.variant2samples.get = async arg => {
		const par = {
			...
			variant2samples: 1,
			get: arg.querytype,
			hardcodeCnvOnly: tk.hardcodeCnvOnly // unfortunately this is still needed by gdc, but not non-gc ds
		}
