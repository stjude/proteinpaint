...
export class IDCSearchView {
	...
	render(pagination: Pagination, gdcTotal?: number): void {
		...
		caseCountDiv.append('span').html(`IDC-matched Cases:&nbsp;<b>${pagination.total}</b>`)
		if (gdcTotal != null && gdcTotal !== pagination.total) {
			caseCountDiv
				.append('span')
				.style('font-size', '13px')
				.style('color', '#666')
				.html(`from&nbsp;<b>${gdcTotal}</b>&nbsp;GDC cohort cases`)
		}
