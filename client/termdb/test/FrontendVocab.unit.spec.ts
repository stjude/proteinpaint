...
tape('getDescrStats()', async test => {
	...
	testId = 'c'
	const msg = `Should throw error for non-numeric data`
	try {
...
tape('getterm()', async test => {
	...
	const testId = 'a'
	const result = await frontendVocabApi.getterm(testId)
	test.ok(result.id == testId, `Should return term object for test id = ${testId} `)

	const msg = `Should throw for missing term id`
	try {
		await frontendVocabApi.getterm()
		test.fail(msg)
	} catch (e) {
		test.pass(`${msg}: ${e}`)
	}
...
tape('graphable()', async test => {
	...
	const message = `Should throw for missing term`
	try {
	...
	const result = await frontendVocabApi.graphable(termjson['diaggrp'])
	test.equal(true, result, 'Should return true for graphable term')
})
