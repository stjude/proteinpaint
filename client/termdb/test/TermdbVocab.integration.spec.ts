...
tape.skip('getNestedChartSeriesData()', async test => {
	...
	const result = await termdbVocabApi.getNestedChartSeriesData(opts)
	console.log(result)
...
tape.skip('getTdbDataUrl()', async test => {
	...
	let opts, result, message, term_1, term_2
	...
	test.equal(paramsNotFound.length, 0, message)
	...
	term_1 = result.includes(`term1_id=${opts.term.id}`)
	term_2 = result.includes(`term2_id=${opts.term2.id}`)
	// const term_3 = result.includes(`term3_id=${opts.term2.id}`)
	test.ok(term_1 == true && term_2 == true, message)
...
tape('findTerm()', async test => {
	...
	testTerm = 's'
	result = await termdbVocabApi.findTerm(testTerm, 'ABC', null, TermTypeGroups.DICTIONARY_VARIABLES)
	test.equal(result.lst.length, 37, `Should return the correct number of terms`)
...
tape('getCategories()', async test => {
	...
	const testTerm = termjson['Arrhythmias']
...
tape.skip('getTermTypes()', async test => {
    ...
    */
	...
	const testIds = ['agedx']
	const result = await termdbVocabApi.getTermTypes(testIds)
	console.log(result)
