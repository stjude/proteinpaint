...
tape('validateOpts: accepts valid opts object', function (test) {
	...
	test.doesNotThrow(() => handler.validateOpts(opts), 'validateOpts accepts valid options')
	test.end()
})
...
tape('validateOpts: throws error for missing callback', function (test) {
	...
	test.throws(() => handler.validateOpts(opts), /callback is required/, 'throws error when callback is null')
	test.end()
})
...
tape('validateOpts: throws error for missing app', function (test) {
	...
	test.throws(() => handler.validateOpts(opts), /app is required/, 'throws error when app is null')
	test.end()
})
...
tape('validateOpts: throws error for missing holder', function (test) {
	...
	test.throws(() => handler.validateOpts(opts), /holder is required/, 'throws error when holder is null')
	test.end()
})
...
tape('validateOpts: throws error for missing usecase', function (test) {
	...
	test.throws(() => handler.validateOpts(opts), /usecase is required/, 'throws error when usecase is null')
	test.end()
})
...
tape('init: creates term divs for available terms', async function (test) {
	const handler = new SearchHandler()
	const holder = getHolder()
	const terms = [createMockTerm({ name: 'term1', plot: 'plot1' }), createMockTerm({ name: 'term2', plot: 'plot2' })]
	const opts = createValidOpts({
		holder,
...
tape('init: label includes plot name for multiple plots or no meta', async function (test) {
	const handler = new SearchHandler()
	const holder = getHolder()
	const terms = [createMockTerm({ name: 'term1', plot: 'plot1' })]
	const usecaseConfig = {
		sample: {
...
tape('init: callback invoked with correct term on click', async function (test) {
	...
	const terms = [createMockTerm({ name: 'term1', plot: 'plot1', sample: 'sample1' })]
	const opts = createValidOpts({
		holder,
		callback,
		app: createMockApp(terms)
...
tape('init: callback term includes sample from usecase if not in term', async function (test) {
	...
	const terms = [createMockTerm({ name: 'term1', plot: 'plot1', sample: undefined })]
	const usecaseConfig = {
