...
export class TermdbVocab extends Vocab {
    ...
    */
	async getNestedChartSeriesData(opts) {
		...
		const headers = this.mayGetAuthHeaders('termdb')
		const data = await this.dofetch3(route, { headers, body }, this.opts.fetchOpts)
		if (data.error) throw data.error
    ...
    */
	async getViolinPlotData(arg, _body = {}) {
		...
		const d = await this.dofetch3('termdb/violin', { headers, body })
	...
	async getBoxPlotData(arg, _body = {}) {
		...
		if (body.filter) body.filter = getNormalRoot(body.filter)
		const d = await this.dofetch3('termdb/boxplot', { headers, body })
	...
	async getPercentile(term_id, percentile_lst, filter) {
		...
		return await this.dofetch3('termdb/getpercentile', { body })
	}
	...
	*/
	async getCategories(term, filter, _body = {}) {
		...
		try {
			const data = await this.dofetch3('termdb/categories', { headers, body })
	...
	async getAnnotatedSampleData(opts, _refs = {}) {
        ...
        */
		...
		while (true) {
			...
			promises.push(
				this.dofetch3('termdb', init, { cacheAs: 'decoded' }).then(data => {
					if (data.error) throw data.error
