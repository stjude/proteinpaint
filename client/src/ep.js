...
const hl_strokeopacity=.6
const normal_strokeopacity=.2
...
*/
	...
	*/
	...
	if(arg.block) {
		...
		closebut.on('click',()=>{this.epaintfold(arg.block)})
	}
	...
	this.treediv=this.pane.body.append('div')
		.style('display','none')
		.style('margin','20px 0px 30px 0px')
		.style('padding','10px')
		.style('background-color','#FFFFE8')
	this.treediv.append('div')
		.style('font-size','.7em')
		.style('color','#858585')
	...
	this.grabbar=this.svgg.append('rect')
		.attr('x',0)
		.attr('y',0)
		.attr('fill','white')
		.attr('fill-opacity',0)
		.on('mousedown',()=>{
			...
			this.grab.shadebox
				...
				.attr('fill-opacity',.1)
			this.grab.shadehandle1.attr('fill-opacity',.5)
			this.grab.shadehandle2.attr('x',2).attr('fill-opacity',.5)
	...
	this.verticalline=this.svgg.append('line')
	...
	this.grab.shadebox=this.grab.shade.append('rect')
	...
	this.grab.shadehandle1=this.grab.shade.append('rect')
		.attr('x',-5)
		.attr('width',5)
		.attr('height',40)
		.attr('fill',this.p.hlcolor)
	...
	this.grab.shadehandle2=this.grab.shade.append('rect')
		.attr('x',0)
		.attr('width',5)
		.attr('height',40)
		.attr('fill',this.p.hlcolor)
	...
	// dots
	this.epdotg=this.graph.selectAll()
		.data(this.data)
		.enter().append('g')
	...
	this.render()
	const allsamplerow=this.treediv.append('div')
		.style('margin','10px')
		.style('padding','2px')
	...
	if(this.p.cohort) {
		...
		const div=this.treediv.append('div')
			.style('margin','10px')
			.style('height','400px')
...
epaintfold(block) {
	if(!this.handle) {
		...
		this.handle=block.ds2handle[this.dsname].handle.append('div')
...
makescale_ep() {
	...
	if(this.uselog) {
		...
		this.axisg.append('text')
			...
			.each(function(){fontwidth=this.getBBox().width})
			.remove()
...
render(width,height) {
	...
	this.width=width
	this.width2=width*.05
	this.dotsize=Math.max(13,width/30)
	this.height=height
	this.sf_boxheight.range([10,this.height/8])
	const groupmax=Math.min(this.data.length*.7,300)
	this.sf_boxlabelfontsize.domain([0,Math.log(groupmax),Math.log(this.data.length)])
	...
	this.axish=Math.max(18,this.width*.04)
	this.tickh=this.axish*.3
	this.makescale_ep()
	this.grabbar.attr('width',this.width)
	this.grabbar.attr('height',this.axish)
	// only to get width2_
	const tmp=this.svgg.append('text')
	...
	tmp.remove()
	const tmp2=this.svgg.append('text')
		.text('COUNT')
		.attr('font-size',1)
		.attr('font-family',client.font)
	...
	if(this.p.cohort) {
		if(this.p.cohort.annotation && this.p.cohort.key4annotation) {
			...
			for(const l of this.p.cohort.levels) {
				...
				lst.push({
					k:(l.label || l.k),
					v:d[l.k]+(l.full ? (d[l.full] ? ' <span style="font-size:.8em;color:#858585">'+d[l.full]+'</span>' : '') : '')
				})
			}
			return false
		}
...
dot_appendtext(set,lookup) {
	...
	.each(function(d){ d.bb=this.getBBox() })
	.each(d=>{
...
may_hl(mlst,hl) {
	...
	if(nohit) return
	this.epdot.filter(d=> samples.has(d[this.p.sampletype]))
		//.attr('fill-opacity', hl ? .7 : 0)
		.attr('r', hl ? this.dotsize*.8 : this.dotsize/2)
	...
	for(const m of mlst) {
		...
		if(typeof(v)=='string') {
			...
			lookup.set(sn,
				v.v2==0 ? 'No '+this.p.maf.label+': site not covered' :
...
function showgene(ep,name) {
	...
	if(ds.dbexpression) {
		...
		fetch( new Request(ep.hostURL+'/dbdata', {
			method:'POST',
	...
	fetch( new Request(ep.hostURL+'/dsdata', {
		method:'POST',
...
function makeButtons( ep ) {
	...
	butrow.append('button').text('Cohort').on('click',()=>{
	...
	butrow.append('button').text('Log10').on('click',()=>{
		const dur=700
		ep.uselog=!ep.uselog
		ep.makescale_ep()
		ep.epdotg.transition().duration(dur)
			.attr('transform',(d,i)=> 'translate('+((ep.uselog && d.value==0) ? 0 : Math.max(0,ep.x_scale(d.value)))+','+d._y+')')
		for(const bp of ep.boxplots) {
			bp.vlines.transition().duration(dur)
				.attr('x1',v=> Math.max(0,ep.x_scale(v)))
				.attr('x2',v=> Math.max(0,ep.x_scale(v)))
			bp.box.transition().duration(dur)
		...
		if(ep.grab.x) {
			...
			ep.grab.x=x1
			ep.grab.width=x2-x1
			ep.grab.shade.transition().duration(dur).attr('transform','translate('+x1+','+ep.grab.y+')')
	...
	butrow.append('button').text('SVG').on('click',()=>{
		client.to_svg(ep.svg.node(),ep.genename+'_expression')
	})

	butrow.append('button').text('Data').on('click',()=>{
		...
		client.export_data(ep.genename+' '+ep.p.name+' data',[{text:header.join('\t')+'\n'+rows.join('\n')}])
	})
	...
	butrow.append('input')
		...
		.on('keyup',()=>{
			...
			ep.epdot.filter(i=>set.has(i[ep.p.sampletype]))
				.attr('r', ep.dotsize)
