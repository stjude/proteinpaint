...
const dotstrokeopacity=.9 // circle or rect
...
const mouseover=(s,x,y)=>{
	...
	const lst=[
		{k:ik,v:s[ik]},
		{k:cfg.xsampletype,v:s.sampletypes[cfg.xsampletype].vaf.toFixed(2)+', '+s.sampletypes[cfg.xsampletype].total},
	]
	if(cfg.individualcategories) {
	...
	if(s.sampletypes[cfg.yleftsampletype]) {
		lst.push({k:cfg.yleftsampletype,v:s.sampletypes[cfg.yleftsampletype].vaf.toFixed(2)+', '+s.sampletypes[cfg.yleftsampletype].total})
	}
	if(s.sampletypes[cfg.yrightsampletype]) {
		lst.push({k:cfg.yrightsampletype,v:s.sampletypes[cfg.yrightsampletype].vaf.toFixed(2)+', '+s.sampletypes[cfg.yrightsampletype].total})
	}
...
if(maxdp_yleft) {
	...
	yleftlab=yleftlabg.append('text')
...
if(maxdp_yright) {
	...
	yrightlab=yrightlabg.append('text')
		.attr('transform','rotate(-90)')
...
const xlab = axe_xvaf.append('text')
...
if(maxdp_yleft) {
	...
	rectdp_yleft = axedp_yleft.append('rect') // background box
		.attr('fill',rectfill)
	midlinedp_yleft = axedp_yleft.append('line') // .5 mid line
		.attr('stroke','white')
		.attr('stroke-width',2)
	axisdp_yleft  = axedp_yleft.append('g')
	dotg_yleft = axedp_yleft.append('g').selectAll()
		.data(data.filter(d=>d.sampletypes[cfg.yleftsampletype]))
		.enter().append('g')
	circle_yleft = dotg_yleft.append('circle')
		.attr('fill', d=>d.color || ycolor)
		.attr('fill-opacity',0)
		.attr('stroke', d=>d.color || ycolor)
		.attr('stroke-opacity',dotstrokeopacity)
		.on('mouseover',d=>mouseover(d,d3event.clientX,d3event.clientY))
...
if(maxdp_yright) {
	...
	rectdp_yright = axedp_yright.append('rect') // background box
		.attr('fill',rectfill)
	midlinedp_yright = axedp_yleft.append('line') // .5 mid line
		.attr('stroke','white')
		.attr('stroke-width',2)
	axisdp_yright  = axedp_yright.append('g')
	dotg_yright = axedp_yright.append('g').selectAll()
		.data(data.filter(d=>d.sampletypes[cfg.yrightsampletype]))
		.enter().append('g')
	circle_yright = dotg_yright.append('rect')
		.attr('fill', d=>d.color || ycolor)
		.attr('fill-opacity',0)
		.attr('stroke', d=>d.color || ycolor)
		.attr('stroke-opacity',dotstrokeopacity)
...
const rect_xdp  = axe_xdp.append('rect') // background box
	.attr('fill',rectfill)
const midlinedp_x = axe_xdp.append('line') // .5 mid line
	.attr('stroke','white')
	.attr('stroke-width',2)
const dplab = axe_xdp.append('text')
	.text('Coverage')
...
const dotg_xdp = axe_xdp.append('g').selectAll()
	.data(data)
	.enter().append('g')
const circle_xdp = dotg_xdp.append('circle')
...
const rect_center  = plotg_center.append('rect') // background box
	.attr('fill',rectfill)
const midline_xcenter=plotg_center.append('line') // .5 horizontal mid line
	.attr('stroke','white')
	.attr('stroke-width',2)
const midline_ycenter=plotg_center.append('line') // .5 vertical mid line
	.attr('stroke','white')
	.attr('stroke-width',2)
const dotg_center = plotg_center.append('g').selectAll()
	.data(data)
	.enter().append('g')
const circle_center = dotg_center.append('circle')
...
const render = ()=>{
	...
	if(axevaf_yright) {
		axevaf_yright.attr('transform','translate('+( (maxdp_yleft ? yvafwidth+sp+ydpheight : 0) + sp2+xwidth+sp2+ydpheight+sp)+','+(toppad+yheight)+')')
		yrightlabg.attr('transform','translate('+(yvafwidth-(fontsize))+',-'+(yheight/2)+')')
		yrightlab.attr('font-size',fontsize*2)
	}

	axe_xvaf.attr('transform','translate('+(yvafwidth+sp+ydpheight+sp2)+','+(toppad+yheight+sp2+xdpheight+sp)+')')
	xlab
	...
	if(axedp_yright) {
		axedp_yright.attr('transform','translate('+( (maxdp_yleft ? yvafwidth+sp+ydpheight : 0) +sp2+xwidth+sp2)+','+(toppad+yheight)+')')
	...
	rect_xdp.attr('width',xwidth)
		.attr('height',xdpheight)
	midlinedp_x
		.attr('x1',xwidth/2)
		.attr('x2',xwidth/2)
		.attr('y2',xdpheight)
	...
	if(axisvaf_yleft) {
		client.axisstyle({
			axis:axisvaf_yleft.call(axisLeft().scale(scalevaf_y).tickSize(ticksize-2)),
			showline:true,
			color:gray,
			fontsize:fontsize,
		})
	...
	if(axisvaf_yright) {
		client.axisstyle({
			axis:axisvaf_yright.call(axisRight().scale(scalevaf_y).tickSize(ticksize-2)),
			showline:true,
			color:gray,
			fontsize:fontsize,
		})
	...
	client.axisstyle({
		axis:axis_xvaf.call(axisBottom().scale(scalevaf_x).tickSize(ticksize-2)),
		showline:true,
		color:gray,
		fontsize:fontsize,
	})
	client.axisstyle({
		axis:axis_xdp.call(axisLeft().scale(scaledp_x).tickSize(ticksize-2).tickValues([0,maxdp_x])),
		showline:true,
		color:gray,
		fontsize:fontsize,
	})
	if(axisdp_yleft) {
		client.axisstyle({
			axis:axisdp_yleft.call(axisBottom().scale(scaledp_yleft).tickSize(ticksize-2).tickValues([0,maxdp_yleft])),
			showline:true,
			color:gray,
			fontsize:fontsize,
		})
	...
	if(axisdp_yright) {
		client.axisstyle({
			axis:axisdp_yright.call(axisBottom().scale(scaledp_yright).tickSize(ticksize-2).tickValues([0,maxdp_yright])),
			showline:true,
			color:gray,
			fontsize:fontsize,
		})
