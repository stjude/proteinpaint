...
*/
	...
	svg
		.attr('width', maxlabelw + xpad + width + xpad)
		.attr('height', axisheight + (ypad + boxheight) * arg.list.length + 10)

	const g = svg.append('g').attr('transform', 'translate(' + (maxlabelw + xpad) + ',' + axisheight + ')')
	...
	for (const grp of arg.list) {
		y += ypad
		const sg = g.append('g').attr('transform', 'translate(0,' + y + ')')
		y += boxheight
		...
		if (grp.samples) {
			...
			vlst.sort((a, b) => a - b)
			p05 = vlst[Math.floor(vlst.length * 0.05)]
			p25 = vlst[Math.floor(vlst.length * 0.25)]
			p50 = vlst[Math.floor(vlst.length * 0.5)]
			p75 = vlst[Math.floor(vlst.length * 0.75)]
			p95 = vlst[Math.floor(vlst.length * 0.95)]
		} else {
