...
export function loadstudycohort(genomes,file,holder, hostURL, jwt, noshow, debugmode) {
	/*
	tp entry point
	...
	*/
	...
	d3json(hostURL+'/study').post(JSON.stringify({file:file,jwt:jwt}), data=>{
		...
		if(!noshow) {
			import('./tp.ui').then(p=>{
				p.default(cohort,holder,hostURL,debugmode)
			})
		}
	})
}
...
function parse_browserview(cohort) {
	...
	if(cohort.browserview.defaultassaytracks) {
		...
		for(let i=0; i<cohort.browserview.defaultassaytracks.length; i++) {
			...
			if(!cohort[t.assay][t.level1]) return 'level1 not exist in assay from .defaultassaytracks #'+(i+1)+': '+t.level1
			if(t.level2) {
				if(!cohort[t.assay][t.level1][t.level2]) return 'level2 not exist in assay from .defaultassaytracks #'+(i+1)+': '+t.level2
			}
		}
	}
	return null
