	...
	*/
		...
		for(const n in data) {
			for(const m of data[n]) {
				...
				if(c==common.mclassfusionrna) {
					fusionstr.add((m.sample?m.sample:'')+'.'+(m.sampletype?m.sampletype:'')+JSON.stringify(m.pairlst))
					continue
				}
	...
	for(const [dskey, class2count] of cbreakdown) {
		...
		for(const [k, count] of class2count) {
			...
			if(mc.dt==common.dtsnvindel || mc.dt==common.dtitd || mc.dt==common.dtdel || mc.dt==common.dtnloss||mc.dt==common.dtcloss) {
				lst.push({
					key:k,
					label:mc.label,
					color:mc.color,
