...
function setInteractivity(self) {
	self.editConfig = async (input, variable) => {
		if (!variable) {
			...
			if (input.varClass == 'term') {
				if (!('id' in variable)) variable.id = variable.term.id // when switching between continuous/discrete for independent, variable.id missing (termsetting issue?)
				input.term = variable
			} else {
	...
	self.submit = () => {
		...
		// to ensure new variable of every section goes into config
		// as refGrp for newly selected variable can be missing from config
		self.section2config()
