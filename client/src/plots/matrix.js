...
class Matrix {
	...
	getDataRequestOpts() {
		...
		// create a copy of terms with only the required attributes for the request
		const terms = []
		const usedIds = new Set()
		for (const tw of [...this.rowTerms, this.config.divideBy]) {
			if (!tw) continue
			const copy = {
				term: { id: tw.term.id },
				q: tw.q
			}
			// create a fake ID using the autogenerated termsetting ID,
			// to disambiguate between terms that have the same ID,
			// but belonging to different groups and/or having a different q{}
			if (usedIds.has(tw.id)) copy.$id = tw.$id
			else usedIds.add(tw.id)
	...
	setSampleGroupsOrder(data) {
		...
		if (!this.config.divideBy) {
			...
			const t$id = term.id + ';;' + this.config.divideBy.$id
			const sgtid = term.id
			const ref = t$id in data.refs.byTermId ? data.refs.byTermId[t$id] : data.refs.byTermId[sgtid] || {}

			for (const row of data.lst) {
				const anno = t$id in row ? row[t$id] : row[sgtid]
				if (sgtid in row) {
					const key = anno.key
	...
	setTermOrder(data) {
		...
		for (const [grpIndex, grp] of this.config.termgroups.entries()) {
			for (const [tIndex, tw] of grp.lst.entries()) {
				const t$id = tw.term.id + ';;' + tw.$id
				const ref = data.refs.byTermId[t$id] || data.refs.byTermId[tw.id] || {}
	...
	getSerieses(data) {
		...
		for (const { index, grpIndex, row } of this.sampleOrder) {
			...
			for (const t of this.termOrder) {
				...
				const t$id = termid + ';;' + t.tw.$id
				const altId = t$id in row ? t$id : t.tw.term.type == 'geneVariant' ? t.tw.term.name : ''
