...
export function newboxplot(ep, data, label, id, handle) {
	...
	const p9 = data[Math.floor((data.length - 1) * 0.91)].value,
		p25 = data[Math.floor((data.length - 1) * 0.75)].value,
		p50 = data[Math.floor((data.length - 1) * 0.5)].value,
		p75 = data[Math.floor((data.length - 1) * 0.25)].value,
		p91 = data[Math.floor((data.length - 1) * 0.09)].value
	box.percentile = {
		9: p9 == 0 ? 0.0001 : p9,
		25: p25 == 0 ? 0.0001 : p25,
		50: p50 == 0 ? 0.0001 : p50,
	...
	box.label = holder
		...
		.text(label)
		.on('mousedown', event => boxplot_md(ep, box, event))
		.on('click', () => boxplot_clicklabel(ep, box))
		.each(function() {
			box.labelwidth = this.getBBox().width
		})
	...
	box.box = holder
		...
		.on('mouseover', event => {
			ep.dottip.show(event.clientX, event.clientY).clear()
			const lst = [
	...
	box.vlines = holder
		.selectAll()
		.data([box.percentile[9], box.percentile[25], box.percentile[50], box.percentile[75], box.percentile[91]])
		.enter()
		.append('line')
		.attr('x1', 0)
		.attr('x2', 0)
...
function boxplot_clicklabel(ep, box) {
	...
	ep.epdot
		...
		.duration(1000)
		.attr('r', d => {
			if (d[ep.p.sampletype] in box.lookuphash) return box.highlight ? ep.dotsize * 0.7 : ep.dotsize / 2
			return box.highlight ? ep.dotsize * 0.2 : ep.dotsize / 2
		})
		.attr('stroke', d => {
			if (d[ep.p.sampletype] in box.lookuphash) return box.highlight ? ep.p.hlcolor : 'black'
			return 'black'
		})
		.attr('stroke-opacity', d => {
			if (d[ep.p.sampletype] in box.lookuphash) return box.highlight ? 0.4 : 0.2
			return box.highlight ? 0.1 : 0.2
		})
	for (const b of ep.boxplots) {
...
function boxplot_md(ep, box, event) {
	...
	b.on('mousemove', event => {
		ep.busy = true
		box.yoff = oldboxy + event.clientY - y
