...
import {interpolatePlasma,scaleLinear} from 'd3-scale'
...
export function e2pca_inputui( hostURL, jwt ) {
	...
	const ta_pca=butrow.append('input').attr('size',30).attr('placeholder','PCA/SNE matrix file path').style('margin','10px')
		.property('value','xiang/sep20/set1')
	const ta_db=butrow.append('input').attr('size',30).attr('placeholder','numerical db file path').style('margin','10px')
	...
	butrow.append('span').style('padding-left','10px').html('<a href=https://docs.google.com/document/d/1Midt0rYs1iIJveUMjeng9si3q31YelcNe_PI3Njce9E/edit?usp=sharing target=_blank>Help</a>')
}
...
const svgholder=arg.holder.append('div')
...
const circles=dots.append('circle')
	...
	.on('mouseover',d=>{
		...
		menu.clear()
		menu.show(d3event.clientX, d3event.clientY)
		const lst=[ 
			{k:'name',v:d.sample}
		]
...
const underdiv=svgholder.append('div')
...
// legend
const legendholder=underdiv.append('div')
...
export function e2pca_genesearchui(arg) {
	...
	*/
	...
	const geneta=arg.holder.append('input')
	...
	geneta.on('keyup',()=>{
		...
		d3json(arg.hostURL+'/dbdata').post(JSON.stringify({ db:arg.obj.dbfile,tablename:'data',keyname:'gene',key:gene.toLowerCase(), jwt:arg.jwt }),data=>{
