...
if(cohort.genenetwork && cohort.genenetwork.list) {
	...
	for(const network of cohort.genenetwork.list) {
		const holder=folder.append('div').style('display','inline-block').style('margin','10px').style('border','solid 1px #ccc')
		holder.append('div').style('background-color','#f1f1f1').style('padding','5px 10px').text(network.name || 'network.name is undefined')
...
for(const ds of dslst) {
	...
	row1.append('button').text('Submit').style('margin-right','3px').on('click',()=>{
		prepdata_input(ds,input,td2,config)
	})
	row1.append('button').text('Clear').style('margin-right','10px').on('click',()=>{
		input.node().value=''
	})
	row1.append('a').text('Help')
...
function makegraph(ds,json,holder,config) {
	...
	if(config.diseasecolor) {
		...
		*///////////////////
		...
		diseasecolor=scaleOrdinal().domain(["AML","NBL","TALL","BALL","WT","OS"]).range(["#dc5ce2","#ff1111","#ffa500","#1f78b4","#3e8320","#6a3d9a"])
	}
	// getting data, mlst
	for(const node of json.nodes) {
	...
	holder.append('div').append('button').text('Snapshot').on('click',()=>{
	...
	.attr("orient", "auto")
	.append('rect')
	.attr('y',-5).attr('width',2).attr('height',10).attr('fill','red')

	const legend=svg.append('g')

	// link lines
	const linkg=svg.selectAll()
		.data(json.links)
		.enter().append('line')
	linkg.attr('stroke',d=>d.inhibit ? 'red' : 'black')
		.attr('stroke-width','2')
		.attr('marker-end',d=>d.inhibit ? 'url(#arrowinhibit)' : 'url(#arrowactivate)')

	// force nodes
	const node1g=svg.selectAll()
		.data(json.nodes)
		.enter().append('g')
		.each(function(d){
			d.nodeg=this
	...
	for(const node of json.nodes) {
		...
		node.pack.gs=d3select(node.nodeg)
			...
			.enter().append('g')
		node.pack.circle=node.pack.gs.append('circle')
			.attr('fill',d=>{
	...
	for(const node of json.nodes) {
		...
		node.pack.gs.filter((d)=>!d.children)
		.each((d)=>{
			...
			.on('mouseover',()=>{
				const menu=client.menushow(d3event.clientX+20,d3event.clientY+20)
				const div=menu.append('div')
				...
				if(d.pie) {
					...
					for(const pie of d.pie) {
						p.append('div').style('margin','3px 0px').html('<span style="background-color:'+pie.data.color+'">&nbsp;&nbsp;&nbsp;</span> '+pie.data.name+' '+pie.data.size)
					}
				}
			})
			.on('mouseout',()=>{
	...
	const simulation=d3force.forceSimulation(json.nodes)
	...
	function render(restart) {
		...
		for(const node of json.nodes) {
			node.pack.gs.filter((d)=>!d.children)
			.each((d)=>{
				d.textlabel.attr('font-size',fontsize)
				.each(function(){
		...
		simulation.force('center', d3force.forceCenter(width/2,height/2))
			.force('collision',d3force.forceCollide()
				.radius(d=>{
					return d.root.r+Math.max(20,d.root.r*.3)
				})
				.strength(.9)
			)
	...
	function ticked() {
		linkg.each(d=>d.linelen=Math.sqrt(Math.pow(d.source.x-d.target.x,2)+Math.pow(d.source.y-d.target.y,2)))
		.attr('x1',d=>{
	...
	/*
		...
		*/
		if (!d3event.active) simulation.alphaTarget(.1).restart()
		d.fx = d.x
		d.fy = d.y
	}
	function dragged(d) {
