...
import {duplicate as svduplicate} from '../../shared/src/bulk.sv'
...
butrow.append('button').text('Show all').style('display','none').style('font-size','80%').on('click',()=>{
...
butrow.append('a').attr('href','https://docs.google.com/document/d/1buFKS9CN6HNjdFKNh1sJwULeJESQ7ftZntF52dpjFrY/edit?usp=sharing').attr('target','_blank').style('padding-left','10px').text('Help')
const scrolltable=folder.append('div')
...
for(const dsname in ds2dtlst) {
	tr1.append('td').attr('colspan',ds2dtlst[dsname].length)
...
for(const patientname of pnlst) {
	...
	if(stlst.length==0) {
		const tr=scrolltable.append('tr')
			.classed('sja_tr',true)
		// td1
		tr.append('td')
			.style('font-size','.7em')
	...
	for(let i=0; i<stlst.length; i++) {
		...
		sample.handle=tr.append('td')
			.append('div').classed('sja_opaque8',true)
		...
		for(const as of cohort.assaylst) {
			...
			tr.append('td').style('text-align','center').html(found ? '&#10003;' : '')
		}
		for(const dsname in ds2dtlst) {
			let td
			for(const dt of ds2dtlst[dsname]) {
				...
				if(count>0) {
					...
					.append('span').classed('sja_mcdot',true)
...
pane.header.text((sample.sample ? sample.sample+' ' : '')+(sample.patientname ? sample.patientname+' ':'')+(sample.sampletype||''))
...
for(const chr in genome.majorchr) {
	const chrholder=pane.body.append('div')
