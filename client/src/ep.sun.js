...
const sun=grab.sun
sun.g=grab.holder.append('g').attr('transform','translate('+sun.x+','+sun.y+')')
	.on('mouseover',()=>{
		ep.dottip.hide()
	})
...
const radius=Math.min(ep.width,ep.height)*.3
...
sun.ring=sun.g
	...
	.on('mouseout',d=>{
		setnamesays.text(grab.min+' - '+grab.max)
			.attr('font-size',setnamefontsize)
		setnamesaysbg.text(grab.min+' - '+grab.max)
			.attr('font-size',setnamefontsize)
	...
	.on('click',(d)=>{
		...
		ep.samplecart.setBtns({
			samplelst:lst,
			basket:'Gene Expression',
			note:d.data.name+' samples with '+ep.genename+' '+ep.p.datatype+' between '+ep.grab.min+' and '+ep.grab.max,
			id:ep.genename+(!ep.grab || !d.data.lst.length || !d.data.lst[0]? '' 
				: ' FPKM:'+ep.grab.min+'-'+ep.grab.max)+' '+d.data.lst[0].ancestor_dx,
			container: ep.samplecartWrapper,
			reselectable: true
		})
	})

bgpocket.append('circle')
	.attr('r',bgpocketrad)
	.attr('fill','white')
	.attr('fill-opacity',.7)
...
const fontsize=Math.max(13,ep.dotsize*.7)
sun.g.selectAll()
	.data(palst)
	.enter().append('text')
	.text(d=>(d.parent.data.name?(d.parent.data.name=='root'?'':d.parent.data.name+', '):'')+d.data.name)
...
sun.g.selectAll()
	...
	.attr('stroke','#858585')
// center labels
const bigfont=Math.max(18,emptyspace*.6)
const smallfont=Math.max(14,emptyspace*.2) // small font height for title
let y=-3
let setnamefontsize
let w
const sampletypesays=sun.g.append('text')
	...
	.attr('fill',ep.p.hlcolor)
	.attr('text-anchor','middle')
const numsays=sun.g.append('text')
	...
	.attr('fill-opacity',.8)
y=5
const datatypesays=sun.g.append('text')
	.text(ep.p.datatype)
	.attr('font-size',smallfont)
	.attr('font-family',fontfamily)
	.attr('y',y)
...
const sf=scaleLinear().domain([5,15]).range([emptyspace,emptyspace*2])
const setnamesaysbg=sun.g.append('text')
	.text(text)
	.attr('font-size',1)
	.each(function(){
		var b=this.getBBox()
		setnamefontsize=Math.min(sf(text.length)/b.width,emptyspace*.4/b.height)
		})
	.attr('font-size',setnamefontsize)
	.attr('font-family',fontfamily)
	.attr('y',y+smallfont)
	...
	.attr('stroke-width',3)
	.attr('fill','none')
const setnamesays=sun.g.append('text')
	.text(text)
	.attr('font-size',setnamefontsize)
	.attr('font-family',fontfamily)
	.attr('y',y+smallfont)
...
function removesun(sun, grab, ep) {
	...
	grab.shadebox.transition().duration(500)
		.attr('stroke-opacity',0)
		.attr('fill-opacity',0)
	grab.shadehandle1.transition().duration(500)
		.attr('fill-opacity',0)
	grab.shadehandle2.transition().duration(500)
		.attr('fill-opacity',0)
	delete grab.x
	if(ep.selectsample_button) {
		ep.selectsample_button
			.style('display','none')
			.text('')
	}
}
