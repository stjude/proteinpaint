...
tape('Render PNET scatter plot', function(test) {
	...
	async function runTests(scatter) {
		...
		async function testCreateGroupPlots() {
			const samples = serieG
				.selectAll('path')
				.nodes()
				.filter(p => p.__data__?.category === 'ETMR')
				.map(path => path.__data__)
			test.true(28 == samples.length, `Group should have 28 symbols.`)
