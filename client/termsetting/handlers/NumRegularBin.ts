...
export function renderBoundaryInclusionInput(handler) {
	const self = handler.termsetting
	handler.dom.boundaryInclusionDiv = handler.dom.bins_div.append('div').style('margin-left', '5px')

	handler.dom.boundaryInclusionDiv
		.append('span')
		.style('padding', '5px')
	...
	handler.dom.boundaryInput = handler.dom.boundaryInclusionDiv
		.append('select')
		...
		.on('change', function () {
			...
			function setBinsInclusion(par) {
				par.startinclusive = handler.dom.boundaryInput.node().selectedIndex == 1
				par.stopinclusive = handler.dom.boundaryInput.node().selectedIndex == 0
			}
		})

	type htmlData = { value: string; html: string }

	handler.dom.boundaryInput
		.selectAll('option')
		.data([
...
function renderTypeInputs(handler) {
	const self = handler.termsetting
	const div = handler.dom.bins_div.append('div').style('margin', '10px')
	...
	const tabs: any = [
		{
			...
			callback: async (event, tab) => {
				...
				if (!tabs[0].isInitialized) {
					renderRegularSizedBinsInputs(handler, tab.contentHolder)
		...
		},
		{
			...
			callback: async (event, tab) => {
				...
				if (!tabs[1].isInitialized) {
					renderCustomBinInputs(handler, tab.contentHolder)
...
function mayShowValueconversionMsg(handler, tablediv: any) {
	const self = handler.termsetting
...
function renderRegularSizedBinsInputs(handler, tablediv: any) {
	mayShowValueconversionMsg(handler, tablediv)
	handler.dom.bins_table = tablediv.append('table')
	renderBinSizeInput(handler, handler.dom.bins_table.append('tr'))
	renderFirstBinInput(handler, handler.dom.bins_table.append('tr'))
	renderLastBinInputs(handler, handler.dom.bins_table.append('tr'))
}

function renderBinSizeInput(handler, tr: any) {
	const self = handler.termsetting
...
function renderFirstBinInput(handler, tr: any) {
	const self = handler.termsetting
...
function renderLastBinInputs(handler, tr: any) {
	const self = handler.termsetting
