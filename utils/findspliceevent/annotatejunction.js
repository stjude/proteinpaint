...

	{
		...
		if (gmlst) {
			...
			if (j.exonleft.length)
				j2.exonleft = j.exonleft.map(i => {
					return { gene: i.gm.name, isoform: i.gm.isoform, strand: i.gm.strand, exonidx: i.exonidx }
				})
			if (j.exonleftin.length)
				j2.exonleftin = j.exonleftin.map(i => {
					return { gene: i.gm.name, isoform: i.gm.isoform, strand: i.gm.strand, exonidx: i.exonidx }
				})
			if (j.intronleft.length)
				j2.intronleft = j.intronleft.map(i => {
					return { gene: i.gm.name, isoform: i.gm.isoform, strand: i.gm.strand, intronidx: i.intronidx }
				})
		}
	}

	{
		...
		if (gmlst) {
			...
			if (j.exonright.length)
				j2.exonright = j.exonright.map(i => {
					return { gene: i.gm.name, isoform: i.gm.isoform, strand: i.gm.strand, exonidx: i.exonidx }
				})
			if (j.exonrightin.length)
				j2.exonrightin = j.exonrightin.map(i => {
					return { gene: i.gm.name, isoform: i.gm.isoform, strand: i.gm.strand, exonidx: i.exonidx }
				})
			if (j.intronright.length)
				j2.intronright = j.intronright.map(i => {
					return { gene: i.gm.name, isoform: i.gm.isoform, strand: i.gm.strand, intronidx: i.intronidx }
				})
		}
	}
