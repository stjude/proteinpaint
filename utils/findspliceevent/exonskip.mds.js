/*
finds both exon skipping and a5ss/a3ss
...
if(err) {
	console.log('Error: '+err+`
           ...
           splice event finder (mds-version)
...
--mdsjunction=     .gz of MDS-junction file, tabix indexed
...
function checkarg() {
	...
	if(!arg.mdsjunction) return 'missing junction file'
	if(!arg.mdsjunction.endsWith('.gz')) return 'junction file should be compressed by bgzip'
	if(!fs.existsSync(arg.mdsjunction+'.tbi')) return '.tbi index missing for junction file'
...
const query_junctions = (region)=>{

	return new Promise( (resolve, reject) => {

	const sp = spawn( 'tabix', [arg.mdsjunction, region.chr+':'+region.start+'-'+region.stop] )
	...
	for(const [gene, models] of geneset) {
		...
		*/
		...
		for(const region of regions) {
			...
			const sample2junctions = await query_junctions( region )
