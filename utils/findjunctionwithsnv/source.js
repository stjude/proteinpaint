...
const vcfparse = require('../../server/shared/vcf')
...
const mapjunctiontoexons = require('../../client/src/spliceevent.prep').mapjunctiontoexons

const lines = fs
	.readFileSync(junctiontextfile, 'utf8')
	.trim()
	.split('\n')
for (const line of lines) {
	...
	*/
	...
	if (
		j.leftsnv ||
		j.rightsnv ||
		(j.leftcompete && j.leftcompete.find(i => i.snvatcompetesite)) ||
		(j.rightcompete && j.rightcompete.find(i => i.snvatcompetesite))
	) {
		outputjunction(j)
	}
}
...
function lefttest(j) {
	...
	return [leftsnv.length ? leftsnv : undefined, competesites.length ? competesites : undefined]
}
...
function righttest(j) {
	...
	return [rightsnv.length ? rightsnv : undefined, competesites.length ? competesites : undefined]
}
...
function lefttest2(j) {
	...
	return [leftsnv, competesites.length ? competesites : undefined]
}
...
function righttest2(j) {
	...
	return [rightsnv, competesites.length ? competesites : undefined]
}
...
function site2exonintron(pos, genes) {
	...
	*/
	...
	return [exons.length ? exons : undefined, introns.length ? introns : undefined]
}
	...
	if (arg.dnavcf) {
		for (const thisvcf of arg.dnavcf) {
			...
			*/
			...
			const cmd =
				'tabix ' + thisvcf.file + ' ' + (thisvcf.nochr ? chr.replace('chr', '') : chr) + ':' + start + '-' + stop
...
function findcompetejunction(chr, competepos, otherend) {
	/*
	 */
	const tmp = exec('tabix ' + arg.junction + ' ' + chr + ':' + competepos + '-' + (competepos + 1), {
		encoding: 'utf8'
	}).trim()
	if (tmp) {
...
function rnaAllelecount(snv) {
	...
	const tmp = exec(
		'samtools mpileup -uv -t DP -t AD -r ' +
	...
	// parse vcf
	// XXX m is mlst
	const [vcferror, m, altinvalid] = vcfparse.vcfparseline(dataline, { samples: ['sample'] })
