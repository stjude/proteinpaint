...
docker build  --file ./build/Dockerfile $BUILDARGS --target ppbase --tag ppbase:$REV --build-arg platform="$ARCH" .
echo "building pprust:$REV image, package version=$TAG"
docker build  --file ./build/Dockerfile $BUILDARGS --target pprust --tag pprust:$REV --build-arg platform="$ARCH" .
echo "generating a build with minimal package jsons"
docker build  --file ./build/Dockerfile $BUILDARGS --target ppminpkg --tag ppminpkg:$REV --build-arg platform="$ARCH" .
