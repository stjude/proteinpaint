...
async function kmplot_menu ( obj ) {
	const mutation_features = obj.features.filter( i=> i.ismutation )

	try {
		if( mutation_features.length <= 1 ) throw 'requires 2 mutation features'
		if( mutation_features.length == 2 ) {
			await kmplot_do( obj, mutation_features[0], mutation_features[1] )
			return
		}
		// TODO list mutation features, let select two to do plot
	} catch(e) {
...
async function kmplot_do ( obj, f1, f2 ) {
/*
...
*/
	...
	if( obj.limitsamplebyeitherannotation ) {
		plot.samplerule.full = {
			byattr:1,
			key: obj.limitsamplebyeitherannotation[0].key,
			value: obj.limitsamplebyeitherannotation[0].value,
			immutable:1,
		}
	}
	...
	pane.header.text(
		( obj.limitsamplebyeitherannotation ? obj.limitsamplebyeitherannotation[0].value+' ' : '' )
		+ 'survival by '+f1.label+' and '+f2.label+' mutation status'
	)
