...
constructor(arg) {
	...
	*/
	...
	if(arg.width) {
		...
		this.width=ntpxwidth*Math.ceil(Math.max(w*.63,minwidth)/ntpxwidth)
	}
	...
	this.holder=arg.holder.append('div')
	...
	{
		const inlinediv=this.holder.append('div') // contains svg and resize button
			.style('position','relative')
			.style('display','inline-block')
		this.svg=inlinediv.append('svg')
			.on('mousedown',()=>{
				d3event.preventDefault()
			})
		if(!arg.noresize) {
			this.resizewidthbutton=inlinediv.append('div')
			.text('drag to resize')
	...
	if(arg.nobox) {
		...
		// use dogtag
		butrow.append('div').html( arg.dogtag ? arg.dogtag : arg.chr)
			.style('display','inline-block')
			.style('padding','5px')
	...
	if(this.usegm) {
		...
		// name button
		this.ctrl.namebutt=butrow.append('span')
			.attr('class','sja_clbtext')
		...
		.on('click',()=>{
			...
			import('./block.ds.gmcustomdata').then(q=>{
				q.default(this,p.left-100,p.top+p.height+3)
			})
		})
		butrow.append('span').html('&nbsp;&nbsp;&nbsp;')
	}
	...
	if(this.usegm) {
		...
		this.coordwidthsays=butrow.append('span')
	...
	*/
	...
	this.ctrl.zinbutt=butrow.append('button')
		.html('In')
	...
	{
		const button=butrow.append('button')
			.text('Tracks')
	...
	this.coord.g0 = this.gbase.append('g')
	...
	this.coord.name=this.coord.g.append('text')
	...
	if(this.hidegenecontrol) {
		this.coord.name.text('Zoom out').classed('sja_clbtext',true).on('click',()=>{
			if(this.busy) return
			if(this.nozoomout()) {
				return
			}
	...
	this.coord.grab=this.coord.g.append('rect')
		.attr('fill','white')
		.attr('fill-opacity',0)
		.attr('x',0)
		...
		if(this.gmmode==client.gmmode.exononly || this.gmmode==client.gmmode.protein) {
			...
			*/
			...
			const scale=scaleLinear()
				.domain( rna ? [exoncumbp,exoncumbp+exonlen] :
...
getntsequence4ruler( r, g ) {
	...
	fetch( new Request(this.hostURL+'/ntseq',{
		method:'POST',
		body:JSON.stringify({ genome:this.genome.name, coord:r.chr+':'+(r.start+1)+'-'+r.stop, jwt:this.jwt })
	}))
...
nozoomout() {
	let noout=false
	const r1=this.rglst[this.startidx], r2=this.rglst[this.stopidx]
	if(this.startidx==0 && this.stopidx==this.rglst.length-1) {
		let r1over=false,r2over=false
...
addhlregion(chr, start, stop, color) {
	...
	const hl={
		...
		rect: this.hlregion.g.append('rect')
			.attr('x',x1)
			.attr('y',0)
			.attr('width', x2-x1)
	...
	*/
	...
	if(pos) {
		...
		if(pos.actualposition && pos.actualposition.len<=40) {
			this.addhlregion(pos.chr, pos.actualposition.position-1, pos.actualposition.position-1+pos.actualposition.len-1 )
		}
...
block_jump_gene(s) {
	fetch( new Request( this.hostURL+'/genelookup', {
		method:'POST',
	...
	.then(data=>{
		...
		for(const r of locs) {
			this.coord.inputtip.d.append('div')
...
block_jump_snp(s) {
	coord.string2snp( this.genome.name, s, this.hostURL, this.jwt )
	.then(r=>{
...
genemenu() {
	...
	p2.append('span')
		...
		.html(
			gm.chr+':'+(gm.start+1)+'-'+gm.stop
			+'&nbsp;&nbsp;<span style="font-size:.7em">'+(gm.strand=='+'?'FORWARD':'REVERSE')+'</span>'
			+'&nbsp;&nbsp;<span style="color:#858585">'+common.bplen(gm.stop-gm.start)+'</span>'
		)
	const sec2=menu.append('div')
		.style('margin','20px 30px 40px 30px')
	sec2.append('div')
		.text('Switch display')
...
old_dshandle_new(dsname) {
	...
	const box=this.ctrl.dshandleholder.append('div')
		.style('display','inline-block')
		.style('margin','1px')
		.style('border','solid 1px #545454')
		.style('font-size','.9em')
	...
	const says=box.append('div')
		...
		.on('mouseover',()=>{
			...
			this.blocktip.showunder( d3event.target )
				.clear()
				.d
				.append('div')
				.style('font-size','80%')
				...
				.on('click',()=>{
					this.blocktip.clear()
					client.make_table_2col( this.blocktip.d, ds.dsinfo )
						.style('padding','10px')
				})
		})
	...
	*/
	...
	for(let i=0; i<this.rglst.length; i++) {
		...
		if(pos>=r.bstart && pos<r.bstop) {
			...
			hits.push({
				ridx:i,
				x:(this.regioncumlen(i,true)+(r.reverse ? r.bstop-pos : pos-r.bstart))*this.exonsf+i*this.regionspace
			})
			break
...
setllabel() {
	...
	this.coord.g0.transition().attr('transform','translate('+(this.leftheadw+this.lpad)+','+(this.coordyp1+this.rulerheight)+')')

	const entirewidth = this.leftheadw + this.lpad+this.width+ this.subpanels.reduce((i,j)=>i+j.leftpad+j.width, 0) + this.rpad
...
addchilddsnoload(childds) {
	...
	if(parenthandle) {
		...
		if(d) {
			...
			// create new icon
			parenthandle.childicon=parenthandle.handle.append('div')
			.style('display','inline-block')
			...
			.on('click',()=>{
				const p=d3event.target.getBoundingClientRect()
				const div=client.menushow(p.left-100,p.top+p.height+5).append('div').style('border','solid 1px black').style('padding','20px')
				div.append('div').text('Click to remove').style('font-size','.9em').style('color','#858585').style('margin-bottom','10px')
				for(const tk of this.tklst) {
...
block_maketk(tk) {
	...
	*/
	...
	tk.tkbodybgrect = tk.gmiddle.append('rect')
		.attr('fill','white')
		.attr('fill-opacity',0)

	tk.glider=tk.gmiddle.append('g').attr('transform','translate(0,0)')
		.style('cursor','default')

	tk.gleft=tk.g.append('g').attr('transform','translate('+this.leftheadw+',0)')
	tk.gright=tk.g.append('g').attr('transform','translate('+(this.leftheadw+this.lpad+this.width+this.rpad)+',0)')
	...
	if(tk.list_description) {
		tk.tklabel.on('mouseover',()=>{
			tk.tktip.clear()
				.show(d3event.clientX,d3event.clientY-30)
			client.make_table_2col(tk.tktip.d,  tk.list_description)
				.style('margin','0px')
		})
	...
	tk.cloakbox=tk.cloak.append('rect')
		.attr('fill','white')
		.attr('fill-opacity',0)
	tk.cloaktext=tk.cloak.append('text').text('Loading ...')
	...
	tk.cloakline=tk.cloak.append('line')
...
tk_load(tk) {
	...
	*/
	...
	case client.tkt.ds:
		...
		if(tk.dsuninitiated || !this.usegm || this.gmmode==client.gmmode.genomic || this.gmmodepast==client.gmmode.genomic) {
	...
	*/
	...
	for(const t of this.tklst) {
		...
		for(const [i,p] of t.subpanels.entries()) {
			p.subpaneltkbgrect
				.attr('width', this.subpanels[i].width)
				.attr('height', t.height)
		}
	}
...
setgmmode(mode, willupdate) {
	...
	if(this.rglst.length>1 && this.regionspace>0) {
		// adjust width
		if(this.regionspace*(this.rglst.length-1)>this.width*.3) {
			this.regionspace=Math.max(2,this.width*.3/(this.rglst.length-1))
		}
...
showgmmode4switch(holder, hideuponselect) {
	...
	if(this.usegm.exon.length>1) {
		...
		client.sketchSplicerna(
			row2.append('div')
				.style('vertical-align','middle')
				.style('display','inline-block').style('padding','0px'),
			this.usegm,
			width,
			common.exoncolor
		)
...
showisoform4switch(holder, hideuponselect) {
	...
	*/
	...
	let pxwidth=370
	let intronpx=10
	if(intronpx*(rglst.length-1)>pxwidth*.3) {
		intronpx=Math.max(2,pxwidth*.3/(rglst.length-1))
	}
	...
	for(const gm1 of this.allgm) {
		...
		.on('click',()=>{
			if(hideuponselect) {
				...
				for(const gm2 of gmlabellst) {
					gm2.label.style('color', gm2.isoform==gm1.isoform && gm2.chr==gm1.chr && gm2.start==gm1.start ? '#cc0000' : '#545454')
				}
			}
			...
			*/
			new Promise((resolve,reject)=>{
				...
				fetch( new Request(this.hostURL+'/ntseq',{
					method:'POST',
					body:JSON.stringify({ genome:this.genome.name, coord:gm1.chr+':'+(gm1.start+1)+'-'+(gm1.stop), jwt:this.jwt })
				}))
				.then(data=>{return data.json()})
				.then(data=>{
					if(data.error) throw({message:data.error})
		...
		tr.append('td').text(gm1.isdefault ? 'DEFAULT' : '')
			.style('font-size','.6em')
		const lab=tr.append('td').text(gm1.isoform)
			.style('color', gm1.isoform==this.usegm.isoform && gm1.chr==this.usegm.chr && gm1.start==this.usegm.start ? '#cc0000' : '#545454')
		gmlabellst.push({
			isoform:gm1.isoform,
			chr:gm1.chr,
			start:gm1.start,
	...
	*/
	...
	for(let panelidx=0; panelidx<this.subpanels.length; panelidx++) {
		...
		for(const tk of this.tklst) {
			...
			if(thispanel.chr==panelrecord.chr && thispanel.start==panelrecord.start && thispanel.stop==panelrecord.stop && thispanel.width==panelrecord.width) {
...
new_tk_subpanel(tk, panelrecord) {
	...
	*/
	...
	obj.gtksubpanel = tk.gtksubpanels.append('g')
		.on('mousedown',()=>{
			...
			body.on('mousemove',()=>{
				...
				for(const t2 of this.tklst) {
					...
					const t2panel = t2.subpanels.find(i=>i.chr==panelrecord.chr && i.start==panelrecord.start && i.stop==panelrecord.stop)
					if(t2panel) {
	...
	obj.subpaneltkbgrect = obj.gtksubpanel.append('rect')
		.attr('fill','white')
		.attr('fill-opacity',0)
	...
	obj.cloakbox = obj.cloak.append('rect')
		.attr('fill','white')
		.attr('fill-opacity',0)
	obj.cloaktext = obj.cloak.append('text')
		.text('Loading ...')
		.attr('fill',common.defaultcolor)
		.attr('fill-opacity',0)
		.attr('font-weight','bold')
...
tkerror_subpanel(panel,msg) {
	panel.gerror = panel.glider.append('text')
		.text(msg)
		.attr('x',panel.width/2)
		.attr('text-anchor','middle')
		.attr('y', panel.height/2)
...
zoombutton_mouseover( fold, zoomout, button) {
	...
	*/
	...
	
	{
		...
		this.blocktip.clear().showunder( button )
			.d
			.style('top',null)
			.style('bottom', (window.innerHeight-p.top-window.pageYOffset+3)+'px')
	}

	this.blocktip.d.append('div')
...
init_coord_subpanel(panel) {
	...
	*/
	...
	panel.subpanelbgrect = panel.coord.g0.append('rect')
		.attr('y',-this.coordyp1-this.rulerheight)
	if(panel.background) {
		panel.subpanelbgrect.attr('fill', panel.background)
	} else {
		panel.subpanelbgrect.attr('fill','white')
			.attr('fill-opacity',0)
	}

	panel.subpanelleftborder = panel.coord.g0.append('rect')
		.attr('y', -this.coordyp1-this.rulerheight)
...
updateruler_subpanel(panel) {
	...
	panel.coord.height = this.coordyp1+this.rulerheight+this.coordyp2+( panel.stop-panel.start<=panel.width ? baseheight+2 : 0)
	this.update_ruler_height()
	...
	if(panel.stop-panel.start <= panel.width) {
		// get nt sequence
		const seqg = panel.coord.axesg.append('g')
			.attr('transform','translate(0,'+this.coordyp2+')')
...
mds_handle_make(key) {
	...
	if(!ds) return
	const d0=this.ctrl.mdsHandleHolder.append('div')
		.style('display','inline-block')
	...
	ds.handle.labelButton=d0.append('div')
		.text(ds.label)
...
function makecoordinput(bb, butrow) {
	...
	bb.coord.input=butrow.append('input')
		.attr('type','text')
		.attr('size',20)
		.style('margin-left','10px')
		...
		.then(data=>{
			...
			if(data.hits && data.hits.length) {
				for(const s of data.hits) {
					bb.coord.inputtip.d.append('div').attr('class','sja_menuoption').text(s)
					.on('click',()=>{
						bb.coord.inputtip.hide()
...
function maygetdna(block, tip) {
	...
	if(totallen > dnalenlimit) {
		tip.d
			.append('div')
			.text('Please zoom in under '+client.bplen(dnalenlimit)+' to get DNA sequence.')
		return
	}
	const wait = tip.d
		.append('div')
...
function init_cursorhlbar(block) {
	...
	block.cursorhlbar=block.gbase.append('rect')
		.attr('fill',block.cursorhlbarFillColor)

	block.gbase.on('mousemove',()=>{
