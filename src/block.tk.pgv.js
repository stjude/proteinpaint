	...
	*/
	...
	if(tk.genevaluetklst) {
		...
		for(const gvtk of tk.genevaluetklst) {
			...
			gvtk.label = tk.gright.append('text')
		...
		if(tk.genevaluetklst.length==1) {
			...
			for(const gvtk of tk.genevaluetklst) {
				gvtk.label
					.attr('y',tk.axisfontsize+2-tk.toppad)
		...
		tk.config_handle = block.maketklefthandle(tk, -block.labelfontsize + gvtkheaderyoff)
			.text('CONFIG')
			.attr('fill','#858585')
			.attr('x',5)
		...
		tk.changegenelabel = tk.gright.append('text')
			.attr('text-anchor','end')
	...
	if(block.legend && block.legend.holder) {
		...
		if(tk.legendimg && tk.legendimg.file) {
			...
			.then(data=>{
				...
				let fold=true
				const img = tk.td_legend.append('img')
					.attr('class','sja_clbb')
					.attr('src',data.src)
					.style('height','80px')
				img.on('click',()=>{
		...
		if(tk.genevaluetklst) {
			...
			for(const gvtk of tk.genevaluetklst) {
				if(gvtk.multivaluekey) {
					...
					gvtk.legend.label_genespecific = gvtk.legend.div.append('div')
						.style('display','inline-block')
						.style('margin','5px 10px 10px 0px')
						.style('color','#858585')
						.style('vertical-align','top')
					gvtk.legend.contentdiv = gvtk.legend.div.append('div')
						.style('display','inline-block')
	...
	for(const t of tk.tracks) {
		...
		*/
		...
		t.tklabel=t.immobileg.append('text')
			.attr('font-size',tk.axisfontsize)
			.attr('font-family',client.font)
			.attr('text-anchor','end')
		...
		if(tk.genevaluetklst) {
			...
			for(const gvtk of tk.genevaluetklst) {
				...
				obj.hline = obj.gvtk_g_xshift.append('line')
					.attr('stroke','#ccc')
					...
					if(data.nodata) {
						...
						client.axisstyle({
							axis:t.leftaxis.call(
								axisLeft().scale(scale).tickValues([t.scale.min,t.scale.max])
							),
							color:'black',
							showline:true
						})
					...
					.on('click',()=>{
						...
						if(gvtk.legend.gene2hiddenkeys.has(tk.__usegene) && gvtk.legend.gene2hiddenkeys.get(tk.__usegene).has(name)) {
							...
							tk.legendtip.d.append('div')
								...
								.on('click',()=>{
									tk.legendtip.hide()
									if(!gvtk.legend.gene2hiddenkeys.has(tk.__usegene)) gvtk.legend.gene2hiddenkeys.set(tk.__usegene, new Set())
						...
						tk.legendtip.d.append('div')
							...
							.on('click',()=>{
								...
								for(const name2 of gvtk.runtimekey2color_genespecific.keys()) {
									if(name2!=name)
										gvtk.legend.gene2hiddenkeys.get(tk.__usegene).add(name2)
								}
								showgeneplot(tk, block, tk.__usegene)
							})
						tk.legendtip.d.append('div')
...
function configPanel_bwcommon(tk, block) {
	...
	bigwigconfigpanel(mock, block, tk.tkconfigtip.d, (code)=>{
		...
		case client.bwSetting.height:
			...
			tk.tracks.forEach(t=>{if(t.type==client.tkt.bigwig) t.barheight=mock.barheight})
			break
		case client.bwSetting.pcolor:
			tk.bigwigsetting.pcolor = mock.pcolor
			tk.tracks.forEach(t=>{if(t.type==client.tkt.bigwig) t.pcolor=mock.pcolor})
			break
		case client.bwSetting.ncolor:
			tk.bigwigsetting.ncolor = mock.ncolor
			tk.tracks.forEach(t=>{if(t.type==client.tkt.bigwig) t.ncolor=mock.ncolor})
			break
		case client.bwSetting.pcolor2:
			tk.bigwigsetting.pcolor2 = mock.pcolor2
			tk.tracks.forEach(t=>{if(t.type==client.tkt.bigwig) t.pcolor2=mock.pcolor2})
			break
		case client.bwSetting.ncolor2:
			tk.bigwigsetting.ncolor2 = mock.ncolor2
			tk.tracks.forEach(t=>{if(t.type==client.tkt.bigwig) t.ncolor2=mock.ncolor2})
			break
		case client.bwSetting.autoscale:
			tk.bigwigsetting.scale.auto = 1
			tk.tracks.forEach(t=>{if(t.type==client.tkt.bigwig) t.scale.auto=1})
			break
		...
		case client.bwSetting.nodotplot:
			...
			tk.tracks.forEach(t=>{if(t.type==client.tkt.bigwig) delete t.dotplotfactor})
			break
		case client.bwSetting.usedotplot:
			tk.bigwigsetting.dotplotfactor = mock.dotplotfactor
			tk.tracks.forEach(t=>{if(t.type==client.tkt.bigwig) t.dotplotfactor=mock.dotplotfactor})
			break
		...
		case client.bwSetting.nodividefactor:
			...
			tk.tracks.forEach(t=>{if(t.type==client.tkt.bigwig) t.normalize.disable=1})
			break
		}
		loadTk(tk, block)
	})
