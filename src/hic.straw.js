...
export function hicparsefile(hic, debugmode) {
	...
	*/
	...
	hic.wholegenomebutton = td.append('button')
		.style('display','none')
		.text('Genome')
		.on('click',()=>{
			hic.inwholegenome=true
	...
	hic.chrpairviewbutton = td.append('button')
		.style('display','none')
		.on('click',()=>{
	...
	.then(()=>{
		...
		return fetch( new Request(hic.hostURL+'/textfile',{
			method:'POST',
	...
	.then(()=>{
		return fetch( new Request(hic.hostURL+'/hicstat',{
			method:'POST',
...
export function hicparsestat(hic, txt) {
	...
	*/
	...
	for(let i=0; i<lines.length; i++) {
		if(lines[i].startsWith('Chromosomes')) {
			chrline=i
		} else if(lines[i].startsWith('Base')) {
			baseline=i
		} else if(lines[i].startsWith('Frag')) {
			fragline=i
		}
	}
	if(chrline==-1) return 'Chromosomes not found in file stat'
	if(baseline==-1) return 'Base pair-delimited resolutions not found in file stat'
	if(fragline==-1) return 'Fragment-delimited resolutions'

	const chrlst=[]
	for(let i=chrline+1; i<baseline; i++) {
		chrlst.push(lines[i].trim().split(/\s/)[0])
	}
	...
	hic.bpresolution=[]
	for(let i=baseline+1; i<fragline; i++) {
		const s=lines[i].trim()
		const v=Number.parseInt(s)
		if(Number.isNaN(v)) {
			return 'invalid basepair resolution value: '+s
		}
		hic.bpresolution.push(v)
	}
	hic.bpresolution.sort((a,b)=>b-a) // descending order

	hic.fragresolution = []
	for(let i=fragline+1; i<lines.length; i++) {
		const s=lines[i].trim()
		const v=Number.parseInt(s)
		if(Number.isNaN(v)) {
			return 'invalid fragment resolution value: '+s
		}
		hic.fragresolution.push(v)
	}
	hic.fragresolution.sort((a,b)=>b-a)
}
...
function init_wholegenome(hic) {
	...
	*/
	...
	const table=hic.holder.append('table')
	const tr1=table.append('tr')
	hic.c.td = tr1.append('td')
		.style('vertical-align','top')
	hic.y.td = tr1.append('td')
		.style('vertical-align','top')
	const tr2=table.append('tr')
	...
	*/
	...
	hic.wholegenome.svg
		.attr('width', hardcode_wholegenomechrlabwidth+xoff)
		.attr('height', fontsize+yoff)
	return
}
...
function makewholegenome_chrleadfollow(hic, lead, follow) {
	...
	*/
	...
	obj.canvas = hic.holder.append('canvas')
		.style('display','none')
		.node()
	...
	obj.img = hic.wholegenome.layer_map.append('image')
	...
	if(lead!=follow) {
		...
		obj.canvas2 = hic.holder.append('canvas')
			.style('display','none')
			.node()
		...
		obj.img2 = hic.wholegenome.layer_map.append('image')
...
function getdata_leadfollow(hic, lead, follow) {
	...
	fetch( new Request(hic.hostURL+'/hicdata', {
		method:'POST',
	...
	.then(data=>{
		...
		for(const [plead, pfollow, v] of data.items) {
			...
			const p= v>=hic.wholegenome.bpmaxv ? 0 : Math.floor(255*(hic.wholegenome.bpmaxv-v)/hic.wholegenome.bpmaxv)
...
function tooltip_sv(hic, item) {
	hic.tip.clear()
		...
		.append('div')
		.text(
			item.chr1==item.chr2 ? item.chr1+' : '+item.position1+' - '+item.position2
...
function init_chrpair(hic, chrx, chry) {
	...
	*/
		...
		client.axisstyle({
			axis: svg.append('g')
					...
					.call( axisRight().scale(
						scaleLinear().domain([0,chrylen]).range([0, h])
						)
		...
		client.axisstyle({
			axis: svg.append('g')
					...
					.call( axisBottom().scale(
						scaleLinear().domain([0, chrxlen]).range([0, w ])
						)
						.tickFormat(d3format('.2s'))
					),
			showline:true,
	...
	const canvas=hic.c.td.append('canvas')
...
function getdata_chrpair(hic) {
	...
	.then(data=>{
		...
		*/
		...
		const maxv = vlst.sort((a,b)=>a-b) [ Math.floor(vlst.length*.99) ]
...
function init_detail(hic, chrx, chry, x, y) {
	...
	*/
	...
	hic.chrpairviewbutton
		.text('Entire '+chrx+'-'+chry)
	...
	const canvasholder = hic.c.td.append('div')
		.style('position','relative')
		.style('width',blockwidth+'px')
		.style('height',blockwidth+'px')
		.style('overflow','hidden')

	const canvas = canvasholder.append('canvas')
		.style('display','block')
		...
		.on('mousedown',()=>{
			...
			body.on('mousemove',()=>{
				...
				canvas.style('left',(oldx+xoff)+'px')
					.style('top',(oldy+yoff)+'px')
			})
			body.on('mouseup',()=>{
	...
	*/
	{
		const row = canvasholder.append('div')
			.style('position','absolute')
			.style('right','100px')
			.style('top','20px')
		row.append('button')
		...
		row.append('button')
			...
			.on('click',()=>{
				...
				if( regionx.chr==regiony.chr && Math.max(regionx.start,regiony.start)<Math.min(regionx.stop,regiony.stop) ) {
					...
					arg.width = default_subpanelpxwidth
					arg.subpanels = [ {chr:regiony.chr, start: regiony.start, stop: regiony.stop, width: default_subpanelpxwidth, leftpad:10,leftborder:subpanel_bordercolor} ]
				}
				blocklazyload(arg)
			})
	...
	arg.onloadalltk_always=(bb)=>{
		...
		*/
		canvasholder
			.style('width', (2*hic.detailview.bbmargin+bb.leftheadw+bb.lpad+bb.width+bb.rpad+bb.rightheadw)+'px')
	...
	const sheath=hic.y.td.append('div')
		.style('position','relative')
		.style('width','200px') // dummy size
		.style('height','800px')

	const rotor = sheath.append('div')
		.style('position','absolute')
		.style('bottom','0px')
	...
	arg2.onsetheight = (bbh)=>{
		rotor.transition()
			.style('left', ( hic.detailview.bbmargin + bbh + buttonrowh )+'px')
	}
...
function detailviewupdateregionfromblock(hic) {
	...
	detailviewupdatehic(
		hic,
		rx.chr, rx.start, rx.stop,
		ry.chr, ry.start, ry.stop
	)
}
	...
	*/
	...
	.then(()=>{
		...
		*/
		...
		return fetch( new Request( hic.hostURL+'/bedjdata', {
			method:'POST',
			body:JSON.stringify(arg)
		}))
		.then(data=>{return data.json()})
	...
	*/
	...
	.then(data=>{
		...
		hic.detailview.canvas
			.attr('width',hic.detailview.xb.width)
			.attr('height',hic.detailview.yb.width)
