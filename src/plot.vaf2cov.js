...

{
	...
	for(const i of arg.data) {
		...
		ybin[Math.floor( (i.maf==1 ? .99 : i.maf) /ybs)]++
	}
}
...
const box=boxg.append('rect')
	.attr('stroke',gray)
	.attr('stroke-dasharray','2,2')
	.attr('fill','none')
	.attr('shape-rendering','crispEdges')
const midline=boxg.append('line')
	.attr('stroke',gray)
	.attr('stroke-dasharray','2,2')
	.attr('shape-rendering','crispEdges')
// ybin
const ybing=svg.append('g')
const ybinbar=ybing.selectAll()
	.data(ybin).enter()
	.append('rect')
const ybinaxis=svg.append('g')
// xbin
const xbing=svg.append('g')
const xbinbar=xbing.selectAll()
	.data(xbin).enter()
	.append('rect')
...
const spgkick=spg.append('circle')
	.attr('fill','white')
	.attr('fill-opacity',0)
	.on('mouseover',(d)=>{
		...
		client.make_table_2col(arg.tip.d, lst)
			.style('margin','none')
...
function resize() {
	...
	svg.attr('width',fontsize+axisw+pad0+width+pad+barwidth+pad2+ticksize)
		.attr('height',fontsize/2+barheight+pad+height+pad0+axish+ticksize+fontsize)
	xlab.attr('font-size',fontsize)
	...
	xaxis.attr('transform','translate('+(fontsize+axisw+pad0)+','+(fontsize/2+barheight+pad+height+pad0)+')')
	...
	yaxis.attr('transform','translate('+(fontsize+axisw)+','+(fontsize/2+barheight+pad)+')')
	...
	spg.attr('transform',d=> 'translate('+xscale(d.total > maxtotal ? maxtotal : d.total)+','+yscale(d.maf)+')' )
	...
	ybing.attr('transform','translate('+(fontsize+axisw+pad0+width+pad)+','+(fontsize/2+barheight+pad+height)+')')
	...
	ybinbar
		...
		.attr('height',binh)
		.attr('fill',gray)
	ybinaxis.attr('transform','translate('+(fontsize+axisw+pad0+width+pad)+','+(fontsize/2+barheight+pad+height+pad0)+')')
		.call(axisBottom()
			.scale(ybinscale)
	...
	xbinbar.attr('x',(d,i)=> binw*i )
		.attr('y',(d)=> xbinscale(d)-barheight )
		.attr('height',(d)=> barheight-xbinscale(d) )
		.attr('width',binw)
		.attr('fill',gray)
	xbinaxis.attr('transform','translate('+(fontsize+axisw)+','+(fontsize/2)+')')
		.call(axisLeft()
			.scale(xbinscale)
