...
export default function maftimelineui(genomes) {
	...
	inputdiv.append('p').html('<a href=https://docs.google.com/document/d/1WXlCVHLq_P2jGiRGJ55uzXCC6p-6Jqn7h406RUID_xo/edit?usp=sharing target=_blank>File format</a>')
...
function render(data,header,holder) {
	...
	for(const patient in data) {
		...
		const div=patientholder.append('div')
...
function makegraph(snvlst0,header,holder,issub) {
	...
	if(showcircle) {
		...
		.attr('fill',d=>d.color) .attr('fill-opacity',.1)
		.on('mouseover',d=>{
			d3event.target.setAttribute('fill-opacity',.4)
			tip.clear()
	...
	const setsize=(w,h)=>{
		...
		drag.attr('x',yaxisw+xpad+width)
			.attr('y',toppad+height+ypad+xaxish-4)
		client.axisstyle({
		...
		client.axisstyle({
			axis:yaxisg.call(axisLeft().scale(yscale).tickValues([1,.1,.01,.001,.0001,zerovalue]).tickFormat(d3format(' '))),
			showline:true,
			color:'black',
		...
		if(showcircle) {
			...
			legend.append('circle')
				...
				.attr('cy',y)
				.attr('fill','#ccc')
			legend.append('text').text(maxcoverage)
		...
		for(const snv of snvlst) {
			...
			legend.append('text').text(snv.gene)
				.attr('dominant-baseline','central')
...
function makecontrol(snvlst,header,holder,graphholder,g1,g2,g3,g4) {
	...
	for(const snv of snvlst) {
		...
		const c1=td11.append('input').attr('type','checkbox').attr('id',cname).on('change',()=>cchange(1,d3event.target))
		td11.append('label').html('&nbsp;shared').attr('for',cname)

		cname=Math.random()
		const td22=tr.append('td')
		const c2=td22.append('input').attr('type','checkbox').attr('id',cname).on('change',()=>cchange(2,d3event.target))
		td22.append('label').html('&nbsp;rising').attr('for',cname)

		cname=Math.random()
		const td33=tr.append('td')
		const c3=td33.append('input').attr('type','checkbox').attr('id',cname).on('change',()=>cchange(3,d3event.target))
		td33.append('label').html('&nbsp;falling').attr('for',cname)

		cname=Math.random()
		const td44=tr.append('td')
		const c4=td44.append('input').attr('type','checkbox').attr('id',cname).on('change',()=>cchange(4,d3event.target))
