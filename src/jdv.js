...
import {bplen, exoncolor} from './common'
...
function serverquery4sample(jdv, spobj) {
	...
	*/
	...
	if(spobj.textfile) {
		...
		d3json(jdv.hostURL+'/textfile').post(JSON.stringify({file:spobj.textfile,from:(spobj.junctions.length+1),to:100}),data=>{
			if(spobj.ui) {
...
function newjunction2table(spobj, jlst) {
	...
	*/
	...
	for(const j of jlst) {
		...
		const tr=spobj.ui.listtable.append('tr')
			.attr('class','sja_clb_gray')
		...
		tr.append('td')
			.style('font-size','.8em')
			.html(j.chr+':'+(j.start+1)+'-'+(j.stop+1)
				+'<br><span style="color:#858585">'+bplen(j.stop-j.start)+'</span>'
				)
		...
		// 3 - gene
		tr.append('td')
			.text(cellvalue_genename4junction(j.jd))
...
function makesampleui(spobj, jdv) {
	...
	*/
	...
	const column1=tr0.append('td')
		.style('vertical-align','top')
	const column2=tr0.append('td')
		.style('vertical-align','top')

	const errdiv=column1.append('div')
		.style('margin-bottom','10px')

	const div=column1.append('div')
		.style('padding-top',headerpad)
		.style('position','relative')
	
	const scrollholder=div.append('div')
		.style('overflow-y','scroll')
		.style('height','600px')
		.style('resize','vertical')

	const listtable=scrollholder.append('table')
		.style('border-spacing','2px')
		.style('border-collapse','separate')
	
	const header=listtable.append('tr')
		.style('font-size','.8em')
	...
	header.append('td')
		.append('div')
		.style('position','absolute').style('top','1px')
		.html('read<br>count')
	// 3 - gene
	header.append('td')
		.append('div')
	...
	const loadbutton=loadtr.append('td')
		.attr('colspan',7)
...
function cellvalue_exonskip(td, events) {
	...
	// exon numbers
	td.append('span').html('<span style="font-size:.7em;color:#ccc">EXON</span>'+events[0].skippedexon.map(i=>i+1).join(','))
}
	...
	*/
	...
	// normal junction
	{
		const line=g.append('path')
			.attr('d',
				'M'+(intronstart)+','+(junctionheight+exonheight)+
				'L'+((intronstart+intronstop)/2)+','+(junctionheight*2+exonheight)+
				'L'+(intronstop)+','+(junctionheight+exonheight)
				)
			.attr('stroke',exoncolor)
			.attr('fill','none')
	...
	svg.attr('width',xpad*2+exonwidth*2+intronwidth+distlabelw+distlabelpad*2)
