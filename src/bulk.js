...
exports.init_bulk_flag=function(genome) {
	if(!genome) {
		return null
	}
...
exports.parsesample=function(m,flag,i,lst,badline) {
		...
		if(m.disease) {
			if(m.sample in flag.sample2disease) {
				if(m.disease != flag.sample2disease[m.sample]) {
					flag.snv.badlines.push([i,'conflict of disease types for sample "'+m.sample+'": '+m.disease+', '+flag.sample2disease[m.sample],lst])
					return true
				}
			} else {
