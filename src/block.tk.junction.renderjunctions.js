...
client.axisstyle({
	axis:tk.leftaxis.call(
		axisRight().scale(tk.yscale).ticks(6,'.0f')
	),
	color:'black',
	showline:true
})
...
function set_rim(d){
	return d.mode==modefold ? 0 : .7
}
function set_qmg(d,tk){
	if(d.mode==modesample) return 'translate(0,-'+(tk.legheight+tk.maxradius*4+tk.lowpad+tk.padheight)+') scale(1)'
...
function set_jug2(d,tk) {
	...
	if(d.mode==moderaise) return 'translate(0,-'+(tk.legheight+tk.lowpad+tk.maxradius*4+tk.padheight+tk.axisheight-tk.yscale( d.median ))+')'
...
function set_all(tk) {
	const dur=500
	if(tk.totalsamplecount>1) {
		tk.jug.selectAll('.sja_jug_qmg')
			.transition().duration(dur)
			.attr('transform',(d)=> set_qmg(d,tk))
		tk.jug.selectAll('.sja_jug_discnum')
			.transition().duration(dur)
			.attr('fill-opacity',(d)=> set_discnum(d))
		/*
...
function foldedjunctioninfo(d, tk) {
	...
	let pad=5,
		y=pad,
		width=0
	const bg=tk.pica.g.append('rect')
		.attr('stroke', d.color)
		.attr('shape-rendering','crispEdges')
		.attr('fill','white')
		.attr('fill-opacity',.8)
	if(d.type) {
		tk.pica.g.append('text').text(d.type)
			.attr('fill',d.color)
	...
	tk.pica.g.append('line')
		...
		.attr('y2',y+15)
	y+=15
	tk.pica.g.attr('transform','translate('+(d.x-30)+','+(tk.toppad+tk.axisheight+tk.maxradius*4+tk.padheight-d.radius*2-y)+')')
}
...
function expandedjunctioninfo(d, tk, circle) {
	...
	holder.append('div').html(common.bplen(d.stop-d.start)+' <span style="font-size:.8em;">'+d.chr+':'+(d.start+1)+'-'+(d.stop+1)+'</span>')
	if(d.data) {
		// multi-sample
		if(d.data.length>1) {
			holder.append('div').html(d.data.length+' <span style="font-size:.8em;color:#858585">samples</span>')
			if(d.rimcount) {
				holder.append('div').html(d.rimcount+' <span style="font-size:.8em;color:#858585">show '+tk.rimwhat+'</span>')
			}
	...
	if(d.spliceevents && d.spliceevents.length>0) {
		const renderdiv=holder.append('div')
	...
	*/
	...
	if(tk.cohortsetting && tk.cohortsetting.cohort && tk.cohortsetting.cohort.levels) {
		...
		for(const lev of tk.cohortsetting.cohort.levels) {
			if(sample[lev.k]) {
				holder.append('div').html(sample[lev.k]+' <span style="font-size:.7em;color:#858585">'+lev.label+'</span>')
			}
		}
	}
	...
	if(sample.metadata) {
		// FIXME
		holder.append('div').html('<span style="background-color:'+sample.metadata.color+'" class=sja_mcdot>&nbsp;</span> '+sample.metadata.value+' <span style="font-size:.7em;color:#858585">'+tk.metadata.m.label+'</span>')
	}
	...
	if(j.spliceevents) {
		...
		*/
		...
		const skipwhichexon = 'exon '+ evt.skippedexon.map(i=>i+1).join(',')+' '
			+(evt.isskipexon ? 'skip' : 'alt. use')
		// find out percentage
