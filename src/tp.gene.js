...
toprow.append('input')
	...
	.on('keyup',()=>{
		...
		for(let i=0; i<Math.min(30,hit.length); i++) {
			...
			.html(n+' <span style="font-size:.7em">'+hit[i].count+'</span>')
			.on('click',()=>{
				paintgene(n)
			})
		}
...
const scrollholder=secondrow.append('div')
	.style('padding-top',scrolltoppad+'px')
	.style('position','relative')
const scrolldiv=scrollholder.append('div')
...
const importdiv=optiondiv.append('div').style('padding','10px').style('border-bottom','dashed 1px #ccc')
// option row1
const gcsays=oprow1.append('span').style('padding-right','10px')
...
const genesetta=oprow2.append('textarea')
	.attr('rows',1)
	.attr('cols',20)
...
importdiv.append('p').style('color','#858585').style('font-size','.8em').text('Load 10 genes each click.')
const hassamplelst=[]
for(const k in cohort.dsset) {
...
if(hassamplelst.length) {
	...
	for(const ds of hassamplelst) {
		oprow5.append('button').text(hassamplelst.length==1?'show':ds.label).on('click',()=>{
			const bars=[]
	...
	for(const ds of hassamplelst) {
		oprow6.append('button').text(hassamplelst.length==1?'show':ds.label).on('click',()=>{
			const samplehash={}
...
const dotable=()=>{
	...
	tr1.append('td') // numeritor
		.style('height','0px')
		.style('padding','0px')
	tr1.append('td') // gene name
		.style('height','0px')
		.style('padding','0px')
	for(const ds of attrlst) {
		...
		tr1.append('td').attr('colspan',spannum)
	...
	for(const dsname in ds2import) {
		...
		const td=tr1.append('td').attr('colspan',2+importclass[dsname].length)
			.style('border-right',impspace)
			.append('div')
			.style('top','2px')
			.style('position','absolute')
		...
		td.append('span').text(cohort.genome.datasets[dsname].label
	...
	for(const a of attrlst) {
		...
		for(const at of lst) {
			...
			td.append('div')
				.html(
					at.rotate ? (at.sort ? (at.descend? '&#9664;':'&#9654;')+' '+at.label : at.label) :
