...
// for skewer track
export const middlealignshift=.3
...
export function dstkload(tk,block) {
	...
	*/
	...
	if(tk.ds.bulkdata || tk.ds.mlst) {
		...
		if(block.usegm) {
			...
			if(tk.ds.dbexpression && (!tk.eplst || tk.eplst.length==0)) {
				...
				fetch( new Request(block.hostURL+'/dbdata', {
					method:'POST',
	...
	fetch( new Request(block.hostURL+'/dsdata',{
		method:'POST',
		body:JSON.stringify(par)
	}))
	.then(data=>{return data.json()})
...
// disc containers
const discg=tk.skewer.selectAll()
	.data(d => d.groups)
	.enter().append('g')
	.attr('transform',d=>'translate(0,'+(d.aa.showmode==modefold ? 0 : d.yoffset*(tk.aboveprotein ? -1:1))+')')
...
textslc
	.filter(d=>d.dt==common.dtsnvindel)
	.attr('fill','white')
textslc
...
textslc
	...
	.attr('fill',d=> color4disc(d.mlst[0]))
// right-side label
const textlab=discg.append('text')
...
if(tk.hlaachange) {
	...
	//const big=1.3
	textlab.filter((d)=>tk.hlaachange.has(d.mlst[0].mname))
	.classed('sja_pulse',true)
	/*
	.attr('font-weight',(d)=>{
...
// set stem lengths
{
	...
	tk.stem2= scaleLinear()
		...
		.range([0,dotwidth,dotwidth*3])
		(lapcount)
}
// stem3
const hbaseline=dotwidth*.7
...
tk.skewer.append('circle')
	...
	.on('mouseover',d=>{
		...
		tk.pica.g
			.attr('transform','translate('+tk.pica.x+','+tk.pica.y+')')
		_g.filter(g=>g.mlst.length>1)
			...
			.attr('dominant-baseline',(abp?'hanging':'auto'))
			.attr('fill','#9e9e9e')
		const handle=tk.pica.g.append('g')
...
if(stackbars.length>0) {
	...
	.on('mouseover',d=>{
		...
		const fontsize1=14,
			...
			hpad=4
		let width=0
		pica.g.append('text').text(label1)
			...
			.remove()
		pica.g.append('text').text(label2)
		...
		pica.y= tk.aboveprotein ? tk.height_main -tk.toppad-tk.bottompad-d.y-d.height-3-boxheight : d.y+d.height+3
...
mgsk.append('rect')
	...
	.attr('width',d=>{
		d.ssk_width=Math.max(
			d.mlst.length.toString().length*8 +6,
			2*(d.maxradius+d.maxrimwidth)
			)
		return d.ssk_width
		})
...
export function skewer_flip(tk) {
	...
	skbars.selectAll('.sja_aa_stackbar_g')
	...
	skbars.filter(d=>d.showmode==modeshow)
		.selectAll('.sja_aa_stackbar_rect')
		.transition().duration(dur)
		.attr('y',d => abp ? -d.height : 0 )
	tk.skewer.transition().duration(dur)
		.attr('transform',d=>{
			d.y=skewer_sety(d,tk)
			return 'translate('+d.x+','+d.y+')'
		})
	tk.skewer.selectAll('.sja_aa_stem')
		.transition().duration(dur)
		.attr('d',d=> skewer_setstem(d,tk))
	tk.skewer.selectAll('.sja_aa_discg')
		.transition().duration(dur)
		.attr('transform',d=>{
			d.y=(abp ? -1 : 1)*(d.aa.showmode==modefold?d.aa.maxradius:d.yoffset)
			return 'translate(0,'+d.y+')'
		})
	tk.skewer.selectAll('.sja_aa_ssk_kick')
		.attr('y',abp?0:-tk.stem1)
	tk.skewer.selectAll('.sja_aa_ssk_bg')
		.attr('y',abp?0:-tk.stem1)
	tk.skewer.selectAll('.sja_aa_ssk_text')
		.attr('y',(abp?1:-1)*tk.stem1/2)
	tk.skewer.selectAll('.sja_aa_skkick')
		.attr('cy',d => (abp?-1:1)*d.maxradius)
	tk.skewer	
...
function unfold_glyph(newlst,tk,block) {
	...
	if(hasfolded) {
		...
		set.transition().duration(dur)
			.attr('transform',d=>'translate('+d.x+','+d.y+')' )
		set.selectAll('.sja_aa_discg')
			.transition().duration(dur)
			.attr('transform',d=>{
		...
		set.selectAll('.sja_aa_qmg')
			...
			*/
		set.selectAll('.sja_aa_discnum')
			.transition().duration(dur)
			.attr('fill-opacity',1)
			.attr('stroke-opacity',1)
		set.filter(d=> d.groups.length>1)
			.selectAll('.sja_aa_disclabel')
			.transition().duration(dur)
			.attr('fill-opacity',1)
			.attr('transform','scale(1)')
		set.selectAll('.sja_aa_discrim')
			.transition().duration(dur)
			.attr('fill-opacity',1)
			.attr('stroke-opacity',1)
		set.selectAll('.sja_aa_ssk_kick')
			.attr('transform','scale(1)')
			.attr('y',abp?0:-tk.stem1)
		set.selectAll('.sja_aa_ssk_bg')
			.attr('transform','scale(1)')
			.attr('y',abp?0:-tk.stem1)
		set.selectAll('.sja_aa_ssk_text')
			.attr('transform','scale(1)')
			.attr('y',(abp?1:-1)*tk.stem1/2)
		set.selectAll('.sja_aa_skkick')
			.attr('transform','scale(0)')
		set.filter(d=> d.hasstackbar)
...
function unfold_update(tk,block) {
	...
	skbars.selectAll('.sja_aa_stackbar_g')
		.transition().duration(dur)
		.attr('transform',d=> 'translate('+(d.grp.aa.x0-d.grp.aa.x)+','+((abp?1:-1)*(tk.stem1+tk.stem2+tk.stem3-d.y))+')')
	skbars.selectAll('.sja_aa_stackbar_rect')
		.transition().duration(dur)
		.attr('width',d=>{
			return d.pxspan
			})
...
export function fold_glyph(lst,tk) {
	...
	set.selectAll('.sja_aa_discnum')
		...
		.attr('fill-opacity',0)
		.attr('stroke-opacity',0)
	set.selectAll('.sja_aa_disclabel')
		.transition().duration(dur)
		.attr('fill-opacity',0)
	...
	set.selectAll('.sja_aa_ssk_text')
		.attr('transform','scale(0)')
	set.selectAll('.sja_aa_skkick')
		.transition().duration(dur) // to prevent showing pica over busy skewer
		.attr('transform','scale(1)')
	const skbars=set.filter(d=> d.hasstackbar)
	skbars.selectAll('.sja_aa_stackbar_g')
		.transition().duration(dur)
	...
	for(const mcset of tk.vcfinfofilter.lst) {
		...
		if(mcset.numericfilter) {
			...
			for(const o of mcset.numericfilter) {
				...
				const cell=mcset.holder.append('div')
				...
				if(mcset.numericCutoff==o.value && mcset.numericCutoffSide==o.side) {
					cell.style('border-bottom','solid 2px #ccc')
...
function showlegend_populationfrequencyfilter(tk, block) {
	...
	*/
	...
	for(const o of tk.populationfrequencyfilter.lst) {
		...
		const cell=tk.populationfrequencyfilter.holder.append('div')
			.style('display','inline-block')
			.style('padding','10px')
			.attr('class','sja_clb')
			.on('click',()=>{
...
function showlegend_sampleattribute(tk, block) {
	...
	*/
		...
		*/
		for(const attr of smat.lst) {
			...
			runtimelst.push({
				...
				key:attr,
				lst:[...value2count].sort((a,b)=>b[1]-a[1]).map(i=>{return {value:i[0],count:i[1]}})
			})
		}
	} else {
		/*
	...
	const table=smat.holder.append('table')
...
export function mlst_pretreat(tk, block, originhidden) {
	...
	*/
	...
	tk.tklabel.each(function(){ collectleftlabw.push(this.getBBox().width)})
	...
	if(tk.label_stratify) {
		for(const strat of tk.label_stratify) {
			...
			if(set.size==0) {
				...
				strat.svglabel.text(set.size+' '+strat.label+(set.size>1?'s':''))
					.each(function(){
						collectleftlabw.push(this.getBBox().width)
					})
			}
