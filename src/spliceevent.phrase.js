...
export default function(evt) {
	...
	if(evt.isaltexon || evt.isskipexon) {
		...
		htmls.push('<div style="display:inline-block">'+(exonstart==exonstop ? 'exon '+(exonstart+1) : 'exons '+(exonstart+1)+'-'+(exonstop+1))+' '
			+(evt.isaltexon ? 'alternative usage' : 'skipping')
			+'</div>')

		if(evt.isaltexon) {
			htmls.push('<div class=sja_tinylogo_body>'+evt.gmB.isoform+', '+evt.gmA.isoform+'</div><div class=sja_tinylogo_head>ISOFORMS</div>')
		} else {
		...
		if(evt.junctionB.data) {
			htmls.push('<div class=sja_tinylogo_body>'+evt.junctionB.data.length+'</div><div class=sja_tinylogo_head>SAMPLE'+(evt.junctionB.data.length>1?'S':'')+'</div>')
		}
		// percentage
		...
		if(evt.framenocheck) {
			...
			htmls.push('<div class=sja_tinylogo_body style="background-color:'+client.colorinframe+';color:white">IN</div><div class=sja_tinylogo_head>FRAME</div>')
