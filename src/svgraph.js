...
fetch( new Request( hostURL+'/isoformlst', {
	method:'POST',
...
function validate() {
	...
	*/
	...
	for(let i=1; i<pairlst.length; i++) {
		...
		if(curr.chr!=prev.chr) return err('Error: mismatched chromosome at multi-seg breakpoint '+i+': '+curr.chr+' != '+prev.chr)
		...
		if(curr.gm) {
			if(prev.gm) {
				if(curr.gm.isoform != prev.gm.isoform) {
					return err('Error: mismatched gene in two breakpoints: '+prev.gm.symbol+' '+prev.gm.isoform+', '+curr.gm.symbol+' '+curr.gm.isoform)
				}
			} else {
				prev.gm=curr.gm
				err('In multi-segment at '+i+', 5\' breakpoint is joined to the same gene model of 3\'')
				const t=coord.genomic2gm(prev.position,curr.gm)
			...
			if(prev.gm) {
				...
				const t=coord.genomic2gm(curr.position,prev.gm)
	...
	waitdiv
		.style('margin','5px')
		.text('Loading protein domains ...')

	fetch( new Request(hostURL+'/pdomain', {
		method:'POST',
...
for(const scf of scaffold) {
	if(scf.gm) {
		scf.aatotal=(scf.aalen ? scf.aalen : scf.bplen/3)
			+(
...
let maxchrpxwidth=this.width*.7
for(const scf of scaffold) {
	if(scf.gm) {
	...
	for(const n of scf.nodes) {
		...
		if(n.atupstream) {
			...
			n.aaoff=(scf.upstream? scf.upstream/3 :0)
				+n.atutr5.off/3
		} else if(n.atutr3) {
			n.aaoff=(scf.upstream ? scf.upstream/3 :0)
...
const scfg=g0.selectAll()
	.data(scaffold)
	.enter().append('g')
const link=g0.selectAll()
	.data(data.links)
	.enter().append('line')
	.attr('stroke','black')
	.attr('stroke-dasharray','3,2')
// component - node
const node=g0.selectAll()
	.data(data.nodes)
	.enter().append('g')
const nodelabel=node.filter(d=> d.label)
...
const nodelabel2=node.filter(d=> d.antisense)
	.append('text')
	.text('antisense')
	.attr('font-family',client.font)
	.attr('font-size',nodefontsize)
...
chrbar.append('rect')
	...
	.attr('clip-path',d=> d.clipid)
// set9
const chrbarline1=chrbar.append('line')
	.attr('clip-path',d=> d.clipid)
	.attr('stroke','black')
const chrbarline2=chrbar.append('line')
	.attr('clip-path',d=> d.clipid)
	.attr('stroke','black')
	/*
...
// set10
const pupstream=protein.filter(d=> d.upstream)
	.append('line')
	.attr('clip-path',d=> d.clipid)
	.attr('stroke','#545454')
	.attr('stroke-dasharray','2,1')
// set12
const p5utr=protein.filter(d=> d.utr5bp)
	.append('line')
	.attr('clip-path',d=> d.clipid)
	.attr('stroke','#858585')
// set13
const p3utr=protein.filter(d=> d.utr3bp)
	.append('line')
	.attr('clip-path',d=> d.clipid)
	.attr('stroke','#858585')
// set11
const pdownstream=protein.filter(d=> d.downstream)
	.append('line')
	.attr('clip-path',d=> d.clipid)
	.attr('stroke','#545454')
	.attr('stroke-dasharray','2,1')
// set14
const pbg=protein.append('rect') // white bar for background
	.attr('clip-path',d=> d.clipid)
	.attr('fill','white')
// set15
const allpdomain=protein.filter(d=> d.gm.pdomains)
	.selectAll()
	...
	.attr('fill',d=> d.color)
// set15exonboundary
const exonboundary=protein.filter(d=>d.gm.exon.length>1)
	.selectAll()
	.data((d)=>{
		// uses rna bp length
		let cum=0
...
// set16
const pbox=protein.append('rect')
	.attr('clip-path',d=> d.clipid)
...
// set17
const scflabel=scfg.append('text')
	.text(d=> d.name )
	.attr('font-family',client.font)
	.attr('fill','black')
...
// set18
const scflabel2=scfg.filter(d=> !d.nontemplate)
	.append('text')
	.attr('fill','#858585')
...
// set19
const scflabel3=scfg.filter(d=> d.nodes[0].antisense)
	.append('text').text('antisense')
	.attr('fill',client.colorantisense)
	// toggled visibility
	.attr('fill-opacity',0)
	.attr('font-size',nodefontsize)
...
// set20
const scfcliprect=scfg.append('defs')
	.append('clipPath')
	.attr('id',d=> d.clipid0)
	.append('rect')
	.attr('x',0)
	.attr('y',0)
// set21
const scfkick=scfg.append('rect')
	.attr('clip-path',d=> d.clipid)
	.attr('fill','white')
	.attr('fill-opacity',0)
	.on('mousemove',d=>{
		...
		for(let i=0; i<domainlst.length; i++) {
			...
			dg.append('text').text(dm.description)
		...
		svg.attr('height',height-bottompad
			+Math.max(bottompad,pad7+(descfontsize+vsp)*(domainlst.length+1))
			)
	})
	.on('mouseout',()=>{
		gsays.selectAll('*').remove()
...
const setpxsize=()=>{
	// set1
	for(const s of scaffold) {
		s.idealy=connheight*s.id
			+(pad4+regionthick)*2*s.id
	}
	// set2
	height=toppad+bottompad
		...
		+(pad4*2+regionthick*2+connheight)*(scaffold.length-1)
	simulation
		.force('x',d3force.forceX(this.width/2))
		.force('y',d3force.forceY(height/2))
	// set3
	maxgmpxwidth=this.width
	maxchrpxwidth=this.width*.7
	for(const scf of scaffold) {
		if(scf.gm) {
			scf.pxwidth=maxgmpxwidth*scf.aatotal/gmmaxaa
		} else if(scf.nontemplate) {
	...
	.attr('stroke-width',proteinheight/5)
	// set12
	p5utr.attr('x1',d=> d.upstream ? d.scale(d.upstream/3) : 0 )
	.attr('x2',d=> d.scale(((d.upstream?d.upstream:0)+d.utr5bp)/3) )
	.attr('y1',proteinheight/2)
	.attr('y2',proteinheight/2)
	.attr('stroke-width',Math.floor(proteinheight/2)+.5)
	// set13
	p3utr.attr('x1',d=>{
		return d.scale(
			(d.upstream ? d.upstream/3 : 0)
	...
	.attr('height',proteinheight)
	// set15exonboundary
	exonboundary.attr('x1',(boundary)=>{
		const scf= boundary.scf
		boundary.x=scf.scale(
			(scf.upstream ? scf.upstream/3 : 0)
			+boundary.rnabp/3
			)
		return boundary.x
	...
	// set18
	scflabel2
		.attr('y',d=> d.id%2==0 ? -pad6*2-scffontsize-pad6 : proteinheight+pad6*2+scffontsize+pad6)
	// set19
	scflabel3
		.attr('y',d=> d.id%2==0 ? -pad6*2-scffontsize-pad6-nodefontsize-pad6 : proteinheight+pad6*2+scffontsize+pad6+nodefontsize+pad6)
	// set20
	scfcliprect
		.attr('height',proteinheight)
...
simulation.on('tick',()=>{
	...
	scfg.attr('transform',scf=>
		'translate('
			+(scf.nodes[0].x-scf.nodes[0].xoff)
			+','
			+scf.nodes[0].y
			+')'
	)
...
const tochimeric=(slow)=>{
	...
	// position
	scfcliprect.transition().duration(slow ? dur : 0)
	.attr('x',d=>{
		const n0=d.nodes[0]
		if(d.nodes.length==1) {
	...
	scflabel.attr('text-anchor','middle')
	...
	scftick.attr('stroke-opacity',1)
		.attr('x1',d=> d.chimericmidx)
		.attr('x2',d=> d.chimericmidx)
	scflabel2.attr('fill-opacity',1)
		.attr('x',d=> d.chimericmidx )
	scflabel3.attr('fill-opacity',1)
		.attr('x',d=> d.chimericmidx )
	// all on same height
	const yset=(height-toppad*2)/2-proteinheight/2
	scfg.transition().duration(slow ? dur : 0)
	.attr('transform',d=>{
	...
	node.transition().duration(slow ? dur : 0)
	.attr('transform',d=> 'translate('+d.x+','+d.y+')')
	link.transition().duration(slow ? dur : 0)
	.attr('x1',d=>{
	...
	svg.transition().duration(slow ? dur : 0)
		.attr('width',leftpad+w2+rightpad)
	drag.transition().duration(slow ? dur : 0)
		.attr('x',leftpad+w2+rightpad-dragwidth)
	// end of tochimeric
}
let chimeric=false
buttdiv.append('button').text('Toggle chimeric view').on('click',()=>{
	chimeric=!chimeric
	if(chimeric) {
		simulation.stop()
		tochimeric(true)
	...
	scfcliprect.transition().duration(dur)
		.attr('x',0)
		.attr('width',d=>d.pxwidth+pad3+d.labelwidth)
	scflabel.attr('text-anchor', 'start')
		.attr('dominant-baseline','central')
		.attr('x',d=> d.pxwidth+pad3)
		.attr('y', proteinheight/2)
...
if(dgmlst.length) {
	let pdmshown=false
	buttdiv.append('button').text('Protein domains').on('click',()=>{
		if(!pdmshown) {
			pdmshown=true
			for(const scf of dgmlst) {
				const d=domaindiv.append('div')
				...
				for(const e of lst) {
					...
					d2.append('div')
						...
						.on('click',()=>{
							...
							allpdomain.filter(d=>d.__scf.clipid==scf.clipid && d.name+d.description==e.key)
							.transition()
							.attr('height',()=>{
					...
					d2.append('span').text(e.name).style('margin-right','5px')
...
buttdiv.append('button').text('SVG').on('click',()=>{
	...
	svg.attr('width',w+bbl.width)
		.attr('height',h+bbl.height+10)
...
function nodecheck(n) {
	if(n.gm) {
		...
		const t=coord.genomic2gm(n.position,n.gm)
		n.exonbp=t.rnapos
...
function copynode(n,scaffold,data){
	...
	if(!a.gm) {
		...
		a.label=(a.exon ? 'exon '+a.exon+', ' : '')+
			(a.codon ? a.codon+' AA' : a.exonbp+' bp')
	}
	if(a.strand && a.gm) {
