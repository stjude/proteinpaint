...
function setRenderers(self) {
	...
	function renderAxes(xAxis, xTitle, yAxis, yTitle, s, d) {
		...
		xText.append('title').text('test')

		const yTitleLabel = 'Score'
		...
		yText.append('title').text(yTitleLabel)
	}
}
	...
	const pj = new Partjson({
		...
		'=': {
			...
			yScale(row, context) {
				...
				const domain = s.scale == 'byChart' ? [yMax, context.root.yMin] : [context.root.yMax, context.self.yMin]
				return d3Linear()
					.domain(domain)
					.range([0, s.svgh - s.svgPadding.top - s.svgPadding.bottom])
			}
