...
import {exoncolor, IN_frame, OUT_frame, spliceeventchangegmexon} from './common'
...
export function spliceeventanalyze(tk,block) {
	...
	*/
	...
	if(gmdataerr) {
		const fontsize=12
		const pad=2
		tk.eventsg.append('rect')
			.attr('width',block.width)
			.attr('height',pad*2+fontsize)
			.attr('fill','#f1f1f1')
		tk.eventsg.append('text')
			.text(gmdataerr)
	...
	const exonskipalt=findexonskipping(
		tk.totalsamplecount>1,
		tk.data,
		gmdata
		)
...
for(const eventset of allevents) {
	...
	eventset.toplabel=eventset.g.append('text')
		...
		.attr('dominant-baseline','central')
	if(tk.totalsamplecount>1) {
		eventset.middlelabel=eventset.g.append('text')
	...
	*/
	...
	eventset.bottomlabel=eventset.g.append('text')
		.text(eventset.bottomlabelsays
			+(willshowframe ?
	...
	for(const exon of eventset.exonlst) {
		...
		*/
		...
		exonobj.rect=eventset.g.append('rect')
			.attr('fill', eventset.color)
	...
	if(eventset.exonlst.length>1) {
		...
		.attr('y1',stackheight/2-.5)
		.attr('y2',stackheight/2-.5)
	}
	...
	if(eventset.labelwidth > eventset.boxstart) {
		...
		eventset.toplabel
			...
			.attr('x',-boxlabelspacing)
		eventset.bottomlabel
			.attr('text-anchor','end')
			.attr('x',-boxlabelspacing)
		if(eventset.middlelabel) {
			eventset.middlelabel
				.attr('text-anchor','end')
				.attr('x',-boxlabelspacing)
		}
	}
	// mouseover cover
	eventset.cover=eventset.g.append('rect')
		...
		.attr('fill-opacity',0)
		.on('mouseover',()=>{
			eventset.cover.attr('fill-opacity',.1)
		})
		.on('mouseout',()=>{
...
for(const eventset of allevents) {
	...
	eventset.g.attr('transform','translate('+eventset.boxstart+','+(eventset.stackidx*(stackheight+stackspace))+')')
}
...
function checkjunctionframe(arg) {
	...
	*/
	...
	d3json(arg.block.hostURL+'/translategm').post(JSON.stringify({ genome:arg.block.genome.name, gm:gm2, jwt:arg.block.jwt }), data=>{
		if(!data) {
