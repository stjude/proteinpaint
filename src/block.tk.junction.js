...
for(const t of tk.tracks) {
	...
	const getheader = new Promise((resolve,reject)=>{
		...
		*/
		...
		d3json(block.hostURL+'/vcf').post(JSON.stringify(par),data=>{
			...
			*/
			if(!data.metastr) {
				...
				*/
				...
				const l=data.metastr.trim().split('\t')
				t.headersamples=[]
	...
	const getdata = new Promise((resolve,reject)=>{
		getheader.then(()=>{
			...
			d3json(block.hostURL+'/junction').post(JSON.stringify(par),data=>{
				donenum++
...
function label_mcount_fillpane(tk,block,tip) {
	...
	if(types.size) {
		...
		for(const [type,valuelst] of types) {
			d.append('div').style('margin','2px').html(valuelst.length+' <span style="font-size:.8em;color:'+tk.categories[type].color+'">'+type+' junction'+(valuelst.length>1?'s':'')+'</span>')
			...
			if(vlst.length==1) {
				...
				.html(
					'<tr><td style="text-align:right;">'+Math.max(...vlst)+'</td><td style="color:#858585;font-size:.7em">max read count</td></tr>'+
					'<tr><td style="text-align:right;">'+Math.min(...vlst)+'</td><td style="color:#858585;font-size:.7em">min</td></tr>'+
					'<tr><td style="text-align:right;">'+Math.ceil(vlst.reduce((i,j)=>j+i,0)/vlst.length)+'</td><td style="color:#858585;font-size:.7em">mean</td></tr>'
				)
			}
		}
	} else {
	...
	*/
	...
	for(const tmp of samplesortlst) {
		...
		if(sampledata.readcountlst) {
			...
			td3.html(sampledata.readcountlst.length+' <span style="font-size:.8em">junction'+(sampledata.readcountlst>1?'s':'')+', max: '+max+'</span>')
