...
export function svmrparseinput(arg, sayerror, genome, holder, hostURL, jwt) {
	...
	arg.urls.forEach(url=>{
		tasks.push(
			fetch( new Request(hostURL+'/urltextfile',{
				method:'POST',
...
export function svmrui( genomes, hostURL, jwt ) {
	const [pane,inputdiv,gselect,filediv,saydiv,visualdiv]=client.newpane3(100, 100, genomes)
	pane.header.text('Fusion editor')
	pane.body.style('margin','10px')
	inputdiv.append('div').style('margin-top','20px').html('<p>Please upload CICERO output as a text file. '
	...
	inputdiv.append('p').html('<a href=https://www.dropbox.com/s/6b6x2iwmh3p5yf4/CICERO-output.txt?dl=0 target=_blank>Example file</a>')
	function cmt(t,red){
...
export function svmrparseraw(raw, genome) {
	...
	for(let i=1; i<lines.length; i++) {
		...
		if(m.type2) {
			...
			case 'closs': m.iscloss=true;break
			case 'nloss': m.isnloss=true;break
			case 'fusion': m.isfusion=true;break
			case 'itd': m.isitd=true;break
			case 'other': m.isother=true;break
			case 'uptss': m.isuptss=true;break
			default:
