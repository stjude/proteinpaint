	...
	if(arg.pane) {
		...
		if(mlst.length==1) {
			...
			case common.dtsnvindel:
				pane.header.html(
					'<span style="font-weight:bold;color:'+c.color+'">'+
					(m.mname ? m.mname : (m.pos ? m.chr+':'+(m.pos+1) : ''))
					+'</span> <span style="font-size:80%">'+c.label+'</span>')
				break
			case common.dtsv:
		...
		if(sampleset.size>0) {
			...
			for(const m of mlst) {
				...
				if(m.dt==common.dtfusionrna || m.dt==common.dtsv) {
					...
					thisnote= (m.mname ? m.mname+' ' : '')+classlab
						+(block.usegm ? ' in '+block.usegm.name : '')
				}
...
if(tk.ds && tk.ds.id2vcf) {
	...
	for(const m of mlst) {
		...
		if(m.mname) {
			d.append('span').html(m.mname+'\t<span style="font-size:80%;color:#858585">'+common.mclass[m.class].label+'</span>')
		} else if(m.csq) {
			d.append('span').html(m.csq[0]._mname+'\t<span style="font-size:80%;color:#858585">'+m.csq[0].Consequence+'</span>')
		}
		d.append('span').html('&nbsp;&nbsp;'+m.chr+':'+(m.pos+1)+' <span style="font-size:70%">REF</span> '+m.ref+' <span style="font-size:70%">ALT</span> '+m.alt)
	}
	return
}
...
if(mlst.length==1) {
	...
	if(block.variantPageCall_snv) {
		if(m.chr!=undefined && m.pos!=undefined && m.ref!=undefined && m.alt!=undefined) {
			variantpage.set.set(
				m.chr+separatorvp+m.pos+separatorvp+m.ref+separatorvp+m.alt,
				{mname:m.mname, class:m.class}
			)
		}
	}
	const data=[]
	for(const atr of tk.snvindelattr) {
		if(atr.lst) {
			...
			for(const at of atr.lst) {
				if(at.ismaf) {
					...
					kvlst.push({
						k:at.label,
						v:(v ?
							(Number.isFinite(v.f) ? '<span style="color:'+at.fill+'">'+(v.f*100).toFixed(0)+'% ('+v.v1+'/'+v.v2+')</span>' : 'n/a')
							: '')
					})
				} else {
			...
			data.push({
				k:atr.label,
				v:(v ?
					(Number.isFinite(v.f) ? '<span style="color:'+atr.fill+'">'+(v.f*100).toFixed(0)+'% ('+v.v1+'/'+v.v2+')</span>' : 'n/a')
					: '')
			})
		} else {
			let v=atr.get(m)
	...
	if(tk.ds && tk.ds.snvindel_legend) {
		...
		const legenddiv=buttonrow.append('div')
			.style('display','none')
			.style('margin','10px')
			.style('width','400px')
	...
	let h_col,
		...
		h_exp,
		h_leg
	buttrow.append('button').text('Table columns').on('click',function(){
	...
	if(hasSNP && snpfind.bprange.length) {
		snpfind.button=buttrow.append('button').text('loading...').on('click',function(){
	...
	if(tk.ds && tk.ds.snvindel_legend) {
		buttrow.append('button').text('Legend').on('click',function(){
	...
	for(const at of tk.snvindelattr) {
		...
		const check=h_col.append('input')
			.attr('type','checkbox')
			.attr('id',id)
...
if(block.variantPageCall_snv) {
	...
	if(vlst.length==1) {
		...
		.text('Variant Page')
		.on('click',()=>{
			const div=client.menuunderdom(d3event.target)
...
for(let i=0; i<showcount; i++) {
	...
	tr.append('td').style('font-size','.8em').text(i+1)
	for(const atr of attrlst) {
		if(atr.hide) continue
		if(atr.lst) {
			for(const at of atr.lst) {
			...
			if(atr.tablecellwidth) {
				td.style('width',atr.tablecellwidth)
...
function vcfmdetail(m, vcfobj, holder, tk, block) {
	...
	*/
	...
	row1.append('span').text(m.chr+':'+(m.pos+1)).style('padding-right','10px')
	...
	if( mayshowgenotype2boxplot( m, tk, div)) {
		...
		.then(()=>{
			...
			if( m.sampledata.length==0 && tk.ds.vcfcohorttrack) {
				...
				return query_vcfcohorttrack(m, tk, block)
				.then(m2=>{
					if(!m2) {
						// no hit
						return
	...
	if(vcfobj.infopipejoin) {
		...
		if(lst0.length) {
			...
			for(let i=0; i<lst0[0].length; i++) {
				...
				client.make_table_2col(row2,lst)
...

	{
		...
		for(const k in m.altinfo) {
			...
			lst.push({
				k:k,
				v: showvalue.join(', ') + (vcfobj.info && vcfobj.info[k] ? ' <span style="font-size:70%;color:#858585">'+vcfobj.info[k].Description+'</span>' : '')
			})
		}
		if(lst.length) {
...
function mayephl_butt(ep,holder,mlst) {
	...
	if(notfound) {
		holder.append('button').text(ep.p.sampletype+' not in '+ep.p.name).attr('disabled',1)
		return
	}
	let hl=false
	holder.append('button').text('Highlight in '+ep.p.name)
	.on('click',()=>{
		hl=!hl
		ep.may_hl(mlst,hl)
	})
...
function table_sv(mlst,holder,tk,block) {
	...
	table_sort(mlst,table,tk.svattr,tk,
		m=>{
	...
	if(tk.ds.cohort && tk.ds.cohort.levels) {
		for(const l of tk.ds.cohort.levels) {
			tk.snvindelattr.push({
				...
				hide:l.hide,
				get:m=>{
					if(m[l.k]) return m[l.k]+(l.full ? (m[l.full] ? ' <span style="font-size:.8em;color:#858585">'+m[l.full]+'</span>':'') : '')
					return ''
				}
			})
		}
...
export function runtimeattr_trunc(tk,mlst) {
	const skipset=new Set(['__x','mname','gene','strand','isoform','aapos','rnapos','chr','pos','class','dt','rnaposition','partner'])
	tk.truncattr=[]
	for(const m of mlst) {
		if(m.sample) {
	...
	if(tk.ds && tk.ds.cohort && tk.ds.cohort.levels) {
		for(const l of tk.ds.cohort.levels) {
			...
			tk.truncattr.push({
				label:(l.label || l.k),
				//hide:l.hide,
				get:m=>m[l.k] ? m[l.k]+(l.full ? ' <span style="color:#858585;font-size:.8em">'+m[l.full]+'</span>' : '') : ''
			})
		}
	}
	for(const k in mlst[0]) {
...
export function runtimeattr_del(tk,mlst) {
	const skipset=new Set(['__x','mname','isoform','aapos','rnapos','chr','pos','class','dt','rnaposition','rnadellength','a','b'])
	tk.delattr=[]
	for(const m of mlst) {
		if(m.sample) {
...
export function runtimeattr_itd(tk,mlst) {
	const skipset=new Set(['__x','mname','isoform','aapos','rnapos','chr','pos','class','dt','rnaposition','rnaduplength','a','b'])
	tk.itdattr=[]
	for(const m of mlst) {
		if(m.sample) {
...
export function runtimeattr_sv(tk,mlst) {
	const skipset=new Set(['__x','mname','aapos','chr','class','dt','inframe','origin','pos','rnapos','strand','useNterm','pairlst'])
	tk.svattr=[]
	for(const m of mlst) {
		if(m.sample) {
	...
	tk.svattr.push({
		...
		get:m=>{
			...
			const lst= m.pairlst.map(p=>
				(p.a.name ? '<strong>'+p.a.name+'</strong> ' : '')
				+(p.a.chr ? 
					'<span style="color:#858585">'+p.a.chr+':'+(p.a.position+1)
					+' '+(p.a.strand=='+'?'forward':'reverse')
					+'</span> &#10140; '
					: '')
				+(p.b.name ? '<strong>'+p.b.name+'</strong> ' : '')
				+(p.b.chr ? 
					'<span style="color:#858585">'+p.b.chr+':'+(p.b.position+1)
		...
		if(m.pairlst[0].a.rnaposition!=undefined) {
			tk.svattr.push({
				...
				get:m=>{
					...
					const lst= m.pairlst.map(p=>
						(p.a.name ? '<strong>'+p.a.name+'</strong> ': '')
						+'<span style="color:#858585">r.'+(p.a.rnaposition+1)+'</span> &#10140; '
						+(p.b.name ? '<strong>'+p.b.name+'</strong> ': '')
						+'<span style="color:#858585">r.'+(p.b.rnaposition+1)+'</span>'
					)
					return lst.join('<br>')
				}
			})
	...
	if(mlst[0].pairlst[0].a.ratio!=undefined) {
		tk.svattr.push({
			...
			get:m=>{
				const lst=[]
				const w=40,h=12,fill='#FF850A',fillbg='#FFCF9E'
				for(const i of m.pairlst) {
					lst.push(
						(i.a.name ? i.a.name : i.a.chr)
						+' <svg width='+w+' height='+h+'><g><title>'+i.a.ratio+'</title><rect width='+w+' height='+h+' fill="'+fillbg+'"></rect>'
						+'<rect width='+(w*i.a.ratio)+' height='+h+' fill="'+fill+'"></rect>'
						+'</g></svg>'
						+'&nbsp;&nbsp;'
						+(i.b.name ? i.b.name : i.b.chr)
						+' <svg width='+w+' height='+h+'><g><title>'+i.b.ratio+'</title><rect width='+w+' height='+h+' fill="'+fillbg+'"></rect>'
						+'<rect width='+(w*i.b.ratio)+' height='+h+' fill="'+fill+'"></rect>'
						+'</g></svg>'
					)
				}
				return lst.join('<br>')
	...
	tk.svattr.push({
		...
		get:m=>{
			function label(f){
				if(f==0) return '<span style="white-space:nowrap;background-color:'+client.coloroutframe+';font-size:80%;color:white;padding:1px 3px">OUT of frame</span>'
				if(f==1) return '<span style="white-space:nowrap;background-color:'+client.colorinframe+';font-size:80%;color:white;padding:1px 3px">IN frame</span>'
				if(f==2) return '<span style="white-space:nowrap;background-color:'+client.colorinframe+';font-size:80%;color:white;padding:1px 3px">IN frame</span> <span style="font-size:80%;color:#858585">promoter fusion</span>'
				if(f==3) return '<span style="white-space:nowrap;background-color:'+client.colorinframe+';font-size:80%;color:white;padding:1px 3px">IN frame</span> <span style="font-size:80%;color:#858585">alternative promoter</span>'
				return 'err ('+f+')'
			}
			const lst=[]
			for(const p of m.pairlst) {
	...
	*/
	...
	.on('click',()=>{
		...
		for(const s of samplelst) {
			...
			if(hasgt) {
				tr.append('td').style('padding-left','10px').text(s.DP || '')
			}
			if(hascount) {
				const lst=[]
...
function vcfvepbutton(csqlst, holder, tk, headers) {
	...
	.on('click',()=>{
		...
		for(const item of csqlst) {
			...
			const box=tk.tktip.d.append('div')
				.attr('class','sja_menuoption')
				.style('margin','5px')
				.html(thislabel)
				.on('click',()=>{
				...
				if(k4a) {
					...
					if(n) {
						if(tk.ds.cohort.levels) {
							for(const l of tk.ds.cohort.levels) {
								if(!n[l.k]) continue
								ss.sampleobj[ l.label || l.k ] = n[l.k]+( (l.full && n[l.full]) ? ' <span style="font-size:.8em;color:#858585">'+n[l.full]+'</span>' : '')
							}
							// also add k4a
...
function make_url4variant(holder, m, items) {
	for(const item of items) {
		...
		holder.append('a')
			...
			.attr('target','_blank')
			.text(
				item.makelabel ? item.makelabel(m) : 
				(item.label || 'link')
			)
			.style('margin-right','10px')
	}
				...
				if(na) {
					...
					if(tk.ds.cohort.levels) {
						for(const l of tk.ds.cohort.levels) {
							...
							lst.push({
								k: (l.label || l.k),
								v: na[l.k]+ ((l.full && na[l.full]) ? ' <span style="font-size:.8em;color:#858585">'+na[l.full]+'</span>' : '')
							})
						}
					} else {
