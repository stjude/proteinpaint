...
import {bplen as bplength} from './common'
...
if(!holder) {
	...
	pane.header.html('<span style="color:#858585;font-size:70%">Fusion Editor</span> '+filename)
	holder=pane.body
}
this.holder=holder
this.errdiv=holder.append('div').style('width','500px').style('margin','10px')
const butrow=holder.append('div')
	.style('margin','20px')
	.style('padding','0px')
...
butrow.append('button').style('margin-right','20px').text('Export data').on('click',()=>{
	...
	const d0=menu.append('div')
		.style('padding','10px')
		.style('border','solid 1px #ccc')
	const table=d0.append('table')
	...
	tr=table.append('tr')
	tr.append('td').text('Major')
		.style('color','#858585').style('text-align','right')
	tr.append('td').text(single_hq)
	tr.append('td').text(multi_hq)
	tr.append('td').text(itd_hq)
	tr.append('td').text(trunc_hq)
	tr=table.append('tr')
	tr.append('td').text('not Major')
		.style('color','#858585').style('text-align','right')
	tr.append('td').text(single_nhq)
	tr.append('td').text(multi_nhq)
	tr.append('td').text(itd_nhq)
	tr.append('td').text(trunc_nhq)
	let dd=d0.append('div').style('margin','10px')
		.text('Export fusions labeled as "Major"')
	dd.append('button')
		.style('margin','10px')
		.text('Tabular format')
		.on('click',()=> unload.svmr_export_text(this,true) )
	dd.append('button').style('margin','10px')
		.text('JSON format').on('click',()=> unload.svmr_export_json(this,true) )
	dd.append('button').style('margin','10px')
		.text('View in ProteinPaint').on('click',()=> unload.svmr_2pp(this,true) )
	dd=d0.append('div').style('margin','10px').text('Export all fusions')
	dd.append('button').style('margin','10px')
		.text('Tabular format').on('click',()=> unload.svmr_export_text(this,false) )
	dd.append('button').style('margin','10px')
		.text('JSON format').on('click',()=> unload.svmr_export_json(this,false) )
	dd.append('button').style('margin','10px')
		.text('View in ProteinPaint').on('click',()=> unload.svmr_2pp(this,false) )
})
...
// expression
this.expression.div=holder.append('div')
	.style('display','none')
	.style('margin','20px')
	.style('padding','20px')
...
this.expression.prediv.append('div').style('margin','5px 5px 10px 5px').style('font-size','80%')
	.text('The first 3 columns of the file should be: 1) gene name, 2) expression value, 3) sample name')
this.expression.input=this.expression.prediv.append('input').attr('type','file').on('change',()=>{
...
let d0=genediv.append('div').style('display','inline-block')
	.style('border','dashed 1px #bbb')
this.genefilter=d0.append('div').style('background-color','#ededed').style('padding','10px 20px')
let d01=d0.append('div')
	.style('padding','10px 20px').style('overflow-y','scroll').style('resize','vertical').style('height','300px')
...
d0=cutoffdiv.append('div')
	.style('display','inline-block').style('padding','20px').style('border','solid 1px #ededed')
...
d01.append('input').attr('size',3).property('value',this.cf_reads).on('change',()=>{
...
d01.append('input').attr('size',3).property('value',this.cf_repeat).on('change',()=>{
...
d01.append('input').attr('size',3).property('value',this.cf_match).on('change',()=>{
	const v=Number.parseInt(d3event.target.value)
	if(Number.isNaN(v)) {
		return
	}
...
d01.append('input').attr('size',3).property('value',this.cf_ratio).on('change',()=>{
	const v=Number.parseFloat(d3event.target.value)
	if(Number.isNaN(v)) {
		return
	}
...
legenddiv.append('div')
	.style('display','inline-block').style('padding','10px').style('border','solid 1px #ededed')
	.html(
	'<table style="margin:20px">'
	+'<tr><td><div style="display:inline-block;font-size:80%;color:white;background-color:'+client.colorinframe+';padding:2px 5px">IN</div></td><td>In-frame fusion</td></tr>'
	+'<tr><td><div style="display:inline-block;font-size:80%;color:white;background-color:'+client.coloroutframe+';padding:2px 5px">O</div></td><td>Out-of-frame fusion</td></tr>'
	...
	+'</tr>'
	+'<tr><td><div style="display:inline-block;border:solid 1px black;padding:2px 10px;font-size:70%;font-weight:bold;color:'+knownprod_c+'">geneA</div>'
		+'-'
		+'<div style="display:inline-block;border:solid 1px black;padding:2px 10px;font-size:70%;font-weight:bold;color:'+knownprod_c+'">geneB</div>'
		+'</td><td>A known fusion product</td>'
	+'</tr>'
	+'</table>'
	+'<table style="margin:20px">'
	+'<tr><td><div style="width:40px;height:16px;position:relative;">'
		+'<div style="position:absolute;right:0px;top:0px;width:20px;height:16px;background-color:'+client.colorbgleft+'"></div>'
	...
	+'<tr><td><div style="width:40px;height:16px;position:relative;">'
		+'<div style="position:absolute;left:0px;top:0px;width:20px;height:16px;background-color:'+client.colorbgright+'"></div>'
		+'<div style="position:absolute;border:solid 1px black;width:100%;height:15px"></div>'
		+'</div>'
		+'</td>'
...
step_isoform(items) {
	...
	.then(data=>{
		...
		if(isoformErr.length) {
			this.err(isoformErr.length+' invalid isoform'+(isoformErr.length>1?'s':'')+': '+isoformErr.join(', '))
		}
		this.step_eat(items)
	})
	.catch(err=>{
...
step_eat(items) {
	...
	for(const sample of this.samples) {
		...
		if(hqin.length) {
			sample.egglst.push({
				htmlab:'HQ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="background-color:'+client.colorinframe+';color:white;padding:1px 3px;font-size:80%;">in-frame&nbsp;&nbsp;</span>',
				lst:hqin
			})
		...
		if(lqin.length) {
			sample.egglst.push({
				htmlab:'LQ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="background-color:'+client.colorinframe+';color:white;padding:1px 3px;font-size:80%;">in-frame&nbsp;&nbsp;</span>',
				lst:lqin
			})
		...
		if(rtin.length) {
			sample.egglst.push({
				htmlab:'Read-through <span style="background-color:'+client.colorinframe+';color:white;padding:1px 3px;font-size:80%;">in-frame&nbsp;&nbsp;</span>',
				lst:rtin
			})
		}
		if(rtt.length) {
		...
		if(badin.length) {
			sample.egglst.push({
				htmlab:'Bad&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="background-color:'+client.colorinframe+';color:white;padding:1px 3px;font-size:80%;">in-frame</span>',
				lst:badin
			})
		}
		if(badt.length) {
...
dogenefilter(){
	...
	if(va.length+vb.length==0) {
		this.gui.says.text('Showing '+(this.genelst.length>100?100:'all')+' of '+this.genelst.length+' pairs')
...
geneshow(lst){
	...
	for(const evt of lst) {
			...
			for(const sample of evt.samples) {
				...
				for(const prod of sample.prodlst) {
					...
					.on('mouseover',()=>{
						...
						this.showsvpairs({
							prodlst:[prod],
							holder:menu.append('div').style('padding','10px').style('border','solid 1px black'),
							nodetail:true,
							sample:sample,
							eglst:null,
		...
		for(const smp of ['HQ','LQ','RT','bad']) {
			if(hash[smp]) {
				lst.push('<span style="border-radius:6px;background-color:#ededed;padding:1px 6px;font-size:80%">'+smp+(hash[smp]>1?' <span style="font-size:80%">'+hash[smp]+'</span>':'')+'</span>')
			}
		}
...
showevents(sample,eglst,holder) {
	...
	for(const eg of eglst) {
		...
		if(eg.lst.length==1) {
			...
			graphheight+=vpad*2+(rowh+rows)*eg.lst.length
				+(rowh2+rows)*(eg.lst.length-1)
				+gvpad
		}
		for(const evt of eg.lst) {
			...
			svg.append('text').text(labA).attr('font-size',fontsize).attr('font-family','Courier')
				.each(function(){ geneAw=Math.max(geneAw,this.getBBox().width) })
				.remove()
			svg.append('text').text(labB).attr('font-size',fontsize).attr('font-family','Courier')
				.each(function(){ geneBw=Math.max(geneBw,this.getBBox().width) })
				.remove()
			// event logo
			svg.append('text').text(prod.rating).attr('font-size',fontsize).attr('font-family','Courier')
				.each(function(){ evt.svg.ratingw=this.getBBox().width })
				.remove()
			...
			svg.append('text').text(prod.featureA).attr('font-size',fontsizefeature).attr('font-family',client.font)
				.each(function(){ evt.svg.featurew=this.getBBox().width })
				.remove()
			svg.append('text').text(prod.featureB).attr('font-size',fontsizefeature).attr('font-family',client.font)
				.each(function(){ evt.svg.featurew=Math.max(evt.svg.featurew,this.getBBox().width) })
				.remove()
			svg.append('text').text(Math.floor(prod.score)).attr('font-size',fontsize).attr('font-family',client.font)
				.each(function(){ evt.svg.scorew=this.getBBox().width })
				.remove()
			...
			svg.append('text').text(evt.svg.recurtext).attr('font-size',fontsize-4).attr('font-family',client.font)
				.each(function(){ evt.svg.recurw=this.getBBox().width })
				.remove()
	...
	let roww=chrAw+s1
		+rowh+s7
		+geneAw+genesp+s2+genesp+geneBw
		+s7+rowh
		+s1+chrBw+s3
		+eventlogow
		+s6+recurw
	/****************************** render */
	svg.attr('width',hpad0*2+hpad*2+roww+ghandlew)
	...
	for(const eg of eglst) {
		...
		if(eg.lst.length>1) {
			...
			if(eg.ismsg) {
				...
				.on('click',()=>{
					...
					// edit msg
					div.append('button').text('Edit').on('click',()=>{
		...
		for(const evt of eg.lst) {
			...
			if(eg.lst.length>1 && evtid>0) {
				...
				const g_row=g_rows.append('g').attr('transform','translate('+(chrAw+s1+rowh+s7+geneAw+genesp+s2/2)+','+y1+')')
				const text=g_row.append('text')
					.attr('fill','#858585')
			...
			// chr a
			g_row.append('text').text(prod.chrA)
				.attr('x',chrAw)
				.attr('y',rowh/2)
			...
			if(prod.geneA) {
				if(prod.geneA in showngenenotip) {
					...
					if(lst.length>0) {
						extevt.a={lst:lst}
						// gene a tip
						extevt.a.circle=g_row.append('circle')
							.attr('fill','white')
							.attr('stroke','black')
							...
							.attr('r',rowh/2)
						if(lst.length>1) {
							extevt.a.text=g_row.append('text').text(lst.length)
			...
			// gene a label
			g_row.append('text').text(labA)
				.attr('x',chrAw+s1+rowh+s7+geneAw)
				.attr('y',rowh/2)
				.attr('font-size',fontsize)
				.attr('font-family','Courier')
				.attr('font-weight',prod.hlgene ? ((prod.hlgene==1||prod.hlgene==3||prod.hlgene==4) ? 'bold':'normal') : 'normal')
			...
			// gene b label
			g_row.append('text').text(labB)
				.attr('x',chrAw+s1+rowh+s7+geneAw+genesp+s2+genesp)
				.attr('y',rowh/2)
				.attr('font-size',fontsize)
				.attr('font-family','Courier')
				.attr('font-weight',prod.hlgene ? ((prod.hlgene==2||prod.hlgene==3||prod.hlgene==4) ? 'bold':'normal') : 'normal')
				.attr('fill',prod.hlgene ? (prod.hlgene==4 ? knownprod_c : '#545454'):'#545454')
				.attr('dominant-baseline','central')
			if(prod.geneB) {
				if(prod.geneB in showngenenotip) {
					...
					if(lst.length>0) {
						extevt.b={lst:lst}
						// gene b tip
						extevt.b.circle=g_row.append('circle')
							.attr('fill','white')
							.attr('stroke','black')
							...
							.attr('r',rowh/2)
						if(lst.length>1) {
							extevt.b.text=g_row.append('text').text(lst.length)
			...
			// chr b
			g_row.append('text').text(prod.chrB)
			...
			const x0=x
			const logobg=g_row.append('rect')
				.attr('fill','white')
				.attr('stroke','#858585')
				.attr('x',x)
				.attr('y',-1.5)
				...
				.attr('ry',5)
			x+=s5
			prod.hook.mainRating=g_row.append('text').text(prod.rating)
				.attr('font-size',fontsize)
				.attr('font-family',client.font)
				.attr('fill','#858585')
				.attr('x',x)
			...
			prod.hook.mainFrame={}
			prod.hook.mainFrame.bg=g_row.append('rect')
				.attr('x',x+1)
				.attr('y',3)
			...
			prod.hook.mainFrame.text=g_row.append('text').text(evt.svg.frameword)
				...
				.attr('dominant-baseline','middle')
			x+=evt.svg.framew+s5
			prod.hook.mainType=g_row.append('text').text(prod.type2)
				.attr('font-size',fontsize)
				.attr('font-family',client.font)
				.attr('fill','#858585')
				.attr('x',x)
				...
				.attr('dominant-baseline','middle')
			x+=evt.svg.typew+s5
			g_row.append('text').text(prod.featureA)
				...
				.attr('y',rowh/4)
				.attr('dominant-baseline','middle')
			g_row.append('text').text(prod.featureB)
				.attr('font-size',fontsizefeature)
				.attr('font-family',client.font)
				.attr('fill','black')
				.attr('x',x)
			...
			g_row.append('text').text(Math.floor(prod.score))
				.attr('font-size',fontsize)
				.attr('font-family',client.font)
				.attr('fill','#858585')
				.attr('x',x)
			...
			if(prodlst.length>1) {
				const logobg2=g_row.append('rect')
					.attr('fill','none')
					.attr('stroke','#858585')
					.attr('x',x)
					.attr('y',3.5)
					...
					.attr('rx',5)
					.attr('ry',5)
				g_row.append('text').text(prodlst.length-1)
					.attr('fill','black')
					.attr('font-size',fontsizefeature)
					.attr('font-family',client.font)
					.attr('x',x+etw/2)
				...
				.on('mouseover',()=>{
					...
					const menu=client.menushow(p.left,p.top+p.height+2)
					const table=menu.append('div').style('padding','10px').style('border','solid 1px #ccc')
						.append('table').style('border-spacing','10px').style('border-collapse','separate')
			...
			const text=g_row.append('text').text(evt.svg.recurtext)
				.attr('font-size',fontsize-4)
				.attr('font-family',client.font)
			...
			if(evt.svg.hasrecurrence) {
				text.attr('class','sja_svgtext2')
				.on('mouseover',()=>{
					const p=d3event.target.getBoundingClientRect()
					const menu=client.menushow(p.left+p.width+10,p.top-15)
					const slst=this.elab2sample[evt.label]
					const dd=menu.append('div').style('padding','10px').style('border','solid 1px #ccc')
			...
			.on('click',()=>{
				...
				const prod=evt.lst[0]
				pane3.header.html(
					'<span style="padding:2px 4px;background-color:'+client.colorbgleft+';">'+(prod.geneA ? prod.geneA : prod.chrA)+'</span>'
					+'<span style="padding:2px 4px;background-color:'+client.colorbgright+';">'+(prod.geneB ? prod.geneB : prod.chrB)+'</span>'
					)
				this.showsvpairs({
					prodlst:evt.lst,
					holder:pane3.body
...
extevt_mover(ext,dom,sample){
	...
	const menu=client.menushow(p.left,p.top+p.height+2)
	const d=menu.append('div').style('padding','10px').style('border','solid 1px #ccc')
	d.append('div').style('margin','10px').style('color','#aaa').text('Associated fusions from this sample:')
...
eventlogo(prodlst,holder) {
	...
	if(!prodlst || prodlst.length==0) {
		...
		const p=prodlst[0]
		d.append('div').style('display','inline-block').style('padding','2px 4px').style('border','solid 1px #858585').style('border-radius','5px')
		.html(p.rating+'&nbsp;'
			+'<span style="font-size:70%;vertical-align:2px;'
			+(p.usepair ?
				(p.usepair.inframe ? 'padding:2px 4px;background-color:'+client.colorinframe+';color:white;">IN'
			...
			+'</span>'
			+'&nbsp;<span style="color:#858585">'+p.type2+'</span>&nbsp;'
			+'<div style="display:inline-block;font-size:70%;line-height:.9">'
			+p.featureA+'<br>'+p.featureB
			+'</div>'
			+'&nbsp;'
			+Math.floor(p.score)
		)
...
prodstat(prod,holder) {
	...
	tr.append('td').style('padding','5px').style('background-color',prod.readsA<=this.cf_reads?alertcolor:'')
		.text(prod.readsA)
	tr.append('td').style('padding','5px').style('background-color',prod.readsB<=this.cf_reads?alertcolor:'')
		.text(prod.readsB)
	tr=table.append('tr')
...
showsvpairs(arg) {
	...
	for(const prod of arg.prodlst) {
		...
		if(arg.nodetail) {
			...
			const ratingsl=div.append('select').style('margin-right','5px')
				.on('change',()=>{
			...
			const framesl=div.append('select').style('margin-right','5px')
				.on('change',()=>{
					const sl=d3event.target
					...
					if(prod.hook.lessFrame) {
						prod.hook.lessFrame.html(inframe ? 
							'<span style="background-color:'+client.colorinframe+';color:white;padding:2px 3px;font-size:80%;white-space:nowrap">In frame</span>'
							: '<span style="background-color:'+client.coloroutframe+';color:white;padding:2px 3px;font-size:80%;white-space:nowrap">Out of frame</span>')
					}
				})
			const typesl=div.append('select').style('margin-right','5px')
				.on('change',()=>{
					const sl=d3event.target
			...
			const effectsl=div.append('select').style('margin-right','5px')
				.on('change',()=>{
					const sl=d3event.target
					const newv=sl.options[sl.selectedIndex].innerHTML
					prod.functioneffect=newv
				})
			div.append('button').text('Create group')
				.on('click',()=>{
					...
					dnew.append('button').style('margin','0px 10px').text('Apply').on('click',()=>{
		...
		if(!arg.nodetail) {
			...
			for(const pair of prod.pairs) {
				...
				tr.append('td').html((pair.inframe?'in-frame':'out-of-frame')+'<div style="font-size:70%">frame code: '+pair.frame+'</div>')
				const td=tr.append('td')
				...
				tr1.append('td').text(
					pair.a.isoform ? (
					pair.a.codon!=undefined ? 'codon: '+pair.a.codon :
					(pair.a.atutr5?'5\' UTR':
					(pair.a.atutr3?'3\' UTR':
					(pair.a.atupstream?'upstream':'downstream')
					))
					) : ''
				)
				tr2.append('td').text(
					pair.b.isoform ? (
					pair.b.codon!=undefined ? 'codon: '+pair.b.codon :
					(pair.b.atutr5?'5\' UTR':
					(pair.b.atutr3?'3\' UTR':
					(pair.b.atupstream?'upstream':'downstream')
					))
					):''
				)
				...
				tr2.append('td').html(pair.b.contigaa ? '<span style="color:#858585;font-size:70%">starts at</span> '+pair.b.contigaa : '?')
				tr1.append('td').html(pair.a.contigbp ? '<span style="color:#858585;font-size:70%">ends at</span> '+pair.a.contigbp : '?')
				tr2.append('td').html(pair.b.contigbp ? '<span style="color:#858585;font-size:70%">starts at</span> '+pair.b.contigbp : '?')
			}
		}
	}
	if(!arg.sample) {
	...
	if(arg.showothersample && othersample.length>0) {
		arg.holder.append('button')
		.style('display','block').style('margin','20px')
	...
	if(this.expression.genes && geneset.size>0) {
		...
		for(const gene of geneset) {
			...
			if(expd) {
				const div=tr.append('td').style('vertical-align','top')
					.append('div').style('display','inline-block').style('margin-right','20px').style('border','solid 1px #ccc')
				div.append('div').style('background-color','#ededed').style('padding','10px').text(gene)
				if(arg.sample) {
					for(const v of expd) {
						if(v.sample==arg.sample.name) {
							v.ishighlight=true
							div.append('div').style('padding','10px').style('font-size','70%')
				...
				tr.append('td').style('vertical-align','top').text('No expression data for '+gene+'</td>')
			}
		}
	}
}
	...
	for(const sample of arg.samplelst) {
		...
		for(let i=0; i<sample.pairlst.length; i++) {
			...
			tr.append('td').style('text-align','right').html(
				'<span style="background-color:'+client.colorbgleft+';padding:2px 3px;font-size:80%">'+pair.a.name+'</span>'
				+'<span style="background-color:'+client.colorbgright+';padding:2px 3px;font-size:80%">'+pair.b.name+'</span>'
			)
			// frame
			const td=tr.append('td')
			...
			if(pair.inframe) {
				td.html('<span style="background-color:'+client.colorinframe+';color:white;padding:2px 3px;font-size:80%;white-space:nowrap">In frame</span>')
			} else {
				if(pair.a.gm || pair.b.gm) {
					td.html('<span style="background-color:#858585;color:white;padding:2px 3px;font-size:80%;white-space:nowrap">Out of frame</span>')
				} else {
					td.html('<span style="border:solid 1px #858585;color:#858585;padding:1px 2px;font-size:80%;white-space:nowrap">no gene ?</span>')
				}
			}
			// coord
			tr.append('td').html(
				'<div style="background-color:'+client.colorbgleft+';padding:1px 3px;font-size:70%;white-space:nowrap">'
				+pair.a.chr+':'+pair.a.position+' '+pair.a.strand
				+'</div>'
				+'<div style="background-color:'+client.colorbgright+';padding:1px 3px;font-size:70%;white-space:nowrap">'
				+pair.b.chr+':'+pair.b.position+' '+pair.b.strand
				+'</div>'
			)
			// dist
			tr.append('td').html(
				pair.a.chr==pair.b.chr ? bplength(Math.abs(pair.a.position-pair.b.position)) 
			...
			for(const f of fields) {
				...
				if(f.atpair) {
					...
					td.html(
						'<span style="background-color:'+client.colorbgleft+';padding:2px 3px;font-size:80%">'+f.get(pair.a)+'</span>'
						+'<span style="background-color:'+client.colorbgright+';padding:2px 3px;font-size:80%">'+f.get(pair.b)+'</span>'
					)
				}
			}
		}
...
function loadexpression(svmr,file) {
	...
	function done() {
		...
		ep.afterdiv.append('div')
			.text('Expression data loaded for '+genec+' genes, '+samplec+' samples, '+good+' data points'+(bad>0 ? ', '+bad+' bad lines' : ''))
		ep.afterdiv.append('button').text('Delete').style('margin','20px').on('click',()=>{
			delete ep.genes
			ep.prediv.node().removeChild(ep.input.node())
			ep.input=ep.prediv.append('input').attr('type','file')
				.on('change',()=>{
...
function showgenevalues(arg) {
	...
	const dotset=dotg.selectAll()
		.data(arg.data)
		.enter().append('g')
	const dotcir=dotset.append('circle')
		.attr('fill','white')
		.attr('fill-opacity',0)
		.attr('stroke',d=> d.ishighlight ? hlcolor : 'black')
		.attr('stroke-opacity',d=>d.ishighlight ? .7 : .2)
	.on('mouseover',d=>{
		d3event.target.setAttribute('transform','scale(1.5)')
		drag.text(
			(d.sample ? d.sample : d.patient+' '+d.sampletype)
			+' '+d.value).attr('fill',d.ishighlight ? hlcolor : 'black')
		})
	.on('mouseout',d=>{
		d3event.target.setAttribute('transform','scale(1)')
		drag.text('drag to resize').attr('fill','black')
		})
	const drag=svg.append('text').text('drag to resize')
		.attr('font-size',12)
	...
	function sizing() {
		...
		svg.attr('width',dotr*2+width+width2)
			.attr('height',axish+axispad+height+dotr*2)
		client.axisstyle({
			axis: axisg.attr('transform','translate('+dotr*2+','+axish+')')
				.call(
