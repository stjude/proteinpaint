...
export default function spectrumui(genomes) {
	...
	inputdiv.append('div').style('margin-top','20px').html('<ul>'
...
+'</ul>')
	inputdiv.append('p').html('<a href=https://www.dropbox.com/s/qquzf6rxdejflk8/snv.spectrum.txt?dl=0 target=_blank>Example file</a>')
	function cmt(t,red){
...
for(const disease in drawtotal) {
	...
	ul2.append('li').html(
		hasflank ? (lacklflank+lackrflank==0 ? 'all SNVs have flanking sequences' : (
...
d.append('div').classed('sja_menuoption',true).text('Mutation frequency / burden').on('click',()=>{
	burden_render(data,d3event.target.getBoundingClientRect())
})
d.append('div').classed('sja_menuoption',true).text('Mutation spectrum').on('click',()=>{
	spectrum_render(datasnv)
})
if(hasflank) {
	d.append('div').classed('sja_menuoption',true).text('SNV flanking sequence pattern').on('click',()=>{
...
pane.body.append('button').style('display','block').text('SVG').on('click',()=>client.to_svg(svg.node(),'burden'))
const scalewidth=50,
	dwidth=100,
	dpad=10,
	toppad=20,
...
svg.attr('width',scalewidth+diseaselst.length*(dwidth+dpad)+100)
	.attr('height',toppad+height+labpad+maxlabelw)
// y lab
svg.append('g').attr('transform','translate(10,'+(toppad+height/2)+')')
	.append('text')
...
yscale.domain([minmutationf==0?.1:minmutationf,maxmutationf]) .range([height,0])
g.call(
	axisLeft().scale(yscale).ticks(8)
)
client.axisstyle({
	axis:g,
	color:'black',
...
// disease groups
const diseasegroup=g2.selectAll()
	.data(diseaselst)
	.enter().append('g')
...
diseasegroup.append('g')
	...
	.attr('cy',(d)=> yscale((d.f==0 ? .1 : d.f))-height )
	.attr('r',4)
	.attr('fill','black')
	.attr('fill-opacity',.3)
	.attr('stroke','none')
...
for(const disease in data) {
	...
	const div=pane.body.append('div')
		.append('div').style('margin','80px 200px 10px 10px').style('display','inline-block')
...
guide.append('span').text(p.name).style('font-weight','bold')
if(!p.nodetail) {
	let t=0
	for(const s of p.data) {
		t+=s.snvtotal
	}
	guide.append('span').style('padding-left','20px').append('span').text(p.data.length+' samples, '+t+' mutations')
}
guide.append('input').style('margin-left','20px').attr('placeholder','mutation burden cutoff').attr('size',20)
...
guide.append('span').style('padding-left','20px').html('sort samples by&nbsp;')
const select=guide.append('select').style('margin-right','20px').on('change',()=>{
	const s=select.node()
...
guide.append('button').text('SVG').on('click',()=>client.to_svg(svg.node(),'spectrum '+p.name))
guide.append('input').style('margin-left','20px').attr('placeholder','bar width and height').attr('size',20)
	.on('keyup',()=>{
		if(d3event.code!='Enter') return
...
// legend, fixed size
const leg=svg.append('g').attr('transform','translate('+(scalewidth)+',0)')
	.selectAll()
	.data(sp.subs)
	.enter().append('g')
...
const llab2=g0.append('text').text('spectrum').attr('dominant-baseline','hanging' )
const llabg2=g0.append('g')
const llab3=llabg2.append('text').text('Mutation')
const llab4=llabg2.append('text').text('count').attr('dominant-baseline','hanging')
let llab5, llab6
if(p.mafcat) {
	llab5=llabg2.append('text').text('MAF')
	llab6=llabg2.append('text').text('distri.').attr('dominant-baseline','hanging')
}
...
// scale left bottom
const mcountscale= scaleLinear().domain([0,0]).range([0,mcheight])
const mcountaxis=axisLeft().scale(mcountscale)
	.tickSize(4)
const mcountaxisg=g0.append('g')
// samples
const sample_g=g0.append('g').attr('transform','translate('+scalewidth+',0)')
const gsample=sample_g.selectAll()
	.data(p.data)
	.enter().append('g')
// one bar per sampletype
const gbar=gsample.selectAll()
	.data((d)=> d.set)
	.enter()
	.append('g')
// spectrum bars
const gbarbox=gbar.filter((d)=> d.sub)
	.selectAll()
	.data((d)=> d.sub)
	.enter().append('rect')
...
const mcountbar=mcountbarg.append('rect')
	.attr('height',0)
	.attr('fill','#545454')
	.attr('shape-rendering','crispEdges')
// sample label
const samplelabg=gsample.append('g')
const samplelabtext=samplelabg.append('text')
	.text((d)=> d.name)
	.attr('font-family',client.font)
	.attr('text-anchor','end')
	.attr('dominant-baseline','central')
	...
	.on('mouseover',(d)=>{
		...
		const div=client.menushow(pos.left+30,pos.top).append('div').style('border','solid 1px black').style('padding','10px')
		for(const ss of d.set) {
			div.append('div').style('margin','5px 0px').html('<span style="color:#858585">name:</span> '+(ss.name?ss.name:d.name))
			if(!ss.sub) {
				div.append('div').style('margin','5px 0px').text(ss.name+': no non-synonymous SNV')
				continue
			}
			div.append('div').style('margin','5px 0px').html('<span style="color:#858585">total:</span> '+ss.mlst.length)
...
// sampletype label
const stlabg=gbar.filter((d)=> d.name)
	.append('g')
const stlabtext=stlabg.append('text')
	.text((d)=> d.name)
	.attr('font-family',client.font)
	.attr('text-anchor','end')
	.attr('dominant-baseline','central')
...
if(p.mafcat) {
	...
	mafcatlabg.selectAll()
		.data(mafcat)
		.enter().append('text')
		.text((d)=> d.n)
		.attr('y',(d,i)=> mafcath*(i+.5))
		.attr('font-size',mafcath)
		.attr('font-family',client.font)
		.attr('text-anchor','end')
		.attr('dominant-baseline','central')
	mafcatbarg=gbar.append('g')
	mafcatbar=mafcatbarg.selectAll()
		.data((d)=> d.mafcat)
		.enter().append('g')
...
const resizebutton=holder.append('div').style('position','absolute').style('right','0px').style('bottom','0px')
	.text('drag to resize')
	...
	.on('mousedown',()=>{
		...
		b.on('mousemove',()=>{
			resize(
				h+d3event.clientY-y,
				w+(d3event.clientX-x)/samplecount
			)
		})
		b.on('mouseup',()=>{
...
function resize(barh,barw) {
	...
	if(barwidth>=8) {
		...
		samplelabg.attr('transform',(d)=> 'translate('+(barwidth/2)+','+(barheight+pad3+maxsetlabelw+pad4)+')' )
	...
	const svgheight= legheight+pad+barheight+pad3+maxsetlabelw+pad4+maxlabelw+pad5+mcheight+pad2
	...
	axis.scale( scaleLinear().domain([0,1]).range([barheight,0]))
	axisg.call(axis)
	client.axisstyle({
		axis:axisg,
		color:'black',
	...
	mcountaxisg.attr('transform','translate('+(scalewidth-2)+','+(barheight+pad3+maxsetlabelw+pad4+maxlabelw+pad5)+')')
	let barcount=0
	...
	gbarbox.attr('y',(d)=> d.cum*barheight )
	...
	if(mafcatlabg) {
		mafcatlabg.attr('transform','translate('+(scalewidth-2)+','+(barheight+pad3+maxsetlabelw+pad4+maxlabelw+pad5+mcheight+pad2)+')')
		mafcatbarg.attr('transform','translate(0,'+(barheight+pad3+maxsetlabelw+pad4+maxlabelw+pad5+mcheight+pad2)+')')
...
function setcutoff(cutoff) {
	...
	mcountbarg.filter((d)=> d.mlst.length>cutoff)
		.append('text')
...
for(const diseasename in data) {
	...
	for(const setname of setnames) {
		...
		*/
		...
		const svg=holder.append('svg')
		svg.attr('height',pad+barheight+flanklabh+translabfontsize)
			.attr('width',scalewidth+sp.subs.length*(sp2+fllst.length*(barwidth+sp1)-sp1))
		const scale=scaleLinear().domain([0,maxsnvcount*100/snvtotal]).range([barheight,0])
		...
		svg.append('g').attr('transform','translate(10,'+(pad+barheight/2)+')')
			.append('text').text('SNV percentage')
			.attr('font-size',15)
			.attr('text-anchor','middle')
		...
		for(let i=0; i<sp.subs.length; i++) {
			...
			const g=svg.append('g').attr('transform','translate('+(scalewidth+sp2+(sp2+fllst.length*(barwidth+sp1)-sp1)*i)+','+(pad+barheight)+')')
			g.append('text')
			...
			for(let j=0; j<fllst.length; j++) {
				...
				g2.append('g').attr('transform','translate(0,15)')
					.append('text').text(fllst[j])
					.attr('font-size',barwidth)
					.attr('text-anchor','middle')
					.attr('dominant-baseline','middle')
					.attr('transform','rotate(-90)')
