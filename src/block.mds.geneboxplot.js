...
export function init(p) {
	...
	plot.errdiv=plot.holder.append('div')
		.style('margin','10px')
	...
	buttonrow.append('button')
		...
		.on('click',()=> {
			...
			d3event.target.innerHTML=plot.uselog ? 'No log10' : 'Use log10'
			plot.place()
		})
		...
		*/
		...
		// boxplot choice - sv - config
		{
			...
			{ 
				const row = d2.append('div')
				row.append('span')
					.html('Include SV from flanking region of length:&nbsp;')
				row.append('input')
					.property('value', 0)
					.attr('type','number')
					.style('width','80px')
	...
	*/
	...
	plot.place=()=>{
		...
		const scale0 = (plot.uselog ? scaleLog() : scaleLinear())
			.domain([
				plot.data.min==0 ? 0.001 : plot.data.min,
				plot.data.max
			])
			.range([0, axisw])
		const scale = v=>{
		...
		if(plot.data.lst) {
			...
			for(const d of plot.data.lst) {
				d.circle.transition().attr('cx',scale(d.value))
				.attr('cy',y)
				.attr('r',rowheight/2)
				y+=circleyshift
			}
			...
			for(const g of plot.data.groups) {
				...
				const _rowheight = rowheight * (g.boxplots.length>1 ? .8 : 1)
				...
				for(const bp of g.boxplots) {
					...
					if(bp.label) {
						bp.label
							...
							.each(function(){ rightwidth=Math.max(rightwidth,this.getBBox().width)})
					}
				...
				g.label
					...
					.each(function(){labwidth=Math.max(labwidth,this.getBBox().width)})

				if(g.bg) g.bg.attr('y',-rowspace/2).attr('width',axisw).attr('height',h+rowspace)
		...
		plot.svg.attr('width',labwidth+lablspace+axisw+axispad2+rightwidth)
			.attr('height', axisheight+y+30)
	}
	...
	.then(data=>{
			...
			for(const [i,g] of data.groups.entries()) {
				...
				for(const bp of g.boxplots) {
					...
					if(bp.iscnvgain) {
						...
						bp.label=g.g.append('text')
							.text('CNV gain ('+bp.samplecount+')')
					} else if(bp.iscnvloss) {
						color=plot.color.cnvloss
						bp.label=g.g.append('text')
							.text('CNV loss ('+bp.samplecount+')')
					} else if(bp.issv) {
						color='black'
						bp.label=g.g.append('text')
							.text('SV ('+bp.samplecount+')')
					} else {
						color=color0
					}

					if(bp.label) {
						bp.label.attr('fill',color).attr('font-family',client.font).attr('dominant-baseline','central')
					}

					if(bp.w1!=undefined) {
						// has valid values for boxplot, could be missing
						bp.hline=g.g.append('line')
							.attr('stroke',color).attr('shape-rendering','crispEdges')
						bp.linew1=g.g.append('line')
							.attr('stroke',color).attr('shape-rendering','crispEdges')
						bp.linew2=g.g.append('line')
							.attr('stroke',color).attr('shape-rendering','crispEdges')
						bp.box=g.g.append('rect')
							.attr('fill','white')
							.attr('stroke',color).attr('shape-rendering','crispEdges')
						bp.linep50=g.g.append('line')
							.attr('stroke',color).attr('shape-rendering','crispEdges')
					}
					// outliers
					for(const d of bp.out) {
						d.circle=g.g.append('circle')
							.attr('stroke',color)
							.attr('fill','white')
							.attr('fill-opacity',0)
							.on('mouseover',()=>{
		...
		if(plot.sample) {
			...
			plot.sample.svgtext = plot.sample.g.append('text')
				...
				.attr('fill','blue')
			plot.sample.line=plot.sample.g.append('line')
...
function init2(x,y, plot, group) {
	...
	*/
	...
	pp.errdiv=pp.holder.append('div')
		.style('margin','10px')


	const buttonrow=pp.holder.append('div')
		.style('margin','10px')
	const configdiv=pp.holder.append('div')
		.style('margin','10px')
		.style('border','solid 1px #ededed')
		.style('padding','10px')
		.style('display','none')
	...
	buttonrow.append('button')
		...
		.on('click',()=> {
			pp.uselog=!pp.uselog
			d3event.target.innerHTML=pp.uselog ? 'No log10' : 'Use log10'
			pp.place()
		})
	...
	pp.place=()=>{
		...
		for(const col of pp.statuscolumns) {
			...
			for(const d of pp.data.lst) {
				...
				if(cell.label) {
					cell.label
						.attr('font-size', circleradius*2-2)
						.each(function(){
		...
		for(const col of pp.statuscolumns) {
			if(!col.g) {
				...
				col.namelabel = col.g.append('text')
					.attr('font-family',client.font)
					.attr('dominant-baseline','central')
			...
			col.namelabel
				.attr('font-size', Math.min(15, col.width) )
				.each(function(){
		...
		const scale0 = (pp.uselog ? scaleLog() : scaleLinear())
			.domain([
				pp.data.min==0 ? 0.001 : pp.data.min,
				pp.data.max
			])
			.range([0, axiswidth])
		const scale = v=>{
		...
		for(const [idx,d] of pp.data.lst.entries()) {
			...
			if(d.rowbg) {
				d.rowbg
					.attr('width', statustotalwidth+axiswidth)
		...
		pp.svg.attr('width', samplenamewidth + statuscolpad + statustotalwidth + axiswidth + circleradius )
	...
	pp.makegraph=()=>{
		...
		for(const d of pp.data.lst) {
			...
			if(d.gain || d.loss || d.sv || d.estat.ase_monoallelic || d.estat.ase_biallelic || d.estat.ase_uncertain) {
				...
				d.rowbg = d.rowg.append('rect')
					.attr('class','sja_bgbox')
			}

			d.circle = d.rowg.append('circle')
				.attr('fill', 'white')
				.attr('fill-opacity', 0)
				.attr('stroke', '#858585')
				.on('mouseover',()=>{
			...
			if(d.estat.ase_monoallelic || d.estat.ase_biallelic || d.estat.ase_uncertain) {
				...
				const cell = { g: d.rowg.append('g') }
				cell.rect = cell.g.append('rect')
						.attr('fill',  expressionstat.ase_color( d, _p.gecfg ) ),
				cell.label = cell.g.append('text')
			...
			if(status2cell.size) {
				...
				d.samplelabel = d.rowg.append('text')
					.attr('font-family',client.font)
					.attr('text-anchor','end')
					.attr('dominant-baseline', 'central')
					.text(d.sample)
...
function tooltip_pp(d, holder, pp) {
	const lst=[
		{k:'sample', v:d.sample},
		{k: pp._plot.gecfg.datatype, v:d.value}
	]
	if(d.gain || d.loss || d.sv) {
		const l2=[]
		if(d.gain) {
			l2.push('<span style="padding:0px 5px;color:white;background:'+pp._plot.color.cnvgain+'">Copy number gain</span>')
		}
		if(d.loss) {
			l2.push('<span style="padding:0px 5px;color:white;background:'+pp._plot.color.cnvloss+'">Copy number loss</span>')
		}
		if(d.sv) {
