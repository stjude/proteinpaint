...
function mclassorigin(block, height) {
	...
	let h=0
	const g=block.svg.append('g').attr('transform','translate('+(xpad+block.leftheadw+block.lpad+block.width-200)+','+height+')')
	if(block.legend.mclasses && block.legend.mclasses.size>0) {
		for(const [cls, obj] of block.legend.mclasses) {
			if(obj.hidden) {
				continue
	...
	if(block.legend.morigins && block.legend.morigins.size>0) {
		for(const [org, obj] of block.legend.morigins) {
			if(obj.hidden) {
				continue
			}
			const row=g.append('g').attr('transform','translate(0,'+h+')')
