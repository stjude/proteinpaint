...
export function samegene(arg) {
	...
	*/
	...
	let exonleft, exonright,
		exonleftin, exonrightin,
...
export function differentgenes(arg) {
	...
	*/
	...
	let x1=0, x2=0 // ends of junction
	...
	svg.attr('width',xpad*2+x)
		.attr('height',junctionheight+exonheight+ypad*2)
}
...
function exonleft_exonright(l,r,svg) {
	...
	*/
	...
	svg.attr('width',xpad*2+x2+rightboxw)
...
function exonleft_exonrightin(l,r,svg) {
	...
	svg.attr('width',xpad*2+x2+rightboxw/2)
		.attr('height',ypad*2+junctionheight+exonheight)
}
...
function exonleft_intronright(l,r,svg) {
	...
	svg.attr('width',xpad*2+x2+intronwidth/2+rightboxw)
		.attr('height',ypad*2+junctionheight+exonheight)
}
...
function exonleftin_exonrightin(l,r,svg) {
	...
	if(l.exonidx==r.exonidx) {
		...
		g.append('path')
			...
			.attr('fill','none')
		svg.attr('width',xpad*2+leftboxw)
...
function intronleft_intronright(l,r,svg) {
	...
	if(l.intronidx==r.intronidx) {
		...
		svg.attr('width',xpad*2+leftboxw+ 30 +rightboxw)
			.attr('height',ypad*2+junctionheight+exonheight)
		return
	}
...
function leftout_exonright(r,svg) {
	...
	g.append('text')
		...
		.attr('font-size', fontsize)
		.attr('font-family',client.font)
		.each(function(){ leftboxw=this.getBBox().width})
...
function leftout_intronright(r,svg) {
	...
	svg.attr('width',xpad*2+x1+30+exon1w+intronwidth+exon2w)
		.attr('height',ypad*2+junctionheight+exonheight)
}
...
function renderJunction(g,x1,x2,x1atintron,x2atintron,isnormal) {
	g.append('path')
		.attr('d',
			'M'+x1+','+ (junctionheight+(x1atintron?exonheight/2:0)) +
			'L'+((x1+x2)/2)+',0'+
			'L'+x2+','+ (junctionheight+(x2atintron?exonheight/2:0))
			)
...
function renderLeftExon(exonidx, g) {
	...
	g.append('text')
		...
		.attr('font-size', fontsize)
		.attr('font-family',client.font)
		.each(function(){ textw=this.getBBox().width })
		.remove()
