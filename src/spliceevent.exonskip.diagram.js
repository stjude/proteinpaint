...
import {exoncolor, IN_frame, OUT_frame} from './common'
...
const holder=arg.holder.append('div')
...
const diagram=arg.holder.append('svg')
...
if(showup1exon) {
	...
	diagramg.append('path')
		.attr('d',
			'M'+(xoff-intronwidth)+','+(exony+exonheight)+
			'L'+(xoff-intronwidth/2)+','+(exony+exonheight+junctionheight)+
			'L'+xoff+','+(exony+exonheight)
		)
	...
	if(!nojunction) {
		...
		const text=diagramg.append('text')
			.text(readcount)
...
// draw junction B
{
	...
	diagramg.append('path')
		.attr('d',
			'M'+((showup1exon ? exonwidth+intronwidth : 0)+exonwidth)+','+exony
			+'L'+midx+','+(exony-junctionheight)
			+'L'+(xoff-exonwidth-intronwidth)+','+exony
			)
		.attr('fill','none')
	...
	const bclab=diagramg.append('text')
		.text(jBreadcount)
		.attr('x',midx)
