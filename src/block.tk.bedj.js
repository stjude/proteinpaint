...
export function bedjload(tk,block) {
	...
	fetch( new Request(block.hostURL+'/tkbedj',{
		method:'POST',
...
function configpanel(tk, block ) {
	...
	row.append('input').property('value',tk.stackheight).attr('size',5).on('keyup',()=>{
	...
	if(!tk.categories) {
		...
		row.append('input').property('value',tk.color).attr('type','color').on('change',()=>{
