...
import {itemtable,runtimeattr_sv,runtimeattr_itd,runtimeattr_del,runtimeattr_trunc,runtimeattr_snvindel} from './block.ds.itemtable'
	...
	*/
	...
	if(tk.ds.stratify) {
		...
		for(const s0 of tk.ds.stratify) {
			...
			strat.svglabel = block.maketklefthandle(tk, laby)
				.on('click',()=>{
	...
	if(tk.ds.iscustom) {
		...
		block.maketklefthandle(tk, laby)
			.text('Close')
			.on('mousedown',()=> {
	...
	if(tk.ds.id2vcf) {
		...
		for(const vcfid in tk.ds.id2vcf) {
			...
			for(const m of lst2) {
				...
				txt.push(`${m.chr}\t${m.pos}\t${m.ref}\t${m.alt}\t${info.join(';')}\t${altinfo.join(';')}\t${common.mclass[m.class].label}\t${m.mname}`)
			}
		}
...
function getmlstattrnames(tk) {
	const skip=new Set(['__x','isrim1','isrim2','aapos','alt','chr','class','dt','isoform','mname','origin','pos','ref','rnapos','rnaposition','gene','rnaduplength','rnadellength'])
...
function freestratify(key,tk,block,div) {
	...
	const table=div.append('table').style('border-spacing','1px').style('border-collapse','separate').style('font-size','.8em')
...
function label_strat_fillpane(tk,block,strat,tip) {
	...
	root.eachBefore(n=>{
		...
		// td1
		tr.append('td').style('padding-left',(n.depth-1)*15+'px')
			.html(n.data.name+(n.data.full?'&nbsp;<span style="font-size:.7em;color:#858585">'+n.data.full+'</span>':''))
		// td2
		if(strat.bycohort && tk.ds.cohort.root) {
			...
			if(cohortsize==0) {
				...
				const h=10, w=40
				const svg=td2.append('svg').attr('width',w).attr('height',h)
				svg.append('rect').attr('width',w).attr('height',h).attr('fill',tk.ds.cohort.fbarbg)
				svg.append('rect').attr('width',w*n.value/cohortsize).attr('height',h).attr('fill',tk.ds.cohort.fbarfg)
				svg.append('rect').attr('width',w).attr('height',h).attr('fill','white').attr('fill-opacity',0)
					.append('title').text(Math.ceil(100*n.value/cohortsize)+'% ('+n.value+'/'+cohortsize+')')
			}
		}
		// td3
...
function ctrlui_vcfinfofilter(tk,block) {
	...
	*/
	...
	if(obj.setidx4numeric!=undefined) {
		// given setidx4mclass
		if(!Number.isInteger(obj.setidx4numeric)) return '.setidx4numeric value should be array index (non-negative integer)'
			...
			.on('click',()=>{
				...
				if(usethisset) {
					set.namebutton
						...
						.style('border-bottom',    'solid 2px #ccc')
				}
				dstkrender(tk, block)
			})
...
function ctrlui_populationfrequency(tk, block) {
	...
	*/
	...
	tk.populationfrequencyfilter.holder = div.append('div')
		.style('display','inline-block')
}
