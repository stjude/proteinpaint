...
export default function hm2legend(hm, rectFillFxn, textFxn, iconStroke,itemOpacity) {
	...
	function main(g) {
		...
		g.attr('transform', hm.legendontop ? null : 'translate(0,'+hm.svgh+')')
			.on('mouseover.tphm2', hm.handlers.legend.mouseover)

		const l = g.selectAll('g')
			.data(hm.h.legend)
	...
	function addGroup(d,i) {
		...
		let g = select(this)
		const leftdist=hm.legendhangleft ? hm.legendpadleft + hm.legendhangleft-hm.legendpadx  
										: hm.legendpadleft

		const grplabel = g.append('text')
	...
	function addItem(d,i) {
		let g = select(this).attr('transform', 'translate('+currlinex+','+currliney+')')
							.style('opacity',itemOpacity)

		const itemlabel = g.append('text')
		...
		if (hm.legendlinesep || currlinex > hm.svgw - hm.legendpadright) {
			currliney += hm.legendlineh
			const leftdist=!hm.legendhangleft ? hm.legendpadleft
								: hm.legendpadleft + hm.legendhangleft + hm.legendpadx  
	...
	main.finalizePos = function (h) {
		currG.transition().duration(hm.duration)
