	...
	*/
	...
	.then(obj=>{
		if(obj.junctiontotalnumber) {
			tk.junctionCountLabel.text((tk.data.length==obj.junctiontotalnumber ? tk.data.length : tk.data.length+' of '+obj.junctiontotalnumber) + ' junction'+(obj.junctiontotalnumber>1?'s':''))
		} else {
...
function updateLabel(tk,block) {
	...
	tk.junctionCountLabel.each(function(){lst.push(this.getBBox().width)})
	tk.sampleCountLabel.each(function(){lst.push(this.getBBox().width)})
...
function rawdata2track(raw, tk, block) {
	...
	*/
	...
	for(const j of raw) {
		...
		if(e) {
			console.log('junction not in view range: '+j.chr+':'+j.start+'-'+ (j.sv ? j.sv.mate.chr+':'+j.sv.mate.start : j.stop) )
			continue
		}
		junctions.push(j)
	}
...
jug2.append('circle')
	...
	.on('mouseover',d=>{
		...
		d3select(d.disc).attr('fill-opacity',.8)
	...
	.on('click',(j)=>{
		...
		if(!tk.iscustom) {
			pane.header.append('span')
				.style('color','#858585')
				.style('font-size','.8em')
				.text(tk.mds.queries[tk.querykey].name)
		}
		pane.header.append('text')
			.text(j.chr+':'+j.start+'-'+j.stop)
		const div1=pane.body.append('div')
...
doForceLayout(tk, block, viewpxwidth)
	.then(()=>{
...
function set_all(tk) {
	...
	const dur=500
	tk.jug.selectAll('.sja_jug_leg1')
		.transition().duration(dur)
		.attr('y2',-tk.legheight)
		.attr('x1',(d)=> set_leg_x1(d))
	tk.jug.selectAll('.sja_jug_leg2')
		.transition().duration(dur)
		.attr('y1',-tk.legheight)
		.attr('x2',(d)=> set_leg_x2(d))
	tk.jug.selectAll('.sja_jug_jug2')
		.transition().duration(dur)
		.attr('transform', (d)=> set_jug2(d,tk))
	/*
	tk.jug.selectAll('.sja_jug_rim')
		...
		*/
	tk.jug.transition().duration(dur)
		.attr('transform',(d)=> set_jug(d))
	tk.jug.selectAll('.sja_jug_stem')
		.transition().duration(dur)
...
function doForceLayout(tk, block, viewpxwidth) {
	...
	return new Promise((resolve,reject)=>{
		...
		.force('y',d3force.forceY(d=>{
			return tk.axisheight-tk.yscale(d.junction.medianReadCount)
			})
			.strength(1)
			)
		.force('x',d3force.forceX(d=>d.tox)
			.strength(.1)
			)
		.force('collide',d3force.forceCollide( d=> {
...
function mouseoverBoxplot(j, tk) {
	...
	const g=tk.pica.g.append('g')
		.attr('transform','translate('+(-5-w-j.radius2)+',0)')
	// v line
	g.append('line')
		.attr('x1',w/2)
		.attr('x2',w/2)
...
function showMenu_cohortFilter( tk, block, attr, item, div) {
	...
	*/
	...
	if(item.count) {
		tip.d.append('div')
			.html( item.totalCount ? '<span style="font-size:1.5em">'+item.count+'</span> / '+item.totalCount+' sample'+(item.count>1?'s':'')
...
function makeLegend_infoFilter(info, tk, block) {
	...
	*/
	...
	for(const [value, count] of lst) {
		...
		const cell=info.holder.append('div')
			.style('display','inline-block')
			.style('padding','10px')
			.attr('class','sja_clb')
			.on('click',()=>{
	...
	for(const value in info.hiddenCategories) {
		...
		const div=info.holder.append('div')
			.style('display','inline-block')
			.style('padding','8px')
			.style('color','#858585')
			.attr('class','sja_clb')
...
function makeTk(tk, block) {
	// make color gradients
	{
		...
		mid.append('stop').attr('offset',.5).attr('stop-color','white')
	...
	if(tk.permanentHierarchy) {
		...
		// dummy name
		tk.subhierarchylabel = block.maketklefthandle( tk, laby )
	...
	// the callback may have something to do with server query! so may move this part to mdsjunction.render too!!
	{
		...
		tk.junctionCountLabel=block.maketklefthandle(tk,laby)
			.on('click',()=>{
				// TODO
			})
	}
	...
	{
		const tip=new client.Menu({padding:'none'})
		tk.sampleCountLabel=block.maketklefthandle(tk,laby)
			.on('click',()=>{
				// TODO
			})
	}
...
function showOneJunction(j, tk, holder, block, ifeventdetails) {
	...
	{
		const d = row1.append('div')
			.style('display','inline-block')
			.style('margin-right','10px')
		if(!j.sv || j.chr==j.sv.mate.chr) {
			// same chr
			d.html(common.bplen(Math.abs(j.start- (j.sv ? j.sv.mate.start : j.stop)))+' <span style="font-size:.8em;">'+j.chr+':'+(j.start+1)+'-'+((j.sv?j.sv.mate.start:j.stop)+1)+'</span>')
		} else {
			// inter-chr sv
			d.html('<span style="font-size:.8em;">'+j.chr+':'+(j.start+1)+'-'+j.sv.mate.chr+':'+(j.sv.mate.start+1)+'</span>')
		}
	}
...
function showJunctionDiagram(j,tk,holder) {
	...
	*/
	...
	if(j.ongene.exonleftin) j.ongene.exonleftin.forEach(i=>leftgenes.set(i.isoform, {gene:i.gene, strand:i.strand}))
	if(j.ongene.intronleft) j.ongene.intronleft.forEach(i=>leftgenes.set(i.isoform, {gene:i.gene, strand:i.strand}))
	...
	if(j.ongene.exonrightin) j.ongene.exonrightin.forEach(i=>rightgenes.set(i.isoform, {gene:i.gene, strand:i.strand}))
	if(j.ongene.intronright) j.ongene.intronright.forEach(i=>rightgenes.set(i.isoform, {gene:i.gene, strand:i.strand}))
...
function showEventdiagram_a53ss(j, e, tk, holder,block) {
	...
	const e2={
		junctionB:{
			start:j.start, stop:j.stop, v:j.medianReadCount
		},
		a5ss:e.a5ss,
		a3ss:e.a3ss,
	...
	import('./spliceevent.a53ss.diagram').then(p=>{
		...
		setTimeout(()=>{
			...
			fetchReadcount4junctionAbyjunctionBsamples(tk, block, j, new Map([[e.junctionA.start+'.'+e.junctionA.stop,text]]), [[e.junctionA.start,e.junctionA.stop]])
		},
		1000)
	})
}
...
function listAllEvents(lst, holder, j, tk, block) {
	...
	for(const e of lst) {
		div.append('div')
			...
			.on('click',()=>{
				tk.tktip.clear()
					.show(d3event.clientX+20,d3event.clientY-40)
