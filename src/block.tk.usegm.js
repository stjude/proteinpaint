...
import {basecompliment} from './common'
...
function configpanel(tk, block) {
	...
	if(block.allgm.length>1) {
		...
		tr2.append('td')
			.attr('colspan',2)
			.style('color','#858585')
			.text('Only one isoform '+block.usegm.isoform)
	}
	if(block.gmmode==client.gmmode.gmsum) {
		...
		tr3.append('td')
			.text(usecount==block.allgm.length ? 'Showing all '+usecount+' isoforms' : 'Showing '+usecount+' of '+block.allgm.length+' isoforms')
		tr3.append('td')
			.attr('class','sja_menuoption')
	...
	if(block.usegm.exon.length>1 && (block.gmmode==client.gmmode.protein || block.gmmode==client.gmmode.exononly)) {
...
function configpanel_gmsum(block, holder) {
	...
	for(const [genetkname,isoformset] of genetkset) {
		...
		const tkcheckbox=row.append('input')
			.attr('type','checkbox')
			.attr('id',id)
			.on('change',()=>{
		...
		for(const [isoform,obj] of isoformset) {
			const row=holder.append('div')
				.style('margin','3px 5px 3px 30px')
			const id=Math.random()
			obj.checkbox=row.append('input')
				.attr('type','checkbox')
				.attr('id',id)
			if(!obj.hidden) {
			...
			row.append('label')
				.html('&nbsp;&nbsp;'+isoform.isoform+' <span style="color:#858585">'+(isoform.cdslen ? Math.ceil(isoform.cdslen/3)+' AA' : 'noncoding')+'</span>')
				.attr('for',id)
				.attr('class','sja_clbtext')
		}
	}
...
export function gmtkrender(tk,block) {
	...
	if(block.gmmode==client.gmmode.gmsum) {
		...
		for(const gm of block.allgm) {
			...
			const thislab=gm.__tkg.append('text')
				.text(gm.isoform)
				.attr('y',h/2)
				.attr('x',-12)
...
if(gm.coding && (block.gmmode==client.gmmode.protein || block.gmmode==client.gmmode.exononly || block.gmmode==client.gmmode.splicingrna)) {
	// is protein
		...
		*/
		...
		if(showntseq) {
			...
			for(let j=cdsstart; j<cdsstop; j++) {
				...
				tkg.append('text').text(gm.cdseq[j])
				.attr('font-family','Courier')
				.attr('font-size',rowh)
				.attr('dominant-baseline','central')
				.attr('text-anchor','middle')
				.attr('fill','black')
				.attr('x',x+pxstart+(j-cdsstart+.5)*block.exonsf)
				.attr('y',2+rowh/2)
			}
		}
		if(cdsstart%3==2) {
		...
		if(showaaseq) {
			...
			for(let j=cdsstart; j<cdsstop; j++) {
				...
				tkg.append('text').text(gm.aaseq[Math.floor(j/3)])
				.attr('font-family','Courier')
				.attr('font-size',rowh)
	...
	for(const e of thinlst) {
		...
		for(let i=block.startidx; i<=block.stopidx; i++) {
			...
			for(let j=start; j<stop; j++) {
				...
				.attr('x',x
					+(r.reverse ? 
						(r.stop-stop)*block.exonsf
						+(stop-start)*block.exonsf
						-(j-start+.5)*block.exonsf
					: 
						(start-r.start)*block.exonsf+(j-start+.5)*block.exonsf)
					)
				.attr('y',h/2)
			}
		}
...
for(const e of eboxes) {
	...
	.attr('fill', '#858585')
	.attr('fill-opacity',0)
	.on('mouseover',()=>d3event.target.setAttribute('fill-opacity',.2))
	...
	if(p.aapos && gm.pdomains) {
		for(const d of gm.pdomains) {
			...
			if(d.start<=p.aapos && d.stop>=p.aapos) {
				...
				row.append('span').style('font-family',client.font).html('&nbsp;'+d.name)
				if(d.description) {
					row.append('span').style('font-family',client.font).style('font-size','.7em').html('&nbsp;'+(d.description.length>30 ? d.description.substring(0,20)+' ...' : d.description))
				}
			}
		}
	}
...
function domainlegend(tk,block) {
	...
	for(const domaintype of lst) {
		...
		*/
		...
		const row=tk.td_legend.append('div')
		...
		row.append('span').text(domaintype.name)
			.style('color','#aaa')
			.style('padding-right','10px')
		row.append('span').text(domaintype.description)
			.style('color','#aaa')
			.style('font-size','.7em')
			.style('padding-right','10px')
		...
		if(domaintype.SMART) {
			row.append('a')
				.attr('href','http://smart.embl-heidelberg.de/smart/do_annotation.pl?BLAST=DUMMY&DOMAIN='+domaintype.SMART.substr(5))
				.attr('target','_blank')
				.text('SMART')
				.style('font-size','.7em')
				.style('padding-right','10px')
	...
	if(block.usegm.coding && block.usegm.coding.length>0) {
		...
		tk.td_legend.append('div').style('margin-top','10px')
...
function customdomainmakeui(block,tk,pane) {
	...
	pane.header.html('Add domains for '+block.usegm.name+' <span style="font-size:.8em">'+block.usegm.isoform+'</span>')
	...
	pane.body.append('p').style('font-size','.9em').html('<span style="font-size:.8em;color:#aaa">EXAMPLE</span>&nbsp;&nbsp;domain_name ; 100 200 ; red')
	...
	row1.append('button').text('Submit').style('margin-left','5px').on('click',()=>{
	...
	row1.append('button').text('Clear').style('margin-left','5px').on('click',()=>ta.property('value',''))
	const errdiv=pane.body.append('div').style('margin-top','10px').style('display','none')
	const err=(m)=>{
		errdiv.style('display','block')
		client.sayerror(errdiv,m)
	}
	pane.body.append('div').style('color','#858585').style('margin-top','20px').html(
