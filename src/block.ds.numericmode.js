...
function may_customcohortvcf(m, tk, block) {
	// disabled for now
	return false
		...
		if (m.sampledata.length == 0 && tk.ds.vcfcohorttrack) {
			...
			tk.pica.g.attr(
				...
				'translate(' +
					(m.aa.x + m.xoff) +
					',' +
					(tk.numericmode.toplabelheight + tk.numericmode.axisheight + m.radius - m._y) +
					')'
			)
...
function m2sunburst(m, tk, block) {
	...
	*/
	...
	/****************
	to enable this, must port over below lines from block.ds.sun1
	as sun1 should no longer be directly imported here
