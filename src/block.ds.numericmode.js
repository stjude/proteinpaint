...
export function rendernumerictk(tk, block, originhidden) {
	...
	*/
	...
	tk.height_main = tk.toppad+nm.toplabelheight
		//+disclabelspacing
		+nm.maxradius+nm.axisheight+nm.maxradius
...
if(tk.hlaachange) {
	...
	textlabels
		.filter((m)=>tk.hlaachange.has(m.mname))
...
function skewer2_setstem(d,tk) {
	...
	// funnel
	return 'M0,0'
		+'v'+tk.stem1 // vertical down
		+'l'+(-d.xoffset)+','+tk.stem2 // slope 1
		+'v'+tk.stem3 // vertical down
		//+'h1' // to right 1
		+'v-'+tk.stem3 // veritical up
		+'l'+(d.stemw+d.xoffset-1)+',-'+tk.stem2 // slope 2
		+'v-'+tk.stem1
		//+'Z'
}
...
function m2sunburst( m, tk, block) {
	/*
	conditions met!
	collect samples with the variant allele, if gtallref is set, drop sample
	*/
	const lst=[]
	for(const s of m.sampledata) {
		if(s.gtallref) {
	...
	sun1(tk, block, {
		cx: m.aa.x+m.xoff,
		cy: tk.numericmode.toplabelheight+tk.numericmode.axisheight-m._y,
		mlst: [m2],
		label: m.mname,
		cohort: tk.ds.cohort,
		noclickring:true,
