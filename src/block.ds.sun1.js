...
if(radius>tk.height*.42) {
	radius=tk.height*.42
} else if(radius<tk.height*.2) {
	radius=tk.height*.2
}
...
if(newcx!=cx || newcy!=cy) {
	g.transition().delay(dur1+dur2).attr('transform','translate('+newcx+','+newcy+')')
	cx=newcx
	cy=newcy
}
// hierarchy
...
ring.selectAll()
	...
	.on('mouseover',d=>{
		...
		d3event.target.setAttribute('fill', d3rgb(d._color).darker(.5).toString())
	...
	.on('click',d=>{
		...
		if(arg.cohort.annotation) {
			...
			if(d.data.lst.length==1) {
				// single sample
				showsinglesample(d.data.lst[0].k4a, arg.cohort.annotation[ d.data.lst[0].k4a ], d3event.clientX, d3event.clientY)
				return
			}
...
.on('end',()=>{
	eye.shutter=eye.append('rect')
		...
		.attr('fill','#ededed')
		.on('click',()=>remove(sun))
	eye.shutter.transition().duration(dur2)
	.attr('height',emptyspace*2)
	.on('end',()=>{
		// shutter done, add things
		eye.fore=eye.append('g');
		const fontsize1=Math.min(eyeheight/(arg.label.length*client.textlensf),emptyspace*.6)
		eye.fore.append('text')
			.text(arg.label)
		...
		const fontsize = Math.min(18,Math.min(emptyspace*.7/(itemlst.length.toString.length*client.textlensf),(emptyspace-fontsize1/2)*.7))
		eye.fore.append('text')
			.text(itemlst.length)
			.attr('text-anchor','middle')
			.attr('fill','#858585')
		...
		// button at bottom
		sun.listbutt=eye.append('g')
			.attr('transform','translate(0,'+(fontsize1/2)+')')
		const listbutt_bg=sun.listbutt.append('rect')
			.attr('x',-eyeheight/2)
			.attr('width',eyeheight)
			.attr('height',emptyspace-fontsize1/2)
			.attr('fill','#d9d9d9')
		const listbutt_text=sun.listbutt.append('text')
...
function remove(sun) {
	...
	if(sun.eye.shutter) {
		sun.eye.shutter
			...
			.on('end',()=>{
				sun.ring.transition().attr('transform','scale(.5,.5)')
				.attr('fill-opacity',0)
				.on('end',()=>{
					sun.g.remove()
	...
	*/
	if( toangle>=.375 && toangle<=.625 ) {
		...
		if(yoff >= alltkh-30) {
			...
			if(toangle<=.5) {
				toangle=Math.max( .25, d.x0 )
			} else {
				toangle = Math.min( .75, d.x1)
			}
		}
	}
	...
	const text0=pica.g.append('text')
		.attr('stroke','white')
		.attr('stroke-width',3)
		.attr('font-size',fontsize)
		.attr('font-family','Arial')
	const text=pica.g.append('text')
		.attr('fill','black')
		.attr('fill-opacity',1)
		.attr('font-size',fontsize)
		.attr('font-family','Arial')
	...
	const text20=pica.g.append('text')
		.attr('stroke','white')
		.attr('stroke-width',3)
		.attr('font-size',fontsize)
		.attr('font-family','Arial')
	const text2=pica.g.append('text')
		.attr('fill','black')
		.attr('fill-opacity',1)
		.attr('font-size',fontsize)
		.attr('font-family','Arial')
	const tt=d.value+' sample'+(d.value>1?'s':'')
		+(bar ? ' ('+cohortsize+' total)' : '')
	text20.text(tt)
	text2.text(tt)
	...
	if(toangle<.25) {
		...
		text20.attr('dominant-baseline','hanging').attr('text-anchor','end').attr('y',ypad+fontsize)
		text2.attr('dominant-baseline','hanging').attr('text-anchor','end').attr('y',ypad+fontsize)
		if(bar) {
