...
import {dtitd,dtdel,dtfusionrna,dtnloss,dtcloss,mclassitd,mclassnloss,mclasscloss,mclassfusionrna} from './common'
...
export function svmr_export_json(svmr,hqonly) {
	...
	export_data(
		'Fusion data from '+svmr.filename,
		[{text:lines.map(i=>i.join('\t')).join('\n')}]
	)
}
...
export function svmr_export_text(svmr,hqonly) {
	...
	export_data('Fusion data from '+svmr.filename,
		[{text:headerlst.join('\t')+'\n'+rows.join('\n')}])
}
