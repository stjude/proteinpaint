...
function render_samplegroups( tk, block ) {
	...
	*/
	...
	// map items to view range, exclude unmappable stuff
	tk.samplegroups=[]
	...
	// cnv and sv mapped, if in dense mode, sv moved from samplegroups to svlst
	...
	const h=Math.max( block.labelfontsize, svdensityheight, genebaraxisheight )
	tk.cnvleftg.transition().attr('transform','translate(0,'+h+')')
	tk.cnvmidg.transition().attr('transform','translate(0,'+h+')')
	tk.svdensityg.transition().attr('transform','translate(0,'+h+')')
	tk.cnvrightg.transition().attr('transform','translate(0,'+h+')')
	tk.height_main=tk.toppad+h+cnvheight+tk.bottompad
	tk.config_handle
		.transition()
	...
	*/
	...
	let maxheight=0 // of all bins
	{
		...
		for(const b of bins) {
			...
			b.height=b.groups.reduce((i,j)=>i+j.radius*2,0)
			maxheight = Math.max(maxheight, b.offset+b.height)
		}
	}
