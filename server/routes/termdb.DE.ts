...
*/
	...
	if ((group1names.length <= sample_size_limit && group2names.length <= sample_size_limit) || param.method == 'edgeR') {
		...
		const mds_base64Image = await readFile(mds_imagePath)
		const ql_base64Image = await readFile(ql_imagePath)
