...
import { generateFilterTerm, extractGenesetsFromPromptNew, extractGenesFromPrompt, getGenesetNames } from './utils.ts'
...
export async function phrase2entity(
	...
	if (plotType === 'summary') {
		...
		if (!scaffoldResult.genomeBrowserPhrase && !scaffoldResult.genePhrase) {
			throw new Error(
				`LLM response is missing both genomeBrowserPhrase as well as gene: ${JSON.stringify(scaffoldResult)}`
			)
		}
