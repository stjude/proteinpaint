...
/** HDF5-backed data types that can serve as sources for custom termCollection members.
 *  Each must have a .file property on ds.queries[dataType]. */
const hdf5DataTypes = ['isoformExpression', 'geneExpression', 'dnaMethylation', 'ssGSEA'] as const

/** Set up ds.queries.termCollection for custom (non-dictionary) termCollections.
 *  Data-type agnostic: each member term declares its dataType, and the handler
 *  resolves the HDF5 file from ds.queries[dataType].file. */
export function setupCustomTermCollectionHandler(ds: any) {
	// Only register if at least one HDF5-backed query type is configured
	const hasHdf5Source = hdf5DataTypes.some(dt => ds.queries?.[dt]?.file)
	if (!hasHdf5Source) return

	if (!ds.queries) ds.queries = {}
	ds.queries.termCollection = {
		get: async (param: { tw: any; filter?: any; filter0?: any }, _ds: any) => {
			const tw = param.tw
			const termlst: any[] = tw.term.termlst || []
			if (!termlst.length) throw 'custom termCollection has no termlst'

			// Group member terms by dataType
			const byDataType: Record<string, any[]> = {}
			for (const mt of termlst) {
				const dt = mt.dataType || 'isoformExpression'
				if (!byDataType[dt]) byDataType[dt] = []
				byDataType[dt].push(mt)
			}

			// Query each data type's HDF5 file and merge results per sample
			const sampleValues: Record<string, Record<string, number>> = {}
			for (const [dataType, members] of Object.entries(byDataType)) {
				const queryConfig = ds.queries?.[dataType]
				if (!queryConfig?.file) throw `no HDF5 file configured for dataType '${dataType}'`

				const itemIds = members.map((mt: any) => mt.isoform || mt.gene || mt.id || mt.name)
				const raw = JSON.parse(await queryHDF5(queryConfig.file, itemIds, null))
				const queryOutput = raw.query_output || {}

				for (const mt of members) {
					const itemId = mt.isoform || mt.gene || mt.id || mt.name
					const result = queryOutput[itemId]
					if (!result?.samples) continue
					for (const [sampleName, val] of Object.entries(result.samples as Record<string, number>)) {
						const sampleId = ds.cohort.termdb.q.sampleName2id(sampleName)
						if (sampleId == undefined) continue
						if (!sampleValues[sampleId]) sampleValues[sampleId] = {}
						sampleValues[sampleId][itemId] = val
					}
				}
			}

			const samples: Record<string, any> = {}
			for (const [sampleId, memberValues] of Object.entries(sampleValues)) {
				samples[sampleId] = {
					[tw.$id]: { key: sampleId, value: memberValues }
				}
			}
			return { samples }
		}
