 ...
 * 2. Pipe the JSON to python script to generate a plot
		...
		try {
			...
			try {
				...
				if (parsedRustResult.successful_data && Array.isArray(parsedRustResult.successful_data)) {
					...
					console.log(`[GRIN2] Extracted ${dataForR.length} records for python script`)
					console.log(
			...
			// Prepare input for python script - pass the Rust output and the additional properties as input to R
			...
			console.log(`[GRIN2] python analysis took ${formatElapsedTime(Date.now() - pyAnalysisTime)}`)
