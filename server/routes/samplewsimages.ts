import type { Annotation, Mds3, RouteApi, SampleWSImagesRequest, SampleWSImagesResponse, WSImage } from '#types'
		...
		try {
			...
			if (ds.queries.WSImages.getWSIAnnotations) {
				for (const wsimage of wsimages) {
					...
					console.log('annotationsWSI', annotations)

					if (annotations && annotations.length > 0) {
						//Replace with annotations data??
						wsimage.overlays = ['bla']
