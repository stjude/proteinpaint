...
module.exports = function(common) {
	...
	return {
		dbfile: 'anno/db/pediatric.hg38.db',
		...
		queries: [
			{
				...
				genemcount: {
					query: n => {
						return [
							'select gene_symbol,sample_name,variant_class from snvindel_hg38 where gene_symbol=? collate nocase',
							n
						]
					},
					summary: (mlst, re) => {
				...
				makequery: q => {
					...
					return [
						'select snvindel_hg38.*, sample_master.* from snvindel_hg38 left join sample_master on snvindel_hg38.sample_name=sample_master.sample_name where snvindel_hg38.isoform_accession=? collate nocase',
						q.isoform
					]
				},
				tidy: l => {
