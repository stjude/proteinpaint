...
// Generated boundary CSVs cached on disk like tiles: one file per
// (h5ad path+mtime, kind) under the TTL-swept 'wsitiles' cache subdir, so a
// regenerated h5ad starts fresh keys and the existing sweep evicts old ones.
// Cache-Control only helps each browser; this deduplicates the ~700k-row
// python extraction across clients. In-flight misses share one python run
		...
		try {
			...
			if (req.params.action == 'boundaries' || req.params.action == 'annotations') {
				...
				try {
					...
					// the polygon CSV is generated by python once per h5ad version and
					// served from the disk cache after; concurrent first requests
					...
					if (await exists(cacheFile)) {
						...
						if (!pending) {
							pending = (async () => {
								// python regenerates the CSV into a temp file (same
								// print-a-path contract as tile jobs)
								const tmp = (
