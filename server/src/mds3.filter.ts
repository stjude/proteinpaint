...
export async function mayLimitSamples(
param: FilterParam,
_allSamples: Array<{ name: number } | number> | undefined,
ds: any
): Promise<Set<number> | undefined> {
if (!_allSamples) return // no samples from this big file
const allSamples =
typeof _allSamples[0] === 'object'
? new Set((_allSamples as Array<{ name: number }>).map(i => i.name))
: new Set(_allSamples as number[])
...
let filterSamples: Set<number> | undefined
if (ds.cohort?.db) {
...
filterSamples = new Set((await get_samples(q, ds)).map((i: { id: number }) => i.id))
...
*/
export function combinePPfilterAndTid2value(q: FilterParam, ds: any): Filter | undefined {
const lst: Filter[] = []
if (q.filterObj) {
...
export function tid2value2filter(t: { [termId: string]: any }, ds: any): Filter {
const f: Filter = {
type: 'tvslst',
in: true,
join: 'and',
...
function getFilter0(param: FilterParam, ds: any): any {
