...
// -----------------------------------------------------------------------------
// GSEA (blitzgsea) — deterministic pickle caching
// -----------------------------------------------------------------------------

/** Resolve the `genes` + `fold_change` inputs that every GSEA path needs.
 * Shared between `computeGSEA` (blitzgsea), the GSEA route's cerno
 * branch, and the `fetchDE` short-circuit — a single source of truth for
 * "where do the arrays come from" so the cacheId/daRequest contract is
 * enforced in one place. */
export async function resolveGseaGenesAndFoldChange({
	q,
	genomes
}: {
...
export function computeGseaPickleId({
	genes,
	fold_change,
	geneSetGroup,
	num_permutations,
...
/** In-flight blitzgsea computations keyed by pickle filename. Dedupes
 * concurrent callers with identical inputs so only one Python invocation
 * runs per unique pickleId. Same pattern as `pendingReadOrRecompute`. Only
 * gates the initial-computation path (no geneset_name); detail-plot
 * requests bypass this map. */
const pendingGseaCompute = new Map<string, Promise<GenesetEnrichmentResponse | string>>()
...
export async function computeGSEA({
	q,
	genomes
}: {
	q: GenesetEnrichmentRequest
	...
	// Always recompute the pickle filename from the GSEA inputs. The client
	// no longer carries `pickle_file` — the detail-plot request carries the
	// same inputs as the initial enrichment, so the hash resolves to the
	// same filename and Python finds the previously-written pickle.
	const pickle_file = computeGseaPickleId({
		genes,
		fold_change,
	...
	// In-flight dedup for the initial computation only (detail-plot is a
	// different unit of work per geneset_name and fast).
	if (!q.geneset_name) {
		const inFlight = pendingGseaCompute.get(pickle_file)
		if (inFlight) return inFlight
	}

	const work = runGseaPython({ q, genomes, genes, fold_change, pickle_file })

	if (!q.geneset_name) {
		pendingGseaCompute.set(pickle_file, work)
		return work.finally(() => {
			pendingGseaCompute.delete(pickle_file)
		})
	}
