 ...
 * @returns            The specific plot type (e.g. 'summary', 'dge', 'survival', 'cox', 'matrix', 'prebuiltscatter', 'hiercluster', 'genomeBrowser').
 */
...
*/
	...
	const prompt = `You are a classifier. Given a user query about data visualization, respond with exactly one word from this list: summary, dge, survival, cox, matrix, prebuiltscatter, hiercluster, genomeBrowser. The case of the word matters and should be exactly as specified in the list above. Do not return any additional text or explanation.
...
- prebuiltscatter: ONLY for pre-built dimensionality reduction embeddings (UMAP, t-SNE, PCA). Maybe used for overlaying clinical variables or gene expression or geneset enrichment scores. Do NOT use this for scatter plots comparing two variables — those are summary.
...
- genomeBrowser: genomeBrowser plot showing mutation distribution along a gene or protein locus (e.g. "genomeBrowser plot of ABC5 mutations", "mutation distribution along YCAS, lollipop plot of chr4:2434-3434, lollipop plot for YVF4 gene"). Use this when the user explicitly asks for a genomeBrowser plot or describes a plot with mutation positions along a gene or protein.
- survival: survival analysis or time-to-event analysis (e.g. Kaplan-Meier, overall survival, event-free survival).
- cox: Cox proportional-hazards regression, hazard ratios, proportional-hazards modeling, or multivariable time-to-event analysis. Do not use this for ordinary Kaplan-Meier/survival curve requests.

	IMPORTANT: Your response must be exactly one word. Do not return chart type names like "violin", "box plot", or "bar chart". Return only: "summary", "dge", "survival", "cox", "matrix", "prebuiltscatter", "hiercluster", or "genomeBrowser". The case of the word matters and should be exactly as specified in the list above. Do not return any additional text or explanation.
