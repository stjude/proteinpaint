...
*/
	...
	const prompt = `You are a classifier. Given a user query about data visualization, respond with exactly one word from this list: summary, dge, survival, matrix, prebuiltscatter, hiercluster, genomeBrowser. The case of the word matters and should be exactly as specified in the list above. Do not return any additional text or explanation.
...
IMPORTANT: Your response must be exactly one word. Do not return chart type names like "violin", "box plot", or "bar chart". Return only: "summary", "dge", "survival", "matrix", "prebuiltscatter", "hiercluster", or "genomeBrowser". The case of the word matters and should be exactly as specified in the list above. Do not return any additional text or explanation.   
