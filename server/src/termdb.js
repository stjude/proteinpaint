...
export function handle_request_closure(genomes) {
	...
	return async (req, res) => {
		...
		try {
			...
			if (q.for == 'buildAdHocDictionary') return res.send({ status: await tdb.q.buildAdHocDictionary() })
...
async function trigger_findterm(q, req, res, termdb, ds, genome) {
	...
	try {
		if (q.targetType == TermTypeGroups.DICTIONARY_VARIABLES) {
			...
			if (!ds.queries?.proteome?.whole) throw 'ds.queries.proteome.whole missing for the dataset'
			const matches = await ds.queries.proteome.whole.find([q.findterm])
			const foundTerms = []
...
async function get_mutationSignatureplots(q, req, res, ds, genome) {
	if (q.getPlotDataByName) {
		...
		if (!mutationSignatureplots)
			throw 'ds.cohort.termdb.termCollections Mutation Signature (numeric) plots missing for the dataset'
