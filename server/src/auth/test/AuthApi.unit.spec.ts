...
tape('AuthApi.canDisplaySampleIds: denies a logged-out request on the exact /termdb/matrix path', function (test) {
	...
	const { authApi } = makeAuthApi()
	const ds = { cohort: { termdb: { displaySampleIds: true } }, label: dslabel }
	const req = { query: { embedder, dslabel }, headers: {}, path: '/termdb/matrix', cookies: {} }
	test.equal(
		authApi.canDisplaySampleIds(req, ds as any),
		false,
		'a logged-out request on the protected matrix route must not display sample ids'
	)
	test.end()
})

tape('AuthApi.canDisplaySampleIds: allows a logged-in request on the exact /termdb/matrix path', async function (test) {
	test.timeoutAfter(1000)
	test.plan(1)
	...
	test.equal(
		authApi.canDisplaySampleIds(req, ds as any),
		true,
		'a logged-in request on the protected matrix route should display sample ids'
	)
	test.end()
})
