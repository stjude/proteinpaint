	...
	for (const k in type2group) {
		...
		if (k.includes(bamcommon.type_supportalt)) {
				...
				if (k == bamcommon.type_supportalt + var_idx.toString()) {
					if (q.variant.length == 1) {
						...
						if (g.templates.length == 1) {
							g.messages.push({
								isheader: true,
								t: g.templates.length + ' read supports alternative allele'
							})
						} else {
							g.messages.push({
								isheader: true,
								t: g.templates.length + ' reads supports alternative allele'
							})
						}
					} else {
						if (g.templates.length == 1) {
							g.messages.push({
								isheader: true,
								t: g.templates.length + ' read supports alternative allele with ' + altalleles[var_idx] + ' sequence'
							})
						} else {
							g.messages.push({
								isheader: true,
								t: g.templates.length + ' reads supports alternative allele with ' + altalleles[var_idx] + ' sequence'
							})
						}
					}
				}
			...
			if (g.templates.length == 1) {
				g.messages.push({
					isheader: true,
					t: g.templates.length + ' reference read'
				})
			} else {
				g.messages.push({
					isheader: true,
					t: g.templates.length + ' reference reads'
				})
			}
			...
			if (g.templates.length == 1) {
				g.messages.push({
					isheader: true,
					t: g.templates.length + ' read supports neither reference nor alternative alleles'
				})
			} else {
				g.messages.push({
					isheader: true,
					t: g.templates.length + ' reads supports neither reference nor alternative alleles'
				})
			}
		} else {
			// Should not happen
