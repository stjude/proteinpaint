...
async function maySetAuthRoutes(app, basepath = '', _serverconfig = null) {
	...
	*/
	...
	authApi.mayExtendqFilter = function (q, ds, twArr) {
		...
		// such as when the server route is expected to aggregate the data
		...
		const termsToCheck = twArr.filter(tw => q.__protected__.ignored.includes(tw.term.id)).map(tw => tw.term)
		const additionalFilter = ds.cohort.termdb.getAdditionalFilter(q.__protected__, termsToCheck)
		if (!additionalFilter) {
