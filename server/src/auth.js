...
async function maySetAuthRoutes(app, basepath = '', _serverconfig = null) {
	...
	try {
		...
		for (const dslabel in sessions) {
			for (const id in sessions[dslabel]) {
				...
				const session = await saveSessionToFile(
					'',
					q.dslabel,
					q.id,
					q.time,
	...
	app.post(basepath + '/dslogout', async (req, res) => {
		...
		try {
			...
			delete sessions[q.dslabel][id]
			const ip = req.ip
			await saveSessionToFile(sessionsFile, q.dslabel, id, 0, '', session.ip, session.embedder, session.route, {})
			res.send({ status: 'ok' })
		} catch (e) {
			res.status(code)
			res.send({ error: e })
...
async function saveSessionToFile(sessionsFile, dslabel, id, time, email, ip, embedder, route, clientAuthResult) {
	const session = {
		dslabel,
		id,
		time,
