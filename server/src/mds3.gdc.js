...
export async function querySamplesTwlstNotForGeneexpclustering_noGenomicFilter(q, dictTwLst, ds) {
	...
	const re = await memFetch(
		...
		// do not pass a `signal: q.__abortSignal` option, since this shared request/response
		// may cause the app to fail in different browser tabs or subsequent requests if
		// an aborted request is reused before the relevant cache is deleted
...
export function gdc_validate_query_singleCell_data(ds, genome) {
	...
	ds.queries.singleCell.data.get = async q => {
		const { host, headers } = ds.getHostHeaders(q, { accept: undefined })
		const re = await xfetch(joinUrl(host.rest, 'data', q.sample.eID || q.sample.sID), { headers, timeout: false })
		const lines = re.trim().split('\n')
