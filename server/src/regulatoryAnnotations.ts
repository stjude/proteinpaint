...
import type { RegulatoryAnnotation } from '#types'

/**
 * Maps the primary ENCODE cCRE classification (col5, first comma-delimited token)
 * to a regulatory annotation type. Our file (encodeCCRE.hg38.gz) is 6-column V4 format:
 ...
 * "Promoter_chr8_127800000") for use in DMR analysis visualization.
 */
export async function getRegulatoryAnnotations(
	...
	stop: number,
	shoreSize = 2000
): Promise<RegulatoryAnnotation[]> {
	const annotations: RegulatoryAnnotation[] = []
...
async function queryCpgIslands(
	...
	start: number,
	stop: number,
	annotations: RegulatoryAnnotation[],
	shoreSize: number
): Promise<void> {
 ...
 * Features with unrecognized classifications are skipped.
 */
