...
final_data{}
	...
	.tests{}
*/
export async function runCumincR(fdata, final_data) {
	...
	for (const chartId in ci_data) {
		...
		for (const seriesId in ci_data[chartId].estimates) {
			...
			// this time point should be removed so that the curve starts at the time threshold set in the cuminc() method in termdb.sql.condition.js
			series.shift()
