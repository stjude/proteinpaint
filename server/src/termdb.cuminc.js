...
final_data{}
	.case[]
	.tests{}
startTime: first timepoint of curve. Non-null if a minimum time cutoff is given for an analysis (e.g. 5 year post-diagnosis for SJLIFE enrollment)
*/
export async function runCumincR(fdata, final_data, startTime = null) {
	...
	for (const chartId in ci_data) {
		...
		for (const seriesId in ci_data[chartId].estimates) {
			...
			series[0].time = startTime ? startTime : series[0].time
