import {
	type RoutePayload,
	type TermdbAggregateMatrixRequest,
	type TermdbAggregateMatrixResponse,
	type RouteApi,
	type Filter,
	PSEUDOBULK,
	GENE_EXPRESSION
...
function validateAxisSections(axis: any, name: string) {
	...
	for (const [section, terms] of Object.entries<any>(axis)) {
		...
		if (types.size != 1 || types.has(undefined))
			throw new Error(`${name} section '${section}' must contain one term type`)
...
function init({ genomes }) {
	return async (req, res): Promise<void> => {
		...
		try {
			...
			const terms = Object.values(q.columns)
				.flat()
				.map((tw: any) => tw.term)
			/** There's no term type for the genes themselves. Using geneExpression until non-terms are implemented. */
			const pseudobulkCount = terms.filter(
				(term: any) => term.type == PSEUDOBULK || term.type == GENE_EXPRESSION
			).length
